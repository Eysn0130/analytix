...
@dataclass
class MemoryJanitorSummary:
    scanned_count: int = 0
    stale_count: int = 0
    deleted_count: int = 0
    skipped_count: int = 0
    cleared_case_ids: list[str] = field(default_factory=list)
    ...
    def add_stale(self, reason: str, *, memory_tier: str = "") -> None:
        self.stale_count += 1
        _append_reason(self.stale_reason_counts, reason)
        normalized_tier = str(memory_tier or "").strip()
        if normalized_tier and normalized_tier not in self.stale_tiers:
            self.stale_tiers.append(normalized_tier)

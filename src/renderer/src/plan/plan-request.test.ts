...
describe('plan-request', () => {
  ...
  it('skips internal GUI plan prompts', () => {
    const blocks: ChatBlock[] = [
      ...
      },
      {
        kind: 'user',
        id: 'u3',
        text: 'Please read and execute the GUI plan file at `.analytix/plan/auth.md`'
      },
      {
        kind: 'user',
        id: 'u4',
  ...
  it('recognizes internal plan prompts', () => {
    ...
    expect(isInternalGuiPlanPrompt('Please read and execute the GUI plan file at `.analytix/plan/a.md`')).toBe(true)

    ...
    const block = toolBlock({
      meta: {
        plan: {
          ...
          relative_path: '.analytix/plan/x.md',
          operation: 'draft'
        }
      }
    })
  ...
  it('extracts plan metadata from a successful create_plan tool result', () => {
    const block = toolBlock({
      meta: {
        ...
        plan: {
          plan_id: '/tmp/ws:.analytix/plan/login.md',
          workspace_root: '/tmp/ws',
          relative_path: '.analytix/plan/login.md',
          absolute_path: '/tmp/ws/.analytix/plan/login.md',
          operation: 'draft',
    ...
    expect(meta).toEqual({
      planId: '/tmp/ws:.analytix/plan/login.md',
      workspaceRoot: '/tmp/ws',
      relativePath: '.analytix/plan/login.md',
      absolutePath: '/tmp/ws/.analytix/plan/login.md',
      operation: 'draft',

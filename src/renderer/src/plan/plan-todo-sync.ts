import type { ThreadPlanTodoSource, ThreadTodoItem, ThreadTodoList, ThreadTodoSource, ThreadTodoStatus } from '../agent/types'
...
export function extractPlanTodos(input: {
  ...
  for (const line of lines) {
    ...
    const source: ThreadPlanTodoSource = {
      kind: 'plan',
      planId: input.planId,
...
export function mergePlanTodosForRenderer(input: {
  ...
  const nextItems: ThreadTodoItem[] = input.planItems.map((planItem) => {
    ...
    const closedExisting = existing && (
      existing.status === 'completed' || existing.status === 'failed' || existing.status === 'canceled'
    ) ? existing : undefined
    const status = closedExisting?.status ?? planItem.status
    return {
      ...planItem,
      id: existing?.id ?? planItem.id,
      status,
      ...(closedExisting?.statusReasonCode
        ? { statusReasonCode: closedExisting.statusReasonCode }
        : {}),
      ...(closedExisting?.note ? { note: closedExisting.note } : {}),
      createdAt: existing?.createdAt ?? planItem.createdAt,
      updatedAt:
        existing &&
        existing.content === planItem.content &&
        existing.status === status &&
        existing.statusReasonCode === closedExisting?.statusReasonCode &&
        existing.note === closedExisting?.note
          ? existing.updatedAt
          : input.now
    }
  })
...
): Array<Pick<ThreadTodoItem, 'id' | 'content' | 'status' | 'statusReasonCode' | 'source' | 'note'>> {
...
export function sameTodoWriteItems(
  first: Array<Pick<ThreadTodoItem, 'id' | 'content' | 'status' | 'statusReasonCode' | 'source' | 'note'>>,
  second: Array<Pick<ThreadTodoItem, 'id' | 'content' | 'status' | 'statusReasonCode' | 'source' | 'note'>>
): boolean {
  ...
  return first.every((item, index) => {
    ...
    return Boolean(
      ...
      item.statusReasonCode === other.statusReasonCode &&
...
function makePlanTodoId(source: ThreadPlanTodoSource): string {
...
function findExistingPlanTodo(
  ...
  if (source?.kind !== 'plan') return undefined

...
export function PlanPanel({
  ...
  const planTitle = activePlan?.featureName || t('planPanelTitle')
  const planPathLabel = activePlan?.relativePath ?? t('planNoActiveFile')
  const planStatusBusy =
    saveStatus === 'saving' ||
    operationStatus === 'drafting' ||
    operationStatus === 'refining' ||
    operationStatus === 'building'
  const coverageLabel = trace ? t('planCoverageLabel', { covered: traceCovered, total: trace.blocks.length }) : ''
  const uncoveredLabel = trace && trace.uncoveredIds.length > 0
    ? t('planCoverageUncovered', { ids: trace.uncoveredIds.join(', ') })
    : ''
  const driftLabel = traceDriftIds.length > 0
      ...
      <div className="shrink-0 border-b border-ds-border-muted bg-white/92 dark:bg-ds-card">
        <div className="ds-right-panel-topbar">
          ...
          <div className="ds-right-panel-title-group">
            <ClipboardList className="ds-right-panel-title-icon" strokeWidth={1.8} />
            <span className="ds-right-panel-title" title={planTitle}>
              {planTitle}
            </span>
            <span className="ds-right-panel-subtitle" title={planPathLabel}>
              {planPathLabel}
            </span>
          </div>
          <div className="ds-right-panel-actions">
            <span className="ds-right-panel-chip" title={t(statusKey)}>
              {planStatusBusy ? (
                <Loader2 className="animate-spin" strokeWidth={2} />
              ) : (
                <Save strokeWidth={1.85} />
              )}
              <span className="min-w-0 truncate">{t(statusKey)}</span>
            </span>
            {trace && trace.blocks.length > 0 ? (
              <span className="ds-right-panel-chip" title={coverageLabel}>
                <ShieldCheck strokeWidth={2} />
                <span className="min-w-0 truncate">{`${traceCovered}/${trace.blocks.length}`}</span>
              </span>
            ) : null}
            {uncoveredLabel ? (
              <span
                className="ds-right-panel-chip ds-right-panel-chip-warning"
            ...
            {onVerifyPlan ? (
              <button
                ...
                aria-label={busy ? t('planVerifyRunning') : t('planVerify')}
                title={busy ? t('planVerifyRunning') : t('planVerify')}
              >
                <ShieldCheck className="h-4 w-4" strokeWidth={2} />
              </button>
            ) : null}
            {driftLabel && onReplanChanged ? (
              <button
                type="button"
                onClick={() => onReplanChanged(traceDriftIds)}
                disabled={busy || readOnly}
                className="ds-sidebar-toggle-button ds-right-panel-icon-button disabled:cursor-not-allowed disabled:opacity-45"
                aria-label={t('sddReplanButton')}
                title={driftLabel}
              >

...
import { stripWriteTextAlignDirectivesFromMarkdown } from '@shared/write-text-align'
...
export function WriteWorkspaceView({
  ...
  const [currentTypography, setCurrentTypography] =
    useState<WriteTypographySettingsV1>(() => defaultWriteTypography())
  ...
  useEffect(() => {
    if (!activeFilePath || !activeFileIsText || !isMarkdown || renderSafety.readOnly) return
    const directiveLineCount = fileContent
      .split(/\r?\n/)
      .filter((line) => /^\{:\s*align=(?:left|center|right|justify)\s*\}$/i.test(line.trim()))
      .length
    if (directiveLineCount < 2) return
    const nextContent = stripWriteTextAlignDirectivesFromMarkdown(fileContent)
    if (nextContent !== fileContent) setFileContent(nextContent)
  }, [activeFileIsText, activeFilePath, fileContent, isMarkdown, renderSafety.readOnly, setFileContent])

  const showExportNotice = (notice: WriteNotice): void => {
    showTopNotice({ tone: notice.tone, message: notice.message })
  }
  ...
  const applyTextAlign = (alignment: WriteTextAlign): void => {
    if (!workspaceReady || !activeFilePath || renderSafety.readOnly) return
    const richHandle = richModeActive ? richHandleRef.current : null
    if (richHandle) {
      richHandle.setTextAlign(alignment)
      return
    }
    markdownHandleRef.current?.setTextAlign(alignment)
  }
  ...
  const generateInfographic = (): void => {
    ...
    if (typeof window.analytix?.write?.generateWriteInfographic !== 'function') {
  ...
  const completeInfographicGeneration = async (job: {
    ...
    try {
      const result = await window.analytix.write.generateWriteInfographic({
        text: job.text,
  ...
  const resolveInfographicPlaceholder = async (
    ...
    if (
      ...
      typeof window.analytix?.files?.write !== 'function'
    ) {
      return false
    }
    try {
      const file = await window.analytix.files.read({ path: job.filePath, workspaceRoot })
      ...
      if (next === null) return false
      const written = await window.analytix.files.write({
        path: job.filePath,
        workspaceRoot,
        content: next
      })
  ...
  useEffect(() => {
    if (!fileError) return
    showTopNotice({ tone: 'error', message: fileError })
  }, [fileError, showTopNotice])
  ...
  useEffect(() => {
    ...
    if (
      typeof window.analytix?.files?.watch !== 'function' ||
      typeof window.analytix?.files?.unwatch !== 'function' ||
      typeof window.analytix?.files?.onChanged !== 'function'
    ) {
      return
    }

    return startWriteWorkspaceFileWatch({
      api: window.analytix.files,
      workspaceRoot,
      path: activeFilePath,
  ...
  const officialDocumentPreviewMode: WritePreviewMode =
    activeFileIsText && isMarkdown && renderSafety.livePreviewEnabled
      ? 'rich'
      : activeFileIsText && renderSafety.livePreviewEnabled
        ? 'live'
        : 'source'
  const officialDocumentFormatActive = isWriteOfficialDocumentTypography(currentTypography)
  const stripLegacyAlignmentDirectives = (): void => {
    if (!activeFileIsText || !isMarkdown || renderSafety.readOnly) return
    const nextContent = stripWriteTextAlignDirectivesFromMarkdown(fileContent)
    if (nextContent !== fileContent) setFileContent(nextContent)
  }
  const applyOfficialDocumentFormat = (): void => {
    setPreviewMode(officialDocumentPreviewMode)
    stripLegacyAlignmentDirectives()
  }
  const resetOfficialDocumentFormat = (): void => {
    setPreviewMode('live')
    stripLegacyAlignmentDirectives()
  }
  ...
  return (
    <div
      className="write-workspace-view ds-no-drag flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden px-3 sm:px-4 md:px-6 lg:px-8"
      data-write-official-document={officialDocumentFormatActive ? 'true' : 'false'}
    >
      <WriteWorkspaceToolbar
        ...
        onApplyOfficialDocumentFormat={applyOfficialDocumentFormat}

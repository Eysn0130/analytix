...
export type WriteMarkdownEditorHandle = {
  ...
  /** Rewrite markdown alignment directives for the lines spanning the selection. */
  setTextAlign: (align: WriteTextAlign) => boolean
  /**
      ...
      extensions: [
        ...
        EditorView.domEventHandlers({
          paste(event, view) {
            ...
            if (typeof window.analytix?.files?.saveClipboardImage !== 'function') return false

            event.preventDefault()
            void window.analytix
              .files.saveClipboardImage({
    ...
    if (handleRef) {
      handleRef.current = {
        ...
        setTextAlign: (align) => {
          ...
          const next = applyWriteTextAlignToMarkdownLines(lines, align).join('\n')
          if (instance.state.sliceDoc(startLine.from, endLine.to) === next) return false
          instance.focus()
          instance.dispatch({
            changes: { from: startLine.from, to: endLine.to, insert: next },

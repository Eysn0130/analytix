...
export function ReviewPlanCard({
  ...
  return (
    <div
      title={relativePath}
      className="flex min-h-[64px] w-full items-center gap-3 rounded-[18px] border border-ds-border-muted bg-white/[0.78] px-4 py-3 shadow-[var(--ax-shadow-floating)] backdrop-blur-xl dark:border-white/[0.09] dark:bg-white/[0.045]"
    >
...
export function ReviewSummaryCard({ review }: { review: ReviewBlock }): ReactElement {
  ...
  return (
    <section className="overflow-hidden rounded-[20px] border border-ds-border bg-ds-card/80 shadow-[var(--ax-shadow-panel)] backdrop-blur-xl">
      <button
        type="button"
...
export function TurnChangeSummary({
  ...
  const rewindPlan = changes.find((change) => change.meta?.rewindPlan)?.meta?.rewindPlan
  const totals = useMemo(() => sumDiffStats(changes.map((change) => change.detail)), [changes])
  const title = useMemo(
    () =>
      rewindPlan
        ? t('rewindPlanCardTitle', { count: rewindPlan.summary.fileCount })
        : changes.length === 1
        ? t('turnChangeFilesOne')
        : t('turnChangeFilesMany', { count: changes.length }),
    [changes.length, rewindPlan, t]
  )
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {rewindPlan ? (
            <RotateCcw className={compact ? 'h-4.5 w-4.5' : 'h-5 w-5'} strokeWidth={1.85} />
          ) : (
        ...
        <span className="min-w-0 flex-1">
          ...
          {rewindPlan ? (
            <span className={`block ${compact ? 'mt-0.5 text-[11px]' : 'mt-1 text-[12px]'} text-ds-muted`}>
              {t('rewindPlanNoApply')}
            </span>
          ) : totals ? (
              ...
              <div key={change.id} className="border-b border-ds-border-muted/60 last:border-b-0">
                ...
                >
                  ...
                  {stats ? (
                    ...
                    <span className={`shrink-0 text-ds-muted ${compact ? 'text-[11px]' : 'text-[12px]'}`}>
                      {t('rewindPlanNoApply')}
                    </span>
                  ) : null}
                  {open ? (
                ...
                {open && plan ? (
                  <RewindPlanDetails plan={plan} compact={compact} />
                ) : open && change.detail ? (
...
function RewindPlanDetails({
  ...
  const { t } = useTranslation('common')
  if (!plan) return null
  const visibleFiles = plan.files.slice(0, 8)
  const hiddenCount = Math.max(0, plan.files.length - visibleFiles.length)
  return (
    <div className={`bg-ds-card-muted/45 ${compact ? 'px-3 pb-3 pt-1' : 'px-4 pb-4 pt-1'}`}>
      <div className="grid grid-cols-3 gap-2 text-[11px]">
        <span className="rounded-md border border-ds-border-muted bg-ds-card px-2 py-1 text-ds-muted">
      ...
      {plan.conversation ? (
        <div className="mt-2 text-[11px] leading-5 text-ds-muted">
      ...
      <RewindPlanApplyControls plan={plan} compact={compact} />
      <ul className="mt-2 divide-y divide-ds-border-muted/60 rounded-md border border-ds-border-muted bg-ds-card">
        {visibleFiles.map((file) => (
          <li key={`${file.relativePath}:${file.action}`} className="flex min-w-0 items-start gap-2 px-2.5 py-2">
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate font-mono text-[11px] text-ds-ink" title={file.relativePath}>
                {file.relativePath}
              </span>
              <span className="mt-0.5 block text-[11px] leading-5 text-ds-faint">
        ...
        {hiddenCount > 0 ? (
          <li className="px-2.5 py-2 text-[11px] text-ds-faint">
            {t('rewindPlanMoreFiles', { count: hiddenCount })}
          </li>
        ) : null}
      </ul>
    </div>

...
export function GeneratedOfficeArtifact({ artifact }: { artifact: GeneratedArtifactMetadata }) {
  ...
  const Icon = artifact.kind === 'xlsx' ? Sheet : artifact.kind === 'pptx' ? Presentation : FileText
  const open = async () => {
  ...
  return <div className="min-w-0 max-w-md">
    <button type="button" onClick={() => void open()} disabled={busy} aria-busy={busy}
      ...
      {busy ? <Loader2 className="h-5 w-5 animate-spin motion-reduce:animate-none" aria-hidden="true" /> : <Icon className="h-5 w-5 text-blue-600 dark:text-blue-400" aria-hidden="true" />}
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-medium text-ds-ink">{t(artifact.kind === 'xlsx' ? 'generatedOfficeWorkbook' : artifact.kind === 'pptx' ? 'generatedOfficePresentation' : 'generatedOfficeDocument')}</span>
        <span className="block text-xs text-ds-muted">{artifact.kind.toUpperCase()} · {Math.ceil(artifact.byteSize / 1024)} KB</span>
      </span>

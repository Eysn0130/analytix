...
function bucket(date: string, totalTokens: number, turns = 1): DailyUsageBucket {
  return {
    date,
...
describe('InitialSessionUsageHeatmap', () => {
  ...
  it('renders populated usage with accessible day summaries without starter actions', () => {
    ...
    expect(html).toContain('Daily Analytix usage calendar')
  ...
  it('does not present unknown pricing as zero cost', () => {
  ...
  it('shows configured zero pricing as explicit zero cost', () => {
    const html = render(state({
      usage: usage([
        {
          ...bucket('2026-05-01', 1200),
  ...
  it('renders loading, empty, and error states as calendar-only warmup states', () => {
    ...
    expect(loadingHtml).not.toContain('Daily Analytix usage calendar')

...
export function GitBranchPicker({ workspaceRoot }: Props): ReactElement | null {
  ...
  const load = useCallback(async (): Promise<void> => {
    if (!root || typeof window.analytix?.workspace?.getGitBranches !== 'function') return
    setLoading(true)
    setError(null)
    try {
      const next = await window.analytix.workspace.getGitBranches(root)
      setResult(next)
  ...
  const switchBranch = async (branch: string): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.analytix.workspace.switchGitBranch(root, branch)
      setResult(next)
      if (!next.ok) {
  ...
  const createBranch = async (): Promise<void> => {
    ...
    setError(null)
    try {
      const next = await window.analytix.workspace.createAndSwitchGitBranch(root, branch)
      setResult(next)
      if (!next.ok) {
        setError(next.message)
        return
  ...
  return (
    <div ref={wrapRef} className="ds-git-branch-picker ds-no-drag relative min-w-0">
      ...
      {open ? (
        <div className="absolute bottom-[calc(100%+8px)] left-0 z-50 w-[min(420px,calc(100vw-48px))] overflow-hidden rounded-xl border border-ds-border bg-ds-elevated shadow-[var(--ax-shadow-popover)] backdrop-blur-xl">

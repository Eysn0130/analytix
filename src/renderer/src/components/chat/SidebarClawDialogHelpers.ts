...
export function formatClawInstallError(
  ...
  if (
    ...
    (/unavailable/i.test(value) ||
      ...
      /ANALYTIX_OPENCLAW_GATEWAY_URL/.test(value) ||
...
export const DEFAULT_CLAW_WORKSPACE_ROOT = '~/.analytix/claw'
...
export function clawDefaultChannelWorkspacePreview(
  ...
  if (provider === 'telegram') {
    const username = platformAccount?.kind === 'telegram' && platformAccount.botUsername?.trim()
      ? platformAccount.botUsername
      : target
    const workspaceId = sanitizeWorkspaceSegment(
      platformAccount?.kind === 'telegram' && platformAccount.accountId.trim()
        ? platformAccount.accountId
        : channelId?.trim() || '<bot-id-or-channel-id>',
      'telegram'
    )
    return `${DEFAULT_CLAW_WORKSPACE_ROOT}/${provider}/${sanitizeWorkspaceSegment(username, 'telegram')}/${workspaceId}`
  }
  if (provider === 'weixin') {
    const accountId = platformAccount?.kind === 'weixin'
      ? platformAccount.accountId
      : channelId
  ...
  const domain = sanitizeWorkspaceSegment(
    platformAccount?.kind === 'feishu' ? platformAccount.domain : target,
    target === 'lark' ? 'lark' : 'feishu'
  )
  const workspaceId = platformAccount?.kind === 'feishu' && platformAccount.appId.trim()
    ? sanitizeWorkspaceSegment(platformAccount.appId, 'app')
    : channelId?.trim()

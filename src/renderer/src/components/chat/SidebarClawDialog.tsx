...
import type { ClawImInstallQrResult } from '@shared/analytix-api'
...
export function ClawAddImDialog({
  ...
  const editableChannels = useMemo(
    () => channels
      .filter((channel) => channel.provider === 'weixin')
  ...
  const [platformAccount, setPlatformAccount] = useState<ClawImPlatformAccountV1 | undefined>()
  ...
  useEffect(() => {
    ...
    if (existingChannel) {
      const target = existingChannel.provider === 'weixin'
        ? 'weixin'
        : existingChannel.platformAccount?.kind === 'feishu' && existingChannel.platformAccount.domain === 'lark'
          ? 'lark'
          : 'feishu'
      ...
      setPlatformAccount(existingChannel.platformAccount)
    } else {
  ...
  const resolvedPlatformCredential = platformAccount ?? existingChannel?.platformAccount
  ...
  const startOfficialInstallQr = async (): Promise<void> => {
    ...
    if (typeof window.analytix?.connectPhone?.startImInstallQr !== 'function') {
      setInstallQr({
        status: 'error',
        url: '',
        deviceCode: '',
    ...
    try {
      result = await window.analytix.connectPhone.startImInstallQr(officialInstallProvider, {
    ...
    const waitForInstall = async (): Promise<void> => {
      try {
        const poll = await window.analytix.connectPhone.pollImInstall(officialInstallProvider, result.deviceCode)
        if (installAttempt !== installAttemptRef.current) return
        if (poll.done) {
          clearInstallTimers()
          setPlatformAccount(poll.kind === 'feishu'
            ? {
                kind: poll.kind,
  ...
  return (
    <div className="ds-no-drag fixed inset-0 z-[100] flex items-center justify-center bg-black/35 p-4 backdrop-blur-sm">
      <div className="flex max-h-[min(860px,calc(100vh-32px))] w-full max-w-[1080px] flex-col overflow-hidden rounded-[28px] border border-ds-border bg-ds-elevated shadow-[var(--ax-shadow-modal)]">

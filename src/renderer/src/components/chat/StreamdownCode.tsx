...
const DEFER_CODE_HIGHLIGHT_CHARS = 12000
const DEFER_CODE_HIGHLIGHT_LINES = 240
...
export function shouldDeferCodeHighlight(code: string): boolean {
  if (code.length > DEFER_CODE_HIGHLIGHT_CHARS) return true
  return code.split(/\r\n|\r|\n/).length > DEFER_CODE_HIGHLIGHT_LINES
}
...
function InlineFileReferenceCode({
  ...
  const handleOpenEditor = (): void => {
    void openWorkspacePathInEditor(resolvedTarget, workspaceRoot).then((result) => {
      if (!result.ok) {
        void window.analytix?.logs?.error?.('editor-open', 'Failed to open inline file reference', {

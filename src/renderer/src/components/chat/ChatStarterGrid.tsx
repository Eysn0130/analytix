...
const CHAT_STARTERS: Array<{
  ...
  {
    icon: <AnalytixIconRegistry.icons.workspace className="h-4 w-4" strokeWidth={1.8} />,
    tone: 'blue',
  ...
  {
    icon: <AnalytixIconRegistry.icons.bug className="h-4 w-4" strokeWidth={1.8} />,
    tone: 'emerald',
  ...
  {
    icon: <AnalytixIconRegistry.icons.idea className="h-4 w-4" strokeWidth={1.8} />,
    tone: 'violet',
    ...
    <div className={`${compact ? 'mt-5' : 'mt-12'} grid w-full gap-3 sm:grid-cols-2 ${compact ? 'max-w-none' : 'max-w-[980px]'}`}>
      {CHAT_STARTERS.map((starter) => (
        <button
          ...
          className={`ds-empty-hero-card group flex min-h-[112px] items-center gap-4 rounded-[16px] border px-5 py-4 text-left hover:-translate-y-0.5 ${compact ? 'min-h-[92px]' : ''}`}
        >
          <span

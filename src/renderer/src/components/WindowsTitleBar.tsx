...
import type { DesktopCommand } from '@shared/analytix-api'
import {
  resolveKeyboardShortcutBindings,
  type KeyboardShortcutBindingsV1,
  type KeyboardShortcutCommandId
} from '@shared/keyboard-shortcuts'
import analytixLogo from '../../../asset/brand/analytix-app-icon-512.png'
...
function currentPlatform(): string {
  return typeof window !== 'undefined' ? window.analytix?.app?.platform ?? 'unknown' : 'unknown'
}

function defaultRunDesktopCommand(command: DesktopCommand): Promise<void> {
  if (typeof window === 'undefined' || typeof window.analytix?.app?.runDesktopCommand !== 'function') {
    return Promise.resolve()
  }
  return window.analytix.app.runDesktopCommand(command)
}

function defaultOpenLogDir(): Promise<void> {
  if (typeof window === 'undefined' || typeof window.analytix?.logs?.openDir !== 'function') {
    return Promise.resolve()
  }
  return window.analytix.logs.openDir().then(() => undefined)
}
...
export function WindowsTitleBar({ platform, actions }: Props): ReactElement | null {
  ...
  const defaultActions = useMemo<WindowsTitleBarActions>(() => ({
    ...
    showAbout: async () => {
      const version =
        typeof window !== 'undefined' && typeof window.analytix?.app?.getVersion === 'function'
          ? await window.analytix.app.getVersion().catch(() => '')
          : ''
  ...
  return (
    <div ref={rootRef} data-cursor-spotlight-target className="ds-windows-titlebar ds-drag">
      <div className="ds-windows-titlebar-content">
        <img src={analytixLogo} alt="" aria-hidden="true" className="ds-windows-titlebar-icon" />

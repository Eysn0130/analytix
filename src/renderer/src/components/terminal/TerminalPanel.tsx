...
export function TerminalPanel({ className = '', workspaceRoot, onCollapse, height }: Props): ReactElement {
  ...
  const attachTerminal = useCallback(async (sessionId: string) => {
    ...
    const offData = window.analytix.terminal.onData((payload) => {
      if (payload.sessionId !== sessionId) return
      outputScheduler.enqueue(payload.data)
    })
    const offExit = window.analytix.terminal.onExit((payload) => {
      if (payload.sessionId !== sessionId) return
      setExited(true)
    })
    ...
    const disposable = term.onData((data) => {
      void window.analytix.terminal.write({
        sessionId,
        data
      })
    })
    ...
    const onDimensionChange = (dim: { cols: number; rows: number }): void => {
      void window.analytix.terminal.resize({
        sessionId,
        cols: dim.cols,
        rows: dim.rows
      })
    ...
    try {
      const result = await window.analytix.terminal.create({
        sessionId,
  ...
  const handleCloseTab = useCallback((tabId: string) => {
    ...
    void window.analytix.terminal.dispose(tabId)
    setTabs((current) => {
  ...
  const handleCloseOtherTabs = useCallback((tabId: string) => {
    ...
    for (const tab of tabs) {
      if (tab.id !== tabId) void window.analytix.terminal.dispose(tab.id)
    }
    setTabs([keptTab])
    setActiveTabId(tabId)
    setContextMenu(null)
  ...
  const handleCloseAllTabs = useCallback(() => {
    for (const tab of tabs) {
      void window.analytix.terminal.dispose(tab.id)
    }
    setContextMenu(null)
    cancelRenameTab()
  ...
  const handleRestart = useCallback(async () => {
    ...
    try {
      await window.analytix.terminal.dispose(activeTab.id)
    } catch {
      /* ignore */
    }
    setError(null)

...
export const DEFAULT_WORKSPACE_ROOT = '~/.analytix/default_workspace'
...
export function hasValidPort(settings: AppSettingsV1): boolean {
  const port = getAnalytixRuntimeSettings(settings).port
...
export function mergeSettings(current: AppSettingsV1, patch: SettingsPatch): AppSettingsV1 {
  ...
  const { runtime: runtimePatch, provider: providerPatch, ...restPatch } = patch
  return {
    ...applyAnalytixRuntimePatch(safeCurrent, runtimePatch),
    ...restPatch,
...
export function coerceRendererSettings(settings: AppSettingsV1): AppSettingsV1 {
  ...
  return {
    ...
    theme,
    uiFontScale,
    motionPreference: normalizeAppMotionPreference(raw.motionPreference ?? DEFAULT_APP_MOTION_PREFERENCE),
    cursorSpotlight: raw.cursorSpotlight !== false,
    provider: normalizeModelProviderSettings(raw.provider),
    runtime: mergeAnalytixRuntimeSettings(defaultAnalytixRuntimeSettings(), getAnalytixRuntimeSettings(settings)),

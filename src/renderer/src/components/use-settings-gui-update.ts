...
export function useSettingsGuiUpdate({
  ...
  category: 'account' | 'general' | 'providers' | 'write' | 'imageGeneration' | 'mediaGeneration' | 'speechToText' | 'agents' | 'archives' | 'permissions' | 'browser' | 'computerUse' | 'worktree' | 'memory' | 'shortcuts' | 'easterEgg' | 'claw' | 'updates' | 'debug'
  ...
  const checkGuiUpdate = useCallback(async (): Promise<void> => {
    if (typeof window.analytix?.updates?.check !== 'function') return
    setCheckingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const info = await window.analytix.updates.check(channel)
  ...
  const downloadGuiUpdate = async (): Promise<void> => {
    if (typeof window.analytix?.updates?.download !== 'function') return
    setDownloadingGuiUpdate(true)
    setGuiUpdateProgress(null)
    setGuiUpdateError(null)
    try {
      const result = await window.analytix.updates.download(form?.guiUpdate?.channel)
      if (!result.ok) {
  ...
  const installGuiUpdate = async (): Promise<void> => {
    if (typeof window.analytix?.updates?.install !== 'function') return
    setInstallingGuiUpdate(true)
    setGuiUpdateError(null)
    try {
      const result = await window.analytix.updates.install()
      if (!result.ok) {
        setGuiUpdateError(result.message)
        setInstallingGuiUpdate(false)
      }
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.analytix.updates.onState(applyGuiUpdateState)
    if (typeof window.analytix?.updates?.getState === 'function') {
      void window.analytix.updates.getState().then(applyGuiUpdateState).catch(() => undefined)
    }
    return unsubscribe
  }, [applyGuiUpdateState])

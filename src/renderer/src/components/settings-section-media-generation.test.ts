...
const labels: Record<string, string> = {
  ...
  mediaGenerationDesc: 'No production media tools',
  mediaGenerationUnavailable: 'Unavailable',
  mediaGenerationUnavailableDesc: 'No production runtime tool is registered',
...
describe('MediaGenerationSettingsSection', () => {
  it('marks unsupported production media tools unavailable without exposing saved credentials', () => {
    const html = renderToStaticMarkup(createElement(MediaGenerationSettingsSection, {
      ctx: {
        t,
        selectControlClass: 'select',
        updateAnalytix: vi.fn(),
        provider: {
          providers: [{
            id: 'minimax',
            name: 'MiniMax',
    ...
    expect(html).not.toContain('speech-2.8-hd')
    expect(html).not.toContain('music-2.6')
    expect(html).not.toContain('MiniMax-Hailuo-2.3')

...
describe('WindowsTitleBar', () => {
  ...
  it('keeps the titlebar draggable while menu and window buttons remain interactive', () => {
    const winHtml = renderToStaticMarkup(
      createElement(WindowsTitleBar, { platform: 'win32', actions: testActions() })
    )
    const linuxHtml = renderToStaticMarkup(
    ...
    expect(winHtml).toContain('ds-windows-titlebar ds-drag')
    expect(winHtml).not.toContain('ds-windows-titlebar--traffic-left')
    expect(winHtml).not.toContain('ds-win-traffic-controls ds-no-drag')
    expect(winHtml).not.toContain('data-analytix-window-control=')
    expect(winHtml).toContain('ds-windows-menu ds-no-drag')
    expect(winHtml).toContain('ds-window-controls ds-no-drag')
    expect(linuxHtml).toContain('ds-window-controls ds-no-drag')
  })

...
describe('provider-model-editor', () => {
  it('derives the reasoning protocol from the provider connection', () => {
    ...
    expect(
      defaultReasoningProtocolForProvider(
        provider({ id: 'zhipu-coding-plan', baseUrl: 'https://open.bigmodel.cn/api/coding/paas/v4/chat/completions' })
      )
    ).toBe('glm-chat-completions')
    expect(
      defaultReasoningProtocolForProvider(
        provider({ id: 'custom-zai', baseUrl: 'https://api.z.ai/api/coding/paas/v4/chat/completions' })
      )
    ).toBe('glm-chat-completions')
  })

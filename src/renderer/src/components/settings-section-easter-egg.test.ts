...
const labels: Record<string, string> = {
  ...
  uiModeWorkshopTitle: 'Figure modes',
  uiModeWorkshopDesc: 'Pick the workspace figure. The default figure is Xiezhi.',
  uiModeDefaultTitle: 'Default Xiezhi',
  uiModeDefaultSubtitle: 'Discernment and guardrails',
...
describe('EasterEggSettingsSection (mode workshop)', () => {
  ...
  it('renders the default Xiezhi card and install entry without legacy mascot choices', () => {
    ...
    expect(html).toContain('Figure modes')
    expect(html).toContain('Default Xiezhi')
    expect(html).not.toContain('Default Analytix')
    expect(html).toContain('Install plugin folder…')
    expect(html).toContain('docs/UI_PLUGINS.md')
    // 默认模式应处于使用中状态;旧 mascot 不再作为工坊入口展示。
    expect(html).toContain('Active')
    expect(html).not.toContain('mascot mode')
  })

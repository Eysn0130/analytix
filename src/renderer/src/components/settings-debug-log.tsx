            ...
            <div className="min-h-0 overflow-auto py-2">
              {completionEntries.map((entry) => (
                <WriteDebugLogListButton
                  ...
                  ok={entry.ok}
                  title={`${entry.mode} · ${entry.ok ? entry.model : (entry.errorCode || entry.model)}`}
  ...
  return (
    <div className="space-y-3">
      <WriteDebugMeta
        ...
        t={t}
      />
      <WriteInlineEditDebugText label={t('writeInlineEditDebugPrompt')} value={JSON.stringify(entry.request, null, 2)} />
      <WriteInlineEditDebugText label={t('writeInlineEditDebugSuffix')} value={JSON.stringify(entry.suffix, null, 2)} />
      <WriteInlineEditDebugText label={t('writeCompletionDebugCompletion')} value={JSON.stringify(entry.completion, null, 2)} />
      <WriteInlineEditDebugText label={t('writeInlineEditDebugRawResponse')} value={JSON.stringify(entry.response, null, 2)} />
    </div>
  )
}

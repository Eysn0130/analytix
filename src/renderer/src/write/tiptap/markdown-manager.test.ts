...
describe('write markdown round-trip', () => {
  ...
  it('keeps pending infographic tokens intact across the round trip', () => {
    const token = '![信息图](analytix-pending-infographic://0a1b2c3d-e4f5-6789-abcd-ef0123456789)'
    const doc = `第一段。\n\n${token}\n\n第二段。\n`
    const firstPass = serializeWriteMarkdown(parseWriteMarkdown(doc))
    expect(firstPass).toContain(token)
  })

  it('keeps paragraph alignment directives across the rich editor round trip', () => {
    const doc = '# 标题\n{: align=center}\n\n正文段落。\n{: align=justify}\n'
    const firstPass = serializeWriteMarkdown(parseWriteMarkdown(doc))
    const secondPass = serializeWriteMarkdown(parseWriteMarkdown(firstPass))

    expect(firstPass).toContain('# 标题\n{: align=center}')
    expect(firstPass).toContain('正文段落。\n{: align=justify}')
    expect(secondPass).toBe(firstPass)
  })
})
...
describe('auditWriteMarkdownFidelity', () => {
  ...
  it('rejects ordered-list hard-wrapped continuations that lose characters', () => {
    const doc = [
      '1. Add protocol fields in `analytix/src/contracts/`.',
      '2. Add agent behavior in `analytix/src/loop/`, or a',
      '   new port/adapter under `analytix/src/ports/`.',
      ''
    ].join('\n')

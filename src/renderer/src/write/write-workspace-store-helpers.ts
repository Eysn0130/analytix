...
export const WRITE_PREVIEW_MODE_KEY = 'analytix.write.preview-mode'
export const WRITE_ASSISTANT_OPEN_KEY = 'analytix.write.assistant-open'
export const WRITE_ASSISTANT_MODEL_KEY = 'analytix.write.assistant-model'
export const WRITE_ASSISTANT_PROVIDER_KEY = 'analytix.write.assistant-provider'
const DEFAULT_WRITE_ASSISTANT_MODEL = DEFAULT_ANALYTIX_MODEL
...
export function withResolvedInlineCompletionSettings(
  ...
  settings: Pick<AppSettingsV1, 'provider' | 'runtime' | 'write'>
): {
  defaultWorkspaceRoot: string
  activeWorkspaceRoot: string
  workspaces: string[]
...
export function activeFileStorageKey(workspaceRoot: string): string {
  return `analytix.write.active-file:${normalizePath(workspaceRoot)}`
}

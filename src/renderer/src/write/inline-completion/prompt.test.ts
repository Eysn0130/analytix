  ...
  it('expands a local word candidate to a nearby term from recent replacements', () => {
    const payload = buildInlineCompletionPayload(
      context({
        ...
        currentLineText: 'Another Project gui mention needs alignment.',
        currentLinePrefixTrimmed: 'Another Project gui',
    ...
    expect(payload.editCandidate ? payload.editCandidate.to - payload.editCandidate.from : 0).toBe(11)
  })
})

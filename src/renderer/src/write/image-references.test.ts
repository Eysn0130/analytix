...
const reference = (): ImageRegionNativeReference => ({ kind: 'image-region', id: 'local', threadId: 'thread-a',
  ...
  annotationRevision: 'e'.repeat(64), width: 640, height: 480, region: { x: 10, y: 20, width: 80, height: 60 }, editable: false })
const response = (): ObjectEditingResponse => {
  ...
  return { ok: true, scope: { kind: 'image-region', sessionId: r.sessionId, scopeId: r.scopeId!, objectId: r.objectId,
    ...
    region: r.region, purpose: 'discuss', editable: false, current: true } }
}
...
describe('image region references', () => {
  it('sends only the Core scope handle and does not claim image observation or editing', () => {
    ...
    for (const value of [r.path, r.workspace, r.label, r.objectId, r.revision, r.sessionId, r.annotationRevision]) expect(prompt).not.toContain(value)
  ...
  it.each(['sessionId', 'scopeId', 'objectId', 'threadId', 'sourceRevision', 'annotationRevision', 'region', 'width', 'height'])('rejects stale %s', async field => {
    ...
    const changed = field === 'region' ? { x: 11, y: 20, width: 80, height: 60 } : field === 'width' ? 641 : field === 'height' ? 481
      : field === 'threadId' ? 'another-thread' : 'f'.repeat(['sessionId', 'scopeId'].includes(field) ? 48 : 64)

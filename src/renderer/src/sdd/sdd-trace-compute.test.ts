...
const PLAN_PATH = '.analytixsdd/plan/sdd-x.md'

function todosWith(status: ThreadTodoStatus, rawText: string): ThreadTodoList {
  return {
    threadId: 't1',
    ...
    items: [
      {
        ...
        content: rawText,
        status,
        ...(status === 'failed' ? { statusReasonCode: 'verification_failed' as const } : {}),
        ...(status === 'canceled' ? { statusReasonCode: 'user_canceled' as const } : {}),
        source: {
          kind: 'plan',
          planId: 'p1',
...
describe('computeSddTrace', () => {
  ...
  it('does not upgrade failed or canceled todos to building or done', () => {
    for (const status of ['failed', 'canceled'] as const) {
      const result = computeSddTrace({
        requirementMarkdown: REQUIREMENT,
        planMarkdown: PLAN,
        planRelativePath: PLAN_PATH,
        threadTodos: todosWith(status, '添加导出按钮 (covers: R-1)')
      })
      expect(result.perRequirement[0]).toEqual({ id: 'R-1', totalSteps: 1, doneSteps: 0 })
      expect(result.derivedStatuses).toEqual({})
    }
  })
  ...
  it('ignores todos from other plan files', () => {
    ...
    const source = todos.items[0].source
    if (!source || source.kind !== 'plan') throw new Error('expected plan todo source')
    todos.items[0].source = { ...source, relativePath: '.analytixsdd/plan/other.md' }
    const result = computeSddTrace({
      requirementMarkdown: REQUIREMENT,
      planMarkdown: PLAN,
      planRelativePath: PLAN_PATH,

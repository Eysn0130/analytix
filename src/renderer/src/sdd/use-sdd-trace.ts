...
export function useSddTrace(input: {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      if (typeof window.analytix?.files?.read !== 'function') return
      const requirement = await window.analytix
        .files.read({ workspaceRoot, path: draftRelativePath })
        .catch(() => null)
      if (!cancelled) setDiskRequirement(requirement?.ok ? requirement.content : null)
      if (!planIsActive) {
        const plan = await window.analytix
          .files.read({ workspaceRoot, path: planRelativePath })
          .catch(() => null)
        if (!cancelled) setDiskPlan(plan?.ok ? plan.content : null)
      }
      const tracePath = sddDraftTraceRelativePath(draftRelativePath)
      if (tracePath) {
        const trace = await window.analytix
          .files.read({ workspaceRoot, path: tracePath })
          .catch(() => null)
  ...
  useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      try {
        ...
        if (typeof window.analytix?.files?.write !== 'function') return

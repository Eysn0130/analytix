...
export function serializeSddChatTranscript(
  ...
  for (const block of blocks) {
    if (block.kind === 'user') {
      ...
      const text = projectOrdinaryPublicText(block.meta?.displayText ?? block.text)
      if (!text.trim()) continue
      lines.push('', '---', '', '## 用户', '', text)
      continue
    }
    if (block.kind === 'assistant') {
      const text = projectOrdinaryPublicText(sanitizePublicSerializedText(block.text))
      if (!text) continue
    ...
    if (block.kind === 'tool') {
      ...
      const summary = block.toolKind === 'subagent'
        ? 'Subagent activity'
        : block.toolKind === 'command_execution'
          ...
          : block.toolKind === 'file_change'
            ? 'File activity'
            : 'Tool activity'
      lines.push('', `> [工具] ${summary}${status}`)
      continue
    }
    if (block.kind === 'compaction') {
      const status = block.status === 'success' ? '' : `（${block.status}）`
      lines.push('', `> [工具] Context compaction${status}`)
      continue
    }
    if (block.kind === 'approval') {
      const summary = projectOrdinaryPublicText(sanitizePublicSerializedText(block.summary))
      if (summary) lines.push('', `> [审批] ${summary}（${block.status}）`)
      continue
    }
    if (block.kind === 'review') {
      const title = projectOrdinaryPublicText(sanitizePublicSerializedText(block.title))
      if (title) lines.push('', `> [评审] ${title}（${block.status}）`)
      continue
    }
...
async function writeSddChatMeta(input: {
  ...
  if (input.stampThreadId && isPublicProjectionRevoked(input.stampThreadId)) return
  ...
  try {
    const existing = await window.analytix.files.read({
  ...
  if (input.stampThreadId && isPublicProjectionRevoked(input.stampThreadId)) return
  await window.analytix.files.write({
    workspaceRoot: input.workspaceRoot,
    path: metaPath,
...
export async function writeSddChatTranscriptForThread(input: {
  ...
  if (
    input.historyAuthority === 'case_boundary_only_v1' ||
    isPublicProjectionRevoked(input.threadId)
  ) {
    await purgeSddChatTranscriptForThread(input.threadId)
    return false
  }
  if (typeof window.analytix?.files?.write !== 'function') return false
  ...
  try {
    ...
    if (isPublicProjectionRevoked(input.threadId)) {
      await purgeSddChatTranscriptForThread(input.threadId)
      return false
    }
...
export async function purgeSddChatTranscriptForThread(threadId: string): Promise<void> {
  const ref = sddDraftRefForThreadId(threadId)
  const transcriptPath = ref
    ? sddChatTranscriptRelativePath(ref.draftRelativePath, threadId)
    : null
  if (!ref || !transcriptPath || typeof window.analytix?.files?.deleteEntry !== 'function') return
  try {
    await window.analytix.files.deleteEntry({
      workspaceRoot: ref.workspaceRoot,
      path: transcriptPath
    })
  } catch {
    // Absence and concurrent deletion are already a safe terminal state.
  }
}
...
export function notifySddChatTranscriptMirror(get: () => ChatStateLike): void {
  ...
  const ref = sddDraftRefForThreadId(threadId)
  if (!ref) return
  const historyAuthority = state.threads?.find((thread) => thread.id === threadId)?.historyAuthority
  if (historyAuthority === 'case_boundary_only_v1' || isPublicProjectionRevoked(threadId)) {
...
export async function refreshSddChatTranscriptFromProvider(draft: {
  ...
  if (isPublicProjectionRevoked(threadId)) {
    await purgeSddChatTranscriptForThread(threadId)
    return
  }
  try {
    ...
    if (
      ...
      isPublicProjectionRevoked(threadId)
    ) {
      await purgeSddChatTranscriptForThread(threadId)
      return
    }
    await writeSddChatTranscriptForThread({
      ...
      historyAuthority: detail.historyAuthority
    })
  } catch {
    if (isPublicProjectionRevoked(threadId)) {
      await purgeSddChatTranscriptForThread(threadId)

...
function clawThreadPlaceholder(
  ...
  return {
    id: threadId,
    title: `[Connect Phone:${channel.label}]`,
...
function clawChannelTitle(channel: ClawImChannelV1): string {
  return `[Connect Phone:${channel.label}]`
}
  ...
  return {
    ...
    addClawChannel: async (provider, agentProfile, platformAccount, optionsArg) => {
      ...
      const existing = targetChannelId
        ...
        : null
      if (existing) {
        if (optionsArg?.mainFinalized === true) {
          set({
        ...
        const updatedChannel: ClawImChannelV1 = {
          ...
          platformAccount: platformAccount ?? existing.platformAccount,
          updatedAt: now
        }
      ...
      const channel = newClawChannel(provider, agentProfile, platformAccount)

...
const TURN_MODEL_STORAGE_KEY = 'analytix.turnModelLabel'
    ...
    conversations: [
      {
        ...
        senderId: 'sender-1',
        senderName: 'Alex',
        localThreadId: 'analytix-conversation',
        workspaceRoot: '/Users/zxy/project',
        createdAt: now,
        updatedAt: now
      }
...
describe('chat-store Claw helpers', () => {
  ...
  it('compacts code workspace roots while excluding write, temporary, and Claw roots', () => {
    expect(
      compactCodeWorkspaceRoots([
        ...
        '/Users/zxy/project-a/',
        '/tmp/transient',
        '/Users/zxy/.analytix/claw/agent/conversations/chat',
  ...
  it('deduplicates default workspace aliases', () => {
    ...
    ).toEqual(['~/.analytix/default_workspace'])
  })
  ...
  it('collects channel and conversation thread ids for Claw sessions', () => {
    ...
    expect(ids.has('analytix-channel')).toBe(true)
    expect(ids.has('analytix-conversation')).toBe(true)
  })
  ...
  it('recognizes Claw managed prompt summaries as Claw sessions', () => {
    expect(
      clawThreadTitleLooksManaged(`${CLAW_MANAGED_INSTRUCTIONS_HEADING} legacy scheduled-task tools`)
    ).toBe(true)
    expect(isClawThread({ id: 'connect-phone-thread', title: '[Connect Phone:Feishu Agent]' })).toBe(true)
    expect(isClawThread({ id: 'analytix-leaked', title: '[Claw:Feishu Agent]' })).toBe(true)
  })

  it('recognizes Claw sessions by registered thread id', () => {
    expect(
      isClawThread(
        { id: 'analytix-conversation', title: 'hi' },
        [clawChannel()]
      )
    ).toBe(true)
  })
  ...
  it('falls back to the runtime default model, then known defaults', () => {
    ...
    expect(fallbackComposerModel(pick, 'auto')).toBe('deepseek-v4-flash')
    expect(fallbackComposerModel(pick, 'missing-model')).toBe('deepseek-v4-flash')
    expect(fallbackComposerModel(['a-model'], '')).toBe('a-model')
    expect(fallbackComposerModel([], '')).toBe('')
  })

  it('matches deprecated MiMo composer model aliases to the Xiaomi provider group', () => {
    const xiaomiGroup = {
    ...
    } satisfies ModelProviderModelGroup

    expect(providerIdForComposerModel([xiaomiGroup], 'mimo-v2.5-pro-ultraspeed'))
      .toBe('xiaomi-token-plan')
    expect(composerModelSelectable(
      ['mimo-v2.5-pro'],
      [xiaomiGroup],
      'mimo-v2.5-pro-ultraspeed'
    )).toBe(true)
    expect(canonicalComposerModelForSelection([xiaomiGroup], 'mimo-v2.5-pro-ultraspeed'))
      .toBe('mimo-v2.5-pro')
  })

...
const CODE_WORKSPACE_ROOTS_STORAGE_KEY = 'analytix.codeWorkspaceRoots.v1'
...
export function canonicalComposerModelForSelection(
  modelGroups: readonly ModelProviderModelGroup[],
  modelId: string
): string {
  const model = modelId.trim()
  if (!model) return ''
  const group = modelGroups.find((item) => modelGroupHasModel(item, model))
  return group ? canonicalModelIdForGroup(group, model) : model
}
...
export function composerModelSelectable(
  ...
  if (!composerModelAllowed(pickList, modelId)) {
    if (!group || !group.modelIds.some((id) => composerModelAllowed(pickList, id))) return false
  }
...
function canonicalModelIdForGroup(group: ModelProviderModelGroup, modelId: string): string {
  const model = modelId.trim()
  const key = normalizeComposerModelId(model)
  const direct = group.modelIds.find((id) => normalizeComposerModelId(id) === key)
  if (direct) return direct
  const profiles = group.modelProfiles ?? {}
  for (const [profileId, profile] of Object.entries(profiles)) {
    if (!profile.aliases?.some((alias) => normalizeComposerModelId(alias) === key)) continue
    const canonical = group.modelIds.find((id) => normalizeComposerModelId(id) === normalizeComposerModelId(profileId))
...
export function clawThreadTitleLooksManaged(title: string | undefined): boolean {
  ...
  return trimmed.startsWith(CLAW_MANAGED_INSTRUCTIONS_HEADING) ||
    trimmed.startsWith('[Connect Phone:') ||

...
// cache reads into the per-request count. When the measured total dwarfs our
// estimate by more than this factor, treat it as unreliable and fall back to
// the local estimate.
...
// Screenshots and embedded images are forwarded to the model as bounded vision
// payloads, not as raw base64 text. Tool details are JSON.stringify(output),
// which can still contain huge base64 strings; discount those runs to a flat
// per-image cost so one screenshot does not peg the gauge at 100%.

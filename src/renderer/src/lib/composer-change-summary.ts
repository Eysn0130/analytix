...
export type CurrentTurnDiffSummary = ComposerChangeSummary & {
  fileCount: number
}
...
export function collectCurrentTurnDiffSummary(
  blocks: ChatBlock[],
  currentTurnId: string | null,
  workspaceRoot: string
): CurrentTurnDiffSummary | null {
  ...
  for (const block of blocks) {
    if (
      !(
        ...
        block.meta?.turnId === currentTurnId
      )
    ) {
      continue
    }

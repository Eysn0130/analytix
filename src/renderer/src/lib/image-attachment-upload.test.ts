...
describe('image attachment upload preparation', () => {
  ...
  it('uses filename image MIME hints when pasted files omit their own type', async () => {
    const close = vi.fn()
    ...
    const file = new File([new Uint8Array([1, 2, 3, 4])], 'shot.png', { type: '' })
    const prepared = await prepareImageAttachmentUpload(file, {
      maxImageBytes: 100,
      maxImageDimension: 100,
    ...
    expect(prepared.textFallback.mimeType).toBe('image/png')
    expect(close).toHaveBeenCalledTimes(1)
  })

  it('re-encodes HEIC filename hints instead of mislabeling the original bytes', async () => {
    const close = vi.fn()
    const toBlob = vi.fn((callback: (blob: Blob | null) => void, mimeType: string) => {
      callback(new Blob([new Uint8Array([9, 8, 7])], { type: mimeType }))
    })
    const drawImage = vi.fn()
    vi.stubGlobal('createImageBitmap', vi.fn(async () => ({
      width: 10,
    ...
    const file = new File([new Uint8Array([1, 2, 3, 4])], 'photo.heic', { type: '' })
    const prepared = await prepareImageAttachmentUpload(file, {
      maxImageBytes: 100,
      maxImageDimension: 100,
    ...
    expect(prepared.dataBase64).toBe('CQgH')
    expect(prepared.textFallback.mimeType).toBe('image/webp')

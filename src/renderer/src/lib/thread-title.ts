...
const LEGACY_PLACEHOLDER_TITLES = new Set(['New chat', 'New Thread', '新会话'])
...
export function isInternalPlaceholderThreadTitle(title: string | null | undefined): boolean {
  ...
  return raw === PENDING_AUTO_THREAD_TITLE || INTERNAL_PLACEHOLDER_TITLE_PATTERN.test(raw)
}
...
export function shouldAutoTitleThread(
  ...
  if (isInternalPlaceholderThreadTitle(raw)) return true

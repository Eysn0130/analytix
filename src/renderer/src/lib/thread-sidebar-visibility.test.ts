...
describe('thread-sidebar-visibility', () => {
  ...
  it('hides side child threads from the main sidebar lineage', () => {
    expect(
      shouldHideThreadFromSidebarByLineage(
        thread({ id: 'thr_child', title: 'Child agent', relation: 'side', parentThreadId: 'thr_parent' })
      )
    ).toBe(true)
    expect(
      shouldHideThreadFromSidebarByLineage(
        thread({ id: 'thr_legacy_child', title: 'Child agent', parentThreadId: 'thr_parent' })
      )
    ).toBe(true)
    expect(
      shouldHideThreadFromSidebarByLineage(
        thread({ id: 'thr_fork', title: 'Forked answer', relation: 'fork', parentThreadId: 'thr_parent' })
      )
    ).toBe(false)
  })
  ...
  it('filters internal placeholder and empty fallback threads while keeping real threads', async () => {
    const threads = [
      thread({ id: 'thr_internal01', title: '__codex_parent_title__' }),
      thread({ id: 'thr_279f3fef', title: 'thr_279f' }),
      thread({ id: 'thr_child', title: 'Child agent: Turing', relation: 'side', parentThreadId: 'thr_parent' }),

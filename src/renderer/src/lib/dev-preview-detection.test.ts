...
describe('dev preview detection', () => {
  ...
  it('does not promote a hostile legacy tool result into a preview action', () => {
    const sentinel = 'PRIVATE_DEV_PREVIEW_SENTINEL'
    const block = chatBlockFromItem({
      id: 'item-hostile-preview',
      turnId: 'turn-hostile-preview',
      threadId: 'thread-hostile-preview',
      role: 'tool',
      status: 'completed',
      ...
      toolName: 'bash',
      toolKind: 'command_execution',
      callId: 'call-hostile-preview',
      output: {
        command: 'npm run dev',
        output: `VITE ready in 1 ms Local: http://127.0.0.1:4173/${sentinel}`,

import type { CoreAttachmentTextFallbackJson } from '../agent/analytix-contract'
...
async function encodeImageWithBitmap(
  ...
  const sourceMimeType = file.type || imageMimeTypeFromFileName(file.name) || options.preferredMimeType
  if (imageFitsLimits({

...
export type ClawPlatformInstallPollResult =
  | { done: true; kind: 'feishu'; appId: string; appSecret: string; domain: string }
  | { done: true; kind: 'weixin'; accountId: string; token: string }
...
const WEIXIN_BRIDGE_URL_ENV_KEYS = [
  'ANALYTIX_WEIXIN_BRIDGE_URL',
  'ANALYTIX_OPENCLAW_GATEWAY_URL',
...
export async function startWeixinBridgeChannel(
  accountId: string,
...
export async function startFeishuInstallQrcode(isLark: boolean): Promise<ClawPlatformInstallStartResult> {
  try {
    ...
    return { ok: false, message: 'Feishu/Lark QR setup failed.' }
  }
}
...
export async function pollFeishuInstall(deviceCode: string): Promise<ClawPlatformInstallPollResult> {
  try {
    ...
    if (error) {
      ...
      feishuInstallDomains.delete(deviceCode)
      return { done: false, error: 'Feishu/Lark QR setup status unavailable.' }
    }
    if (!result.ok) {
      feishuInstallDomains.delete(deviceCode)
      return { done: false, error: 'Feishu/Lark QR setup status unavailable.' }
...
export async function startWeixinInstallQrcode(
  ...
  try {
    ...
    return {
      ok: false,
      message: error instanceof Error && error.message === WEIXIN_BRIDGE_MISSING_MESSAGE
        ? WEIXIN_BRIDGE_MISSING_MESSAGE
        : 'WeChat QR setup failed.'
    }
  }
}
...
export async function pollWeixinInstall(
  ...
  try {
    ...
    if (!connected) {
      return { done: false, error: 'WeChat login was not completed.' }
    }
    ...
    if (!alreadyConnected && !token) {
      return { done: false, error: 'WeChat login completed, but no account credential was returned.' }
    }
    weixinInstallSessions.delete(deviceCode)
    return { done: true, kind: 'weixin', accountId, token }
  } catch (error) {
    return {
      done: false,
      error: error instanceof Error && error.message === WEIXIN_BRIDGE_MISSING_MESSAGE
        ? WEIXIN_BRIDGE_MISSING_MESSAGE
        : 'WeChat QR setup status unavailable.'
    }
  }
}

...
export type ClawRuntimeDeps = {
  ...
  resolveAccountCredential?: (scope: AccountCredentialScopeV1) => Promise<
...
export function formatFeishuMirrorText(text: string, direction: 'user' | 'assistant'): { markdown: string } {
  const trimmed = direction === 'user' ? text.trim() : sanitizePublicSerializedText(text)
  if (direction === 'user') {
    return {
      markdown: `**From Analytix**\n\n> ${trimmed.replace(/\n/g, '\n> ')}`
    }
  }
...
export function runtimeErrorMessage(result: RuntimeRequestResult, fallback: string): string {
  ...
  if (parsed) {
    ...
    if (typeof message === 'string' && message.trim()) {
      return sanitizePublicSerializedText(message) || fallback
    }
    const error = parsed.error
    if (typeof error === 'string' && error.trim()) {
      return sanitizePublicSerializedText(error) || fallback
    }
    if (typeof error === 'object' && error !== null) {
      const nested = (error as Record<string, unknown>).message
      if (typeof nested === 'string' && nested.trim()) {
        return sanitizePublicSerializedText(nested) || fallback
      }
    }
  }
  return sanitizePublicSerializedText(result.body) || fallback
}
...
export function latestAssistantText(
  ...
  if (detail.historyAuthority === 'case_boundary_only_v1') return ''
  ...
  for (let index = items.length - 1; index >= 0; index -= 1) {
    ...
    const text = sanitizePublicSerializedText(item.text ?? item.detail ?? item.summary ?? '')
    if (text) return text
  }
  return ''
}

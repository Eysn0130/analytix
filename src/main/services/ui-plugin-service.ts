...
/**
 * UI 插件落盘服务。插件目录:~/.analytix/ui-plugins/<id>/
 * 安装走"白名单复制":只复制 manifest.json 与被 figures 引用到的图片,
 * 源目录里的其它任何文件(脚本、可执行文件等)一概不进入 Analytix 数据目录。
 */
...
export function uiPluginsRootDir(analytixHomeDir: string): string {
  return join(analytixHomeDir, 'ui-plugins')
}
...
> {
  ...
  const manifestResult = normalizeUiPluginManifest(raw)
  if (!manifestResult.ok) {
    return { ok: false, errors: ['manifest.json 内容无效'] }
  }
...
export async function installUiPluginFromDirectory(
  ...
  for (const [slot, relativePath] of Object.entries(manifest.figures)) {
    ...
    if (!check.ok) {
      errors.push(`槽位 ${slot} 加载失败:${check.error}`)
      continue
    }

...
export type GuiSkillFileMutationResult = { ok: true; path: string } | { ok: false; message: string }
...
const MANAGED_HUB_SKILL_PROJECTION_MARKER_V1 = '.analytix-hub-skill.json'

/**
 * Enabled, on-disk skill roots passed to the Analytix runtime. Builds the common
 ...
 * enabled Analytix-owned plugin caches. Precedence (earlier wins on duplicate skill
...
export async function guiSkillRootsForRuntime(
  ...
  hostMaterialization: BundledFundsMaterializationBindingV1 | null =
    currentBundledFundsMaterializationBindingV1()
  ...
  const candidates = buildSkillRootCandidates(settings, workspaceRootOverride).filter((candidate) => {
    ...
    // them deliberately and Analytix tolerates missing roots); common conventions
  ...
  const discoveredPluginRoots = (await discoverCodexPluginSkillRoots())
    .filter((root) => existsSync(root))
    .filter((root) => !disabled.has(comparablePath(root)))
    .filter((root) => fundsPluginSkillRootAllowed(root, hostMaterialization))
  const hasAnalytixComputerUseRoot = discoveredPluginRoots.some(isAnalytixComputerUseSkillRoot)
  const pluginRoots = discoveredPluginRoots
    .filter((root) => !(hasAnalytixComputerUseRoot && isLegacyComputerUseSkillRoot(root)))
...
function fundsPluginSkillRootAllowed(
  root: string,
  hostMaterialization: BundledFundsMaterializationBindingV1 | null
): boolean {
  const parts = comparablePath(root).split('/').filter(Boolean)
  if (!parts.includes('analytix-fund-analysis')) return true
  if (!hostMaterialization || hostMaterialization.pluginName !== 'analytix-fund-analysis' ||
      hostMaterialization.factToolsEnabled || hostMaterialization.publishable) return false
  return comparablePath(root) === comparablePath(join(hostMaterialization.activePluginRoot, 'skills'))
}
 ...
 * added by hand to the Analytix config file.
 */
...
export function guiPluginSkillManagedComparablePrefixes(): string[] {
  return uniqueStrings([
    join(homedir(), '.analytix', 'plugins', 'cache')
  ].map(comparablePath).filter(Boolean))
}

export function isAnalytixComputerUseSkillRoot(path: string): boolean {
  const parts = comparablePath(path).split('/').filter(Boolean)
  return parts.at(-1) === 'skills' && parts.includes('analytix-computer-use')
}

export function isLegacyComputerUseSkillRoot(path: string): boolean {
  const parts = comparablePath(path).split('/').filter(Boolean)
  return parts.at(-1) === 'skills' &&
    parts.includes('computer-use') &&
    !parts.includes('analytix-computer-use')
}
...
export async function listGuiSkills(
  ...
  try {
    const roots = await guiSkillRootsForRuntime(settings, workspaceRootOverride, hostMaterialization)
...
export async function saveGuiSkillFile(
  rootPathRaw: string,
  skillNameRaw: string,
  content: string
): Promise<GuiSkillFileMutationResult> {
  try {
    const rootPath = normalizeSkillRootPath(rootPathRaw)
    if (!rootPath) return { ok: false, message: 'Skill directory is required.' }
    const skillDir = skillDirectoryPath(rootPath, skillNameRaw)
...
export async function deleteGuiSkillPackage(
  rootPathRaw: string,
  skillNameRaw: string
): Promise<GuiSkillFileMutationResult> {
  try {
    const rootPath = normalizeSkillRootPath(rootPathRaw)
    if (!rootPath) return { ok: false, message: 'Skill directory is required.' }
    const skillDir = isSkillPackageDirectory(rootPath)
      ? rootPath
      : skillDirectoryPath(rootPath, skillNameRaw)
...
function skillDirectoryPath(rootPath: string, skillNameRaw: string): string {
  const skillName = normalizeSkillFolderName(skillNameRaw)
  const skillDir = resolve(rootPath, skillName)
  const rootComparable = comparablePath(rootPath)
  const skillComparable = comparablePath(skillDir)
  if (skillComparable !== rootComparable && skillComparable.startsWith(`${rootComparable}/`)) {
    return skillDir
  }
  throw new Error('Skill path must stay inside the selected skill directory.')
}

function isSkillPackageDirectory(path: string): boolean {
...
async function discoverCodexPluginSkillRoots(): Promise<string[]> {
  const roots: string[] = []
  await collectSkillRoots(join(homedir(), '.analytix', 'plugins', 'cache'), roots, 0, 5)
  return roots
}
...
async function packageCandidates(root: string): Promise<string[]> {
  ...
  if (!isInertOrInvalidManagedHubSkillProjection(root) &&
      (existsSync(join(root, 'skill.json')) || existsSync(join(root, 'SKILL.md')))) {
    candidates.add(root)
  }
  ...
  for (const entry of entries) {
    ...
    if (!isInertOrInvalidManagedHubSkillProjection(dir) &&
        (existsSync(join(dir, 'skill.json')) || existsSync(join(dir, 'SKILL.md')))) {
      candidates.add(dir)
    }
  }
  return [...candidates]
...
function isInertOrInvalidManagedHubSkillProjection(path: string): boolean {
  const markerPath = join(path, MANAGED_HUB_SKILL_PROJECTION_MARKER_V1)
  try {
    const markerInfo = lstatSync(markerPath)
    if (markerInfo.isSymbolicLink() || !markerInfo.isFile()) return true
    const marker = parseStrictJsonObject(readFileSync(markerPath), {
      maxBytes: 64 * 1024,
      maxDepth: 4,
      maxTokens: 32,
      maxStringBytes: 4096
    ...
    if (actualKeys.length !== expectedKeys.length ||
        actualKeys.some((key, index) => key !== expectedKeys[index])) return true
    const skillName = basename(path)
    const valid = marker.managedBy === 'analytix-hub' &&
      typeof marker.platform === 'string' && marker.platform.trim() !== '' &&
      typeof marker.pluginName === 'string' && marker.pluginName.trim() !== '' &&
      marker.skillName === skillName &&
      marker.skillPath === `skills/${skillName}/SKILL.md` &&
      marker.sourceKind === 'plugin' &&
      typeof marker.version === 'string' && marker.version.trim() !== ''
    if (!valid) return true
    // Funds projections are presentation/migration copies. They cannot be
    // executable authority; the receipt-bound direct-cache generation is
    // discovered independently. Valid projections owned by other plugins
    // retain their existing behavior.
    return marker.pluginName === 'analytix-fund-analysis'
  } catch (error) {

...
describe('workspace-service boundary checks', () => {
  ...
  beforeEach(async () => {
    ...
    rootDir = await mkdtemp(join(tmpdir(), 'analytix-workspace-'))
    ...
    await rm(join(tmpdir(), 'analytix'), { recursive: true, force: true })
  })
  ...
  it('reads clipboard images as PNG base64 and saves a temp file', async () => {
    ...
    expect(dirname(result.localFilePath)).toBe(join(tmpdir(), 'analytix'))
  ...
  it('saves SDD pasted clipboard images into .analytixsdd/img with draft-relative markdown', async () => {
    const currentFilePath = join(workspaceRoot, '.analytixsdd', 'draft', 'draft-1', 'requirement.md')
    await mkdir(join(workspaceRoot, '.analytixsdd', 'draft', 'draft-1'), { recursive: true })
    ...
    const result = await saveWorkspaceClipboardImage({
      workspaceRoot,
      currentFilePath,
      imageDirectory: '.analytixsdd/img'
    })

    expect(result.ok).toBe(true)
    if (!result.ok) return

    expect(await realpath(dirname(result.path))).toBe(await realpath(join(workspaceRoot, '.analytixsdd', 'img')))
    expect(result.markdownPath.startsWith('../../img/pasted-image-')).toBe(true)

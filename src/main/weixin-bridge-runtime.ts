...
const MAX_WEIXIN_API_RESPONSE_BYTES = 4 * 1024 * 1024
...
export type WeixinBridgeAccountCredentialResolution =
  | { status: 'ready'; token: string; channelId: string; generation: string; incarnation: string }
  | { status: 'unavailable'; channelId: string; generation: string; incarnation: string }

export type WeixinBridgeContextTokenAuthority = {
  put: (input: {
    accountId: string
    channelId: string
    userId: string
...
let accountCredentialResolver:
  | ((accountId: string) => Promise<WeixinBridgeAccountCredentialResolution>)
  | null = null
let contextTokenAuthority: WeixinBridgeContextTokenAuthority | null = null
...
const WEIXIN_CONTEXT_TOKEN_TTL_MS = 6 * 60 * 60 * 1000
...
export function configureWeixinBridgeAccountCredentialResolver(
  resolver: ((accountId: string) => Promise<WeixinBridgeAccountCredentialResolution>) | null
): void {
  accountCredentialResolver = resolver
}

export function configureWeixinBridgeContextTokenAuthority(
  authority: WeixinBridgeContextTokenAuthority | null
): void {
  contextTokenAuthority = authority
}
...
function readWeixinPackageInfo(): WeixinPackageInfo {
  ...
  if (!packageJson) {
    throw new Error(
      'Built-in WeChat login component is missing. Reinstall Analytix or rebuild with @tencent-weixin/openclaw-weixin bundled.'
    )
  }
...
function buildBaseInfo(): JsonRecord {
  ...
  return {
    ...
    bot_agent: `Analytix/${app.getVersion() || '0.0.0'}`
  }
}
...
async function readJsonResponse(res: Response): Promise<JsonRecord> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > MAX_WEIXIN_API_RESPONSE_BYTES) {
    await res.body?.cancel().catch(() => undefined)
    throw new Error('WeChat provider response was rejected.')
  }
  ...
  if (res.body) {
    ...
    try {
      while (true) {
        ...
        if (total > MAX_WEIXIN_API_RESPONSE_BYTES) {
          throw new Error('WeChat provider response was rejected.')
        }
        chunks.push(value)
      }
  ...
  try {
    return bytes.length > 0 ? JSON.parse(bytes.toString('utf8')) as JsonRecord : {}
  } catch {
    throw new Error('WeChat provider response was rejected.')
  } finally {
    bytes.fill(0)
...
async function apiPost(
  ...
  const data = await readJsonResponse(res)
  if (!res.ok) {
    throw new Error(`${options.label} failed (HTTP ${res.status}).`)
  }
  return data
}
...
async function resolveWeixinAccount(accountId: string): Promise<WeixinAccount> {
  ...
  const data = await loadWeixinAccountData(id)
  const resolution = accountCredentialResolver
    ? await accountCredentialResolver(id)
    : null
  const token = resolution?.status === 'ready' ? resolution.token.trim() : undefined
  return {
    accountId: id,
    ...
    generation: resolution?.generation ?? '0',
    incarnation: resolution?.incarnation ?? ''
  }
}
...
async function startWeixinLogin(params: JsonRecord): Promise<JsonRecord> {
  ...
  let qr: JsonRecord
  try {
    qr = await fetchQRCode(recordString(params, 'botType') || WEIXIN_DEFAULT_BOT_TYPE)
  } catch {
    throw new Error('WeChat QR setup failed.')
  }
  ...
  while (Date.now() < deadline) {
    ...
    switch (recordString(status, 'status')) {
      ...
      case 'confirmed': {
        ...
        await saveWeixinAccount(accountId, { baseUrl, userId })
...
async function setContextToken(account: WeixinAccount, userId: string, token: string): Promise<void> {
  if (!contextTokenAuthority || !account.channelId || !token.trim() || !userId.trim()) return
  await contextTokenAuthority.put({
    accountId: account.accountId,
    channelId: account.channelId,
    userId: userId.trim(),
    token: token.trim(),
    parentGeneration: account.generation,
    parentIncarnation: account.incarnation,
    expiresAtMs: Date.now() + WEIXIN_CONTEXT_TOKEN_TTL_MS
  })
}

async function getContextToken(account: WeixinAccount, userId: string): Promise<string | undefined> {
  if (!contextTokenAuthority || !account.channelId || !userId.trim()) return undefined
  return contextTokenAuthority.resolve({
    accountId: account.accountId,
    channelId: account.channelId,
    userId: userId.trim(),
    parentGeneration: account.generation,
...
function generateMessageId(): string {
  return `analytix-weixin-${randomUUID()}`
}
...
async function sendGeneratedFilesWeixin(
  ...
  for (const file of files) {
    const current = await resolveWeixinAccount(account.accountId)
    if (!current.configured || !current.token?.trim() ||
      current.generation !== account.generation || current.incarnation !== account.incarnation) return
    try {
      ...
      await sendWeixinMediaFile({
        ...
        to,
        text: '',
        opts: { baseUrl: current.baseUrl, token: current.token, contextToken },
        cdnBaseUrl: current.cdnBaseUrl
      })
      const after = await resolveWeixinAccount(account.accountId)
      if (!after.configured || after.generation !== current.generation ||
        after.incarnation !== current.incarnation) return
    } catch {
      logWarn('weixin-bridge', 'Failed to send generated file to WeChat.', {
        accountId: account.accountId
      })
      const fallbackAccount = await resolveWeixinAccount(account.accountId)
      if (!fallbackAccount.configured || !fallbackAccount.token?.trim() ||
        fallbackAccount.generation !== account.generation ||
        fallbackAccount.incarnation !== account.incarnation) return
...
async function postToDeepSeekGuiWebhook(message: WeixinMessage, accountId: string): Promise<JsonRecord> {
  ...
  if (settings.webhookSecret) {
    ...
    headers['x-analytix-secret'] = settings.webhookSecret
    headers['x-analytix-secret'] = settings.webhookSecret
  }
  ...
  if (!res.ok || data.ok === false) {
    throw new Error(`Analytix webhook failed (HTTP ${res.status}).`)
  }
  return data
}
...
async function monitorWeixinAccount(accountId: string, signal: AbortSignal): Promise<void> {
  ...
  const initial = await resolveWeixinAccount(normalizedAccountId)
  if (!initial.configured || !initial.token?.trim()) {
    throw new Error(`WeChat account is not configured: ${accountId}`)
  }
  await restoreContextTokens(normalizedAccountId)
  try {
    await notifyStart(initial)
  } catch {
    /* best-effort */
  }

  let getUpdatesBuf = await loadSyncBuf(normalizedAccountId)
  ...
  const dispatchToSender = (message: WeixinMessage, to: string, contextToken: string | undefined): void => {
    const task = async (): Promise<void> => {
      ...
      const prior = await resolveWeixinAccount(normalizedAccountId)
      if (!prior.configured || !prior.token?.trim()) return
      const result = await postToDeepSeekGuiWebhook(message, normalizedAccountId)
      const account = await resolveWeixinAccount(normalizedAccountId)
      if (
        !account.configured ||
        !account.token?.trim() ||
        account.generation !== prior.generation ||
        account.incarnation !== prior.incarnation
      ) return
      ...
      const afterText = await resolveWeixinAccount(normalizedAccountId)
      if (!afterText.configured || !afterText.token?.trim() ||
        afterText.generation !== account.generation || afterText.incarnation !== account.incarnation) return
      // Generated media files arrive alongside the text reply and go out as
      // native image / file messages.
      await sendGeneratedFilesWeixin(afterText, to, webhookGeneratedFiles(result), contextToken)
    }
  ...
  while (!signal.aborted) {
    try {
      ...
      const current = await resolveWeixinAccount(normalizedAccountId)
      if (
        !current.configured ||
        current.generation !== account.generation ||
        current.incarnation !== account.incarnation
      ) {
        continue
      }
      ...
      if (nextBuf) {
        ...
        await saveSyncBuf(normalizedAccountId, getUpdatesBuf)
      }
      ...
      for (const message of messages) {
        ...
        if (contextToken) await setContextToken(current, to, contextToken)
  ...
  try {
    const current = await resolveWeixinAccount(normalizedAccountId)
    if (current.configured && current.token?.trim()) await notifyStop(current)
  } catch {
    /* best-effort */
  }
}
...
function startAccountMonitor(accountId: string): void {
  ...
  const promise = monitorWeixinAccount(normalized, controller.signal).catch(() => {
...
export async function sendWeixinBridgeMessage(options: {
  ...
  try {
    ...
    const result = await sendMessageWeixin({
      ...
      to,
      text,
      contextToken: await getContextToken(account, to)
    })
    const current = await resolveWeixinAccount(accountId)
    if (
      !current.configured ||
      current.generation !== account.generation ||
      current.incarnation !== account.incarnation
    ) {
      return { ok: false as const, message: 'WeChat account credential changed during delivery.' }
    }
    ...
    return { ok: false, message: 'WeChat message delivery failed.' }
  }
}

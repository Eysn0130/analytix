 ...
 *
 * 用户可见产品名使用 Analytix。package、executable、CLI、协议和 userData
 * 目录等 machine identity 仍保持 lowercase analytix；electron-builder 的
 * appId / bundle id / AppUserModelID 统一为 com.analytix.desktop。
 */
export const APP_PRODUCT_NAME = 'Analytix'
...
export function configureAppIdentity(env: NodeJS.ProcessEnv = process.env): void {
  const userDataOverride = env[ANALYTIX_USER_DATA_DIR_ENV]?.trim()
  const userDataPath = userDataOverride
    ? resolve(userDataOverride)
    : resolve(app.getPath('appData'), APP_USER_DATA_DIRECTORY_NAME)

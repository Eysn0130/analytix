...
describe('gui-plan path validation', () => {
  it('accepts direct Markdown files under the plan directory', () => {
    ...
    expect(isGuiPlanCurrentRelativePath('.analytix/plan/login.md')).toBe(false)
  })
  ...
  it('produces a stable plan id from workspace and path', () => {
    expect(buildGuiPlanId('/tmp/ws', '.analytixsdd/plan/login.md')).toBe(
      '/tmp/ws:.analytixsdd/plan/login.md'
    )
    expect(buildGuiPlanId('/tmp/ws', '.analytixsdd/plan/Login.md')).toBe(
      buildGuiPlanId('/tmp/ws', '.analytixsdd/plan/login.md')
    )
    expect(buildGuiPlanId('/tmp/ws', '.analytix/plan/Login.md')).toBe(
      buildGuiPlanId('/tmp/ws', '.analytix/plan/login.md')
    )
  })

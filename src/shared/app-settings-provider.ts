...
import { normalizeModelEndpointFormat, type ModelEndpointFormat } from '../../packages/runtime/src/contracts/model-endpoint-format.js'
import { getAnalytixRuntimeSettings } from './app-settings-runtime'
...
const DEPRECATED_PROVIDER_MODEL_ALIASES: Record<string, Record<string, string>> = {
  xiaomi: {
...
export function normalizeModelProviderSettings(
  ...
  const hasTopLevelBaseUrl = typeof input?.baseUrl === 'string' && input.baseUrl.trim() !== ''
  ...
  const defaultProvider = defaultModelProviderProfile(baseUrl)
  ...
  for (const rawProvider of rawProviders) {
    ...
    providersById.set(provider.id, provider.id === DEFAULT_MODEL_PROVIDER_ID
      ? {
          ...defaultProvider,
          ...provider,
          baseUrl: hasTopLevelBaseUrl ? baseUrl : provider.baseUrl || baseUrl,
          modelProfiles: {
  ...
  const normalizedDefaultProvider = providersById.get(DEFAULT_MODEL_PROVIDER_ID) ?? defaultProvider
  const providers = [...providersById.values()]
  const requestedActiveProviderId = normalizeModelProviderId(input?.activeProviderId)
  const activeProviderId = requestedActiveProviderId &&
    providers.some((provider) => provider.id === requestedActiveProviderId)
    ? requestedActiveProviderId
    : ''
  return {
    activeProviderId,
    baseUrl: hasTopLevelBaseUrl ? baseUrl : normalizedDefaultProvider.baseUrl || baseUrl,
...
export function resolveModelProviderBaseUrl(settings: AppSettingsV1): string {
  return normalizeDeepseekBaseUrl(getActiveModelProviderProfile(settings).baseUrl)
}
...
export function getActiveModelProviderId(
  settings: AppSettingsV1,
  providerId: string | undefined = undefined
): string {
  const provider = getModelProviderSettings(settings)
  const requestedProviderId = normalizeModelProviderId(providerId)
  if (requestedProviderId && provider.providers.some((profile) => profile.id === requestedProviderId)) {
    return requestedProviderId
  }
  if (provider.activeProviderId && provider.providers.some((profile) => profile.id === provider.activeProviderId)) {
...
export function getActiveModelProviderProfile(
  ...
  const activeProviderId = getActiveModelProviderId(settings, providerId)
  return getModelProviderProfile(settings, activeProviderId || DEFAULT_MODEL_PROVIDER_ID)
}

export function getModelProviderIdForModel(
  settings: AppSettingsV1,
  modelId: string,
  preferredProviderId: string | undefined = undefined
): string {
  const provider = getModelProviderSettings(settings)
  const normalizedModel = normalizeModelKey(modelId)
  if (!normalizedModel) return ''
  const matchesModel = (profile: ModelProviderProfileV1): boolean => {
    const providerModel = normalizeModelKey(normalizeDeprecatedProviderModelId(profile.id, modelId))
    if (profile.models.some((model) => normalizeModelKey(model) === providerModel)) return true
    return Object.entries(profile.modelProfiles).some(([profileModelId, profile]) =>
      normalizeModelKey(profileModelId) === providerModel ||
      normalizeProviderModels(profile.aliases).some((alias) =>
        normalizeModelKey(alias) === normalizedModel || normalizeModelKey(alias) === providerModel
      )
    )
  }

  const preferredId = normalizeModelProviderId(preferredProviderId)
  if (preferredId) {
    const preferred = provider.providers.find((profile) => profile.id === preferredId)
    if (preferred && matchesModel(preferred)) return preferred.id
  }

  const activeProviderId = normalizeModelProviderId(provider.activeProviderId)
  if (activeProviderId) {
    const active = provider.providers.find((profile) => profile.id === activeProviderId)
    if (active && matchesModel(active)) return active.id
  }

  return provider.providers.find(matchesModel)?.id ?? ''
}

export type ModelProviderRequestSelection = {
  providerId?: string
  model: string
}

function modelProviderHasBoundedChatCatalog(provider: Pick<ModelProviderProfileV1, 'models' | 'modelProfiles'>): boolean {
  return provider.models.some((model) => Boolean(model.trim())) ||
    Object.keys(provider.modelProfiles).some((model) => Boolean(model.trim()))
}

function firstProviderTextChatModel(
  provider: Pick<ModelProviderProfileV1, 'id' | 'models' | 'modelProfiles'>,
  fallbackModel = ''
): string {
  const seen = new Set<string>()
  const candidates = [
    ...(provider.id === DEFAULT_MODEL_PROVIDER_ID ? [DEFAULT_ANALYTIX_MODEL] : []),
    ...provider.models,
    ...Object.keys(provider.modelProfiles)
  ]
  for (const candidate of candidates) {
    const model = normalizeDeprecatedProviderModelId(provider.id, candidate)
    const normalized = normalizeModelKey(model)
    if (!normalized || seen.has(normalized)) continue
    seen.add(normalized)
    const profile = modelProviderModelProfile(provider, model) ?? modelProviderModelProfileByAlias(provider, model)
    if (!isComposerChatModelId(model) || !modelProfileSupportsTextChat(profile)) continue
    return model
  }
  const fallback = normalizeDeprecatedProviderModelId(provider.id, fallbackModel)
  return isComposerChatModelId(fallback) ? fallback : ''
}

export function modelProviderSupportsChatModel(
  provider: Pick<ModelProviderProfileV1, 'id' | 'models' | 'modelProfiles'>,
  modelId: string
): boolean {
  const model = normalizeDeprecatedProviderModelId(provider.id, modelId)
  const normalizedModel = normalizeModelKey(model)
  if (!normalizedModel || !isComposerChatModelId(model)) return false

  const directProfile = modelProviderModelProfile(provider, model) ?? modelProviderModelProfileByAlias(provider, model)
  if (directProfile && !modelProfileSupportsTextChat(directProfile)) return false
  if (!modelProviderHasBoundedChatCatalog(provider)) return true

  const providerModelMatched = provider.models.some((candidate) =>
    normalizeModelKey(normalizeDeprecatedProviderModelId(provider.id, candidate)) === normalizedModel
  )
  if (providerModelMatched) return true

  return Object.entries(provider.modelProfiles).some(([profileModelId, profile]) => {
    if (!modelProfileSupportsTextChat(profile)) return false
    const normalizedProfileModel = normalizeModelKey(normalizeDeprecatedProviderModelId(provider.id, profileModelId))
    if (normalizedProfileModel === normalizedModel) return true
    return normalizeProviderModels(profile.aliases).some((alias) =>
      normalizeModelKey(normalizeDeprecatedProviderModelId(provider.id, alias)) === normalizedModel
    )
  })
}

export function resolveModelProviderRequestSelection(
  settings: AppSettingsV1,
  input: {
    model?: string
    providerId?: string
...
): ModelProviderRequestSelection {
  const providerSettings = getModelProviderSettings(settings)
  const rawModel = typeof input.model === 'string' ? input.model.trim() : ''
  const requestedProviderId = normalizeModelProviderId(input.providerId)
  const runtimeProviderId = normalizeModelProviderId(input.runtimeProviderId)
  const providerForId = (id: string): ModelProviderProfileV1 | undefined =>
    providerSettings.providers.find((provider) => provider.id === id)

  let provider = requestedProviderId ? providerForId(requestedProviderId) : undefined
  if (!provider && rawModel) {
    const modelProviderId = getModelProviderIdForModel(settings, rawModel, runtimeProviderId)
    provider = modelProviderId ? providerForId(modelProviderId) : undefined
  }
  if (!provider) {
    const activeProviderId = getActiveModelProviderId(settings, runtimeProviderId)
    provider = getModelProviderProfile(settings, activeProviderId || DEFAULT_MODEL_PROVIDER_ID)
  }

  const requestedModel = normalizeDeprecatedProviderModelId(provider.id, rawModel)
  const model = requestedModel && modelProviderSupportsChatModel(provider, requestedModel)
    ? requestedModel
    : firstProviderTextChatModel(provider, requestedModel)
  return {
...
export function normalizeModelProviderModelIdForRequest(
  settings: AppSettingsV1,
  modelId: string,
  providerId: string | undefined = undefined
): string {
  ...
  if (!model) return ''
  return resolveModelProviderRequestSelection(settings, {
    model,
    providerId,
...
export function getModelProviderProfile(
  ...
  return provider.providers.find((profile) => profile.id === id) ?? provider.providers[0] ?? defaultModelProviderProfile(provider.baseUrl)
}
...
export function resolveModelProviderEndpointFormat(
  provider: Pick<ModelProviderProfileV1, 'endpointFormat' | 'modelProfiles'>,
  modelId: string
): ModelEndpointFormat {
  return (
    modelProviderModelProfile(provider, modelId)?.endpointFormat ??
    modelProviderModelProfileByAlias(provider, modelId)?.endpointFormat ??
    provider.endpointFormat ??
    DEFAULT_MODEL_ENDPOINT_FORMAT
  )
}

function modelProviderModelProfileByAlias(
  provider: Pick<ModelProviderProfileV1, 'modelProfiles'>,
  modelId: string
): ModelProviderModelProfileV1 | undefined {
  const normalized = normalizeModelKey(modelId)
  if (!normalized) return undefined
  return Object.values(provider.modelProfiles).find((profile) =>
    normalizeProviderModels(profile.aliases).some((alias) => normalizeModelKey(alias) === normalized)
  )
}
...
type AnalytixMediaSettingCore = Partial<{
  enabled: boolean
  providerId: string
  baseUrl: string
  model: string
...
export function defaultMiniMaxMediaGenerationAnalytixPatch(input: {
...
}): AnalytixRuntimeSettingsPatchV1 | undefined {
  const patch: AnalytixRuntimeSettingsPatchV1 = {}
  if (!input.analytixPatch?.textToSpeech && isBlankAnalytixMediaSetting(input.currentAnalytix?.textToSpeech)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'textToSpeech', input.currentAnalytix?.providerId)
    if (match) {
      patch.textToSpeech = {
        enabled: true,
  ...
  if (!input.analytixPatch?.musicGeneration && isBlankAnalytixMediaSetting(input.currentAnalytix?.musicGeneration)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'music', input.currentAnalytix?.providerId)
    if (match) {
  ...
  if (!input.analytixPatch?.videoGeneration && isBlankAnalytixMediaSetting(input.currentAnalytix?.videoGeneration)) {
    const match = configuredMiniMaxMediaCapability(input.providers, 'video', input.currentAnalytix?.providerId)
    if (match) {
...
function isBlankAnalytixMediaSetting(setting: AnalytixMediaSettingCore | undefined): boolean {
...
function providerWithPresetCapabilities(provider: ModelProviderProfileV1): ModelProviderProfileV1 {
  ...
  const presetProfile = tokenPlanPreset?.tokenPlan
    ? modelProviderTokenPlanProfile(tokenPlanPreset, provider.baseUrl)
...
function modelProviderPresetProfileForProvider(provider: ModelProviderProfileV1): ModelProviderProfileV1 | null {
  ...
  return preset ? modelProviderPresetProfile(preset) : null
}
...
export function resolveAnalytixSpeechToTextSettings(settings: AppSettingsV1): AnalytixSpeechToTextSettingsV1 {
...
export function resolveAnalytixTextToSpeechSettings(settings: AppSettingsV1): AnalytixTextToSpeechSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
...
export function resolveAnalytixMusicGenerationSettings(settings: AppSettingsV1): AnalytixMusicGenerationSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
...
export function resolveAnalytixVideoGenerationSettings(settings: AppSettingsV1): AnalytixVideoGenerationSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
...
export function resolveAnalytixMemoryEnabled(settings: AppSettingsV1): boolean {
  const runtime = getAnalytixRuntimeSettings(settings)
...
export function resolveAnalytixImageGenerationSettings(settings: AppSettingsV1): AnalytixImageGenerationSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
...
export function resolveAnalytixVisionBridgeSettings(settings: AppSettingsV1): AnalytixVisionBridgeSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
  const bridge = runtime.visionBridge
  const requestedProviderId = normalizeModelProviderId(bridge.providerId)
  const providerId = requestedProviderId || getActiveModelProviderId(settings, runtime.providerId)
  if (!providerId) {
    return {
      ...bridge,
      providerId,
      endpointFormat: normalizeModelEndpointFormat(bridge.endpointFormat)
    }
  }
  const provider = getModelProviderProfile(settings, providerId)
  return {
    ...bridge,
    providerId: provider.id,
    baseUrl: bridge.baseUrl.trim() || provider.baseUrl.trim(),
    endpointFormat: provider.endpointFormat,
    model: bridge.model.trim() || firstVisionModel(provider) || provider.models[0] || bridge.model
  }
}

export function resolveAnalytixRuntimeSettings(settings: AppSettingsV1): AnalytixRuntimeSettingsV1 {
  const runtime = getAnalytixRuntimeSettings(settings)
  const activeProviderId = getActiveModelProviderId(settings, runtime.providerId)
  const runtimeBaseUrl = runtime.baseUrl?.trim() ?? ''
  const hasLegacyRuntimeCredentialOverride = !activeProviderId && (
    (Boolean(runtimeBaseUrl) && normalizeDeepseekBaseUrl(runtimeBaseUrl) !== DEFAULT_DEEPSEEK_BASE_URL)
  )
  const selection = resolveModelProviderRequestSelection(settings, {
    model: runtime.model,
    providerId: activeProviderId || undefined,
    runtimeProviderId: activeProviderId || undefined
  })
  const providerId = activeProviderId ||
    (!hasLegacyRuntimeCredentialOverride && selection.providerId !== DEFAULT_MODEL_PROVIDER_ID
      ? selection.providerId ?? ''
      : '')
  const provider = getModelProviderProfile(settings, providerId || DEFAULT_MODEL_PROVIDER_ID)
  const providerBaseUrl = provider.baseUrl.trim() || DEFAULT_DEEPSEEK_BASE_URL
  const model = selection.model || firstProviderTextChatModel(provider, runtime.model)
  const modelProfiles = applyModelCapabilityProbeProfiles(
    modelProviderModelProfilesForSettings(settings),
    runtime.modelCapabilityProbes,
    {
      providerId,
      model,
  ...
  return {
    ...
    model,
    baseUrl:
      !providerId && runtimeBaseUrl && runtimeBaseUrl !== DEFAULT_DEEPSEEK_BASE_URL
        ? normalizeDeepseekBaseUrl(runtimeBaseUrl)
        : normalizeDeepseekBaseUrl(providerBaseUrl),
    endpointFormat: provider.endpointFormat,
    imageGeneration: resolveAnalytixImageGenerationSettings(settings),
    speechToText: resolveAnalytixSpeechToTextSettings(settings),
    textToSpeech: resolveAnalytixTextToSpeechSettings(settings),
    musicGeneration: resolveAnalytixMusicGenerationSettings(settings),
    videoGeneration: resolveAnalytixVideoGenerationSettings(settings),
    visionBridge: resolveAnalytixVisionBridgeSettings(settings),
    modelProfiles,
    memoryEnabled: resolveAnalytixMemoryEnabled(settings)
  }
}

export function modelCapabilityProbeKey(input: {
  providerId: string
  model: string
  baseUrl: string
  ...
  return [
    ...
    normalizeModelKey(input.model),
    canonicalBaseUrl(input.baseUrl),
    normalizeModelEndpointFormat(input.endpointFormat)
  ].join('|')
}

function applyModelCapabilityProbeProfiles(
  profiles: Record<string, ModelProviderModelProfileV1>,
  probes: Record<string, ModelCapabilityProbeResultV1> | undefined,
  input: {
    providerId: string
    model: string
    baseUrl: string
  ...
  const key = modelCapabilityProbeKey(input)
  const probe = probes?.[key]
  if (!probe) return profiles
  const stale = Date.parse(probe.staleAfter) <= Date.now()
  const status = stale ? 'stale' : probe.status
  if (status === 'stale' || status === 'unknown') return profiles
  const normalizedModel = normalizeModelKey(input.model)
  const existing = profiles[normalizedModel]
  if (!existing) return profiles
  const next = { ...profiles }
  if (
    status === 'supported' &&
    probe.imageInput === 'supported' &&
    probe.toolCalling === 'supported' &&
    probe.toolResultImage === 'supported'
  ) {
    next[normalizedModel] = {
      ...existing,
      supportsToolCalling: true,
      inputModalities: uniqueModelProviderValues([...existing.inputModalities, 'image']),
      messageParts: uniqueModelProviderValues([...existing.messageParts, 'image_url'])
    }
    return next
  }
  if (status !== 'supported') {
    next[normalizedModel] = {
      ...existing,
      ...(probe.toolCalling === 'supported' ? {} : { supportsToolCalling: false }),
      inputModalities: existing.inputModalities.filter((item) => item !== 'image'),
      messageParts: existing.messageParts.filter((item) => item !== 'image_url' && item !== 'input_image')
    }
  }
  return next
}

function uniqueModelProviderValues<T extends string>(items: readonly T[]): T[] {
  return [...new Set(items)]
}

function firstVisionModel(provider: Pick<ModelProviderProfileV1, 'models' | 'modelProfiles'>): string {
  for (const model of provider.models) {
    const profile = modelProviderModelProfile(provider, model)
    if (profile?.inputModalities.includes('image')) return model
  }
  return ''
}

function defaultModelProviderProfile(baseUrl: string): ModelProviderProfileV1 {
  return {
...
function normalizeModelProviderProfile(
  ...
  const models = normalizeProviderModels(input?.models)
    .map((model) => normalizeDeprecatedProviderModelId(id, model))
    .filter((model, index, all) => all.findIndex((item) => normalizeModelKey(item) === normalizeModelKey(model)) === index)
  ...
  const price = normalizeModelProviderPricing(input?.price)
  const prices = normalizeModelProviderPrices(input?.prices, models)
...
function normalizeModelProviderPricing(
  input: unknown
): ModelProviderPricingV1 | undefined {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return undefined
  const record = input as Record<string, unknown>
  const cacheHit = boundedNonNegativeNumber(record.cacheHit)
  const inputPrice = boundedNonNegativeNumber(record.input)
  const outputPrice = boundedNonNegativeNumber(record.output)
  const currency = normalizePricingCurrency(record.currency)
  if (cacheHit === undefined && inputPrice === undefined && outputPrice === undefined) return undefined
  return {
...
function normalizeModelProviderPrices(
  input: unknown,
  models: readonly string[]
): Record<string, ModelProviderPricingV1> | undefined {
  if (!input || typeof input !== 'object' || Array.isArray(input)) return undefined
  const knownModelKeys = new Set(models.map(normalizeModelKey).filter(Boolean))
  const prices: Record<string, ModelProviderPricingV1> = {}
  for (const [rawModelId, rawPrice] of Object.entries(input as Record<string, unknown>)) {
    const modelId = normalizeModelKey(rawModelId)
    if (!modelId) continue
    if (knownModelKeys.size > 0 && !knownModelKeys.has(modelId)) continue
    const price = normalizeModelProviderPricing(rawPrice)
...
function normalizePricingCurrency(value: unknown): 'USD' | 'CNY' | undefined {
...
function deprecatedProviderAliasKey(providerId: string | undefined): string {
  const normalized = normalizeModelProviderId(providerId)
  return normalized.endsWith(TOKEN_PLAN_PROVIDER_ID_SUFFIX)
    ? normalized.slice(0, -TOKEN_PLAN_PROVIDER_ID_SUFFIX.length)
    : normalized
}

function normalizeDeprecatedProviderModelId(providerId: string | undefined, modelId: string): string {
  const model = typeof modelId === 'string' ? modelId.trim() : ''
  if (!model) return ''
  const aliases = DEPRECATED_PROVIDER_MODEL_ALIASES[deprecatedProviderAliasKey(providerId)]
  const canonical = aliases?.[normalizeModelKey(model)]

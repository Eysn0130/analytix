...
export const CLAW_MANAGED_INSTRUCTIONS_HEADING = '[Connect Phone managed instructions]'
export const CLAW_IM_AGENT_INSTRUCTIONS_HEADING = '[Connect Phone agent instructions]'
...
export function normalizeClawImPlatformAccount(
  input: unknown,
  legacyCredential: unknown,
  fallbackAccountId: string
): import('./app-settings-types').ClawImPlatformAccountV1 | undefined {
  const raw = typeof input === 'object' && input !== null && !Array.isArray(input)
    ? input as Record<string, unknown>
    : typeof legacyCredential === 'object' && legacyCredential !== null && !Array.isArray(legacyCredential)
      ? legacyCredential as Record<string, unknown>
      : {}
  const kind = raw.kind
  ...
  if (kind === 'weixin') {
    ...
    return accountId ? { kind, accountId, createdAt } : undefined
  }
  if (kind === 'telegram') {
    const accountId = typeof raw.accountId === 'string' && raw.accountId.trim()
      ? raw.accountId.trim()
    ...
    const botUsername = typeof raw.botUsername === 'string' ? raw.botUsername.trim().replace(/^@+/, '') : ''
    return {
      kind,
      accountId,
      allowedChatIds: typeof raw.allowedChatIds === 'string' ? raw.allowedChatIds.trim() : '',
      ...(botUsername ? { botUsername } : {}),
      createdAt
    }
  }
  if (kind !== 'feishu') return undefined
  const appId = typeof raw.appId === 'string' ? raw.appId.trim() : ''
  const accountId = typeof raw.accountId === 'string' && raw.accountId.trim() ? raw.accountId.trim() : appId
  if (!appId || !accountId) return undefined
  return {
    kind,
    accountId,
    appId,
    domain: typeof raw.domain === 'string' && raw.domain.trim() ? raw.domain.trim() : 'feishu',
    createdAt
  }
}
...
/**
 * Read the Analytix thread id from a legacy `agentThreadIds` record.
...
export function readLegacyAgentThreadId(input: unknown): string {
  ...
  const candidates = [
    typeof raw.analytix === 'string' ? raw.analytix.trim() : '',
...
export function buildClawRuntimePrompt(
  settings: Pick<AppSettingsV1, 'claw'> & Partial<Pick<AppSettingsV1, 'runtime' | 'provider'>>,
  prompt: string,
  ...
  if (skills.defaultNames.length > 0) {
    instructions.push(`Connect Phone skill policy: prefer these configured skills when relevant: ${skills.defaultNames.join(', ')}.`)
  }
...
export function unwrapClawRuntimePromptForDisplay(text: string): string {
  ...
  const looksManaged =
    ...
    prefix.includes('[Claw managed instructions]') ||
    prefix.includes('[Claw IM agent instructions]') ||
    prefix.includes('Claw skill policy:') ||
    prefix.includes('Connect Phone skill policy:') ||

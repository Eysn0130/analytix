export const GUI_PLAN_RELATIVE_DIR = '.analytixsdd/plan'
export const GUI_PLAN_LEGACY_RELATIVE_DIR = '.analytix/plan'
...
/**
 * Stable name of the native Analytix plan tool. Kept distinct from the
 * historical `gui_plan_create` MCP bridge so the renderer and Analytix
 ...
 * plan/refine turn so Analytix can scope tool availability to the
 * active plan context.
 */
 ...
 * the public surface the model sees; validation is enforced by Analytix
...
/**
 * Build the deterministic plan id used by both renderer and Analytix.
...
export function validateCreatePlanToolInput(input: Partial<CreatePlanToolInput>): string[] {
  ...
  if (input.plan_relative_path != null) {
    ...
    if (!path) {
      ...
      issues.push('plan_relative_path must be a direct Markdown file under .analytixsdd/plan')
    }
  }
 ...
 * plan path checks. Used by Analytix to verify the active workspace

...
export function normalizeWriteTypography(
  ...
  const textAlign =
    ...
    (WRITE_TEXT_ALIGNS as readonly string[]).includes(input.textAlign)
      ? input.textAlign
      : defaults.textAlign
  return { fontPreset, customFontFamily, fontSizePx, lineHeight, textAlign }
}
...
export function resolveWriteInlineCompletionEndpointFormat(
  settings: AppSettingsV1,
  modelId?: string | null
): ModelEndpointFormat {
  return resolveModelProviderEndpointFormat(
...
export function resolveWriteInlineCompletionProviderId(settings: AppSettingsV1): string {
  ...
  return getAnalytixRuntimeSettings(settings).providerId?.trim() || DEFAULT_MODEL_PROVIDER_ID
}
...
export function resolveWriteInlineCompletionModel(
  ...
  const runtimeModel = getAnalytixRuntimeSettings(settings).model?.trim() ?? ''

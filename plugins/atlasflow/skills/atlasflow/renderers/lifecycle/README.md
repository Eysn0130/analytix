...
Render `diagram_type: "lifecycle"` JSON files into the standard AtlasFlow HTML
template.

```bash
node atlasflow/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

Run `npm ci` in the skill folder first to install the locked dependencies.
The renderer validates input against `atlasflow/schemas/lifecycle.schema.json`
...
`atlasflow/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
atlasflow/schemas/lifecycle.schema.json
```

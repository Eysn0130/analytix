...
Render `diagram_type: "workflow"` JSON files into the standard AtlasFlow HTML
template.
...
```bash
node atlasflow/scripts/check-render-output.mjs output.html
```
...
`atlasflow/examples/agent-tool-call.workflow.json`.

The schema lives at:

```text
atlasflow/schemas/workflow.schema.json
```

...
// Shared pure renderer. CLI owns filesystem access; Canvas owns its strict adapter.
import { esc, renderDefinitions, textUnits } from '../shared/utils.mjs';
import { animateAttr, svgRootAttrs } from '../shared/svg-attributes.mjs';
...
export function renderArchitecturePlan(arch, { canvas = false } = {}) {
...
function buildLayoutReport() {
  if (canvas) return {
    ok: true, diagram_type: 'canvas', viewBox: arch.viewport,
    components: [...components.values()].map(c => ({ id: c.id, x: c.x, y: c.y, width: c.width, height: c.height })),
    connections: asArray(arch.connections).map(conn => ({ id: conn.id, from: conn.from, to: conn.to, points: pathFor(conn).points.map(point => [...point]) })),
  };
  const labels = [];
  for (const conn of asArray(arch.connections)) {
    if (!conn.label || !components.has(conn.from) || !components.has(conn.to)) continue;
...
function pathFor(conn) {
  ...
  // Canvas points already include their endpoints; never prepend new anchors.
  const points = canvas && conn.points.length ? conn.points : [start, ...routeVia(conn, from, to, start, end), end];
  const routed = { d: canvas ? polylinePath(points) : roundedPath(points, 8), points };
...
function renderConnectionPath(conn, index) {
  ...
  if (canvas) {
    ...
    return `        <path data-canvas-edge-id="${esc(conn.id)}" d="${routed.d}" fill="none" stroke="${style.stroke}" stroke-width="${style.strokeWidth}"${style.dash === 'dashed' ? ' stroke-dasharray="6 4"' : ''} marker-end="url(#canvas-arrow-${esc(conn.id)})"><title>${esc(conn.factLabel)}</title></path>`;
  }
...
function renderConnectionLabel(conn) {
  ...
  const [lx, ly] = labelPoint(conn, pathFor(conn).points);
  const w = Math.max(30, textUnits(conn.label) * 4.8 + 10);
  const markup = `        <rect x="${lx - w / 2}" y="${ly - 10}" width="${w}" height="14" rx="3" ${canvas ? `fill="${conn.style.fill}"` : 'class="c-mask"'}/>
        <text x="${lx}" y="${ly}" class="${variantAccent(conn.variant)}" font-size="8" text-anchor="middle">${esc(conn.label)}</text>`;
  return canvas ? `<g data-canvas-edge-id="${esc(conn.id)}">${markup}</g>` : markup;
}
...
function renderComponent(c) {
  ...
  const tag = c.tag
    ...
    : '';
  const dash = c.nodeDash === 'dash' || c.nodeDash === 'dashed' || c.frameStyle === 'dashed'
    ? ' stroke-dasharray="6 4"'
    : '';
  if (canvas) {
    const style = c.style;
    const paint = `fill="${style.fill}" stroke="${style.stroke}" stroke-width="${style.strokeWidth}"${style.dash === 'dashed' ? ' stroke-dasharray="6 4"' : ''}`;
    const shape = style.shape === 'ellipse'
      ? `<ellipse cx="${c.cx}" cy="${c.cy}" rx="${c.width / 2}" ry="${c.height / 2}" ${paint}/>`
      : `<rect x="${c.x}" y="${c.y}" width="${c.width}" height="${c.height}" rx="${style.shape === 'rounded' ? Math.min(6, c.width / 2, c.height / 2) : 0}" ${paint}/>`;
    return `        <g data-canvas-node-id="${esc(c.id)}"><title>${esc(c.factLabel)}</title>${shape}<text x="${cx}" y="${labelY}" class="t-primary" font-size="11" font-weight="600" text-anchor="middle">${esc(c.label)}</text></g>`;
  }
  return `        <rect x="${c.x}" y="${c.y}" width="${c.width}" height="${c.height}" rx="6" class="c-mask"/>
        <rect x="${c.x}" y="${c.y}" width="${c.width}" height="${c.height}" rx="6" class="${fill}"${animateAttr(arch.meta, 'node', componentSteps.get(c.id))} stroke-width="1.5"${dash}/>
...
function renderLegend() {
  ...
  for (const type of used) {
    ...
    parts.push(`        <text x="${x + 20}" y="${y}" class="t-muted" font-size="8">${esc(TYPE_LABELS[type] || type)}</text>`);
...
function renderSvg() {
  const root = canvas ? `xmlns="http://www.w3.org/2000/svg" viewBox="${arch.viewport.join(' ')}"` : `viewBox="0 0 ${viewBox[0]} ${viewBox[1]}"`;
  const markers = canvas ? `<defs>${arch.connections.map(conn => `<marker id="canvas-arrow-${esc(conn.id)}" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><polygon points="0 0, 10 3.5, 0 7" fill="${conn.style.stroke}"/></marker>`).join('')}</defs>` : '';
  return `      <svg ${root} ${svgRootAttrs(arch.meta, 'architecture diagram')}>
${renderDefinitions()}${markers}

        <!-- Background Grid -->
        ${canvas ? '<style>.t-primary{fill:#111827}.t-muted,.t-messagebus,.t-security,.t-frontend,.t-backend,.t-cloud,.t-database{fill:#475569}.c-mask{fill:#ffffff}text{font-family:ui-monospace,monospace}</style>' : '<rect width="100%" height="100%" fill="url(#grid)" />'}
...
if (!canvas) validateArchitecture();
return { svg: renderSvg(), layout: buildLayoutReport() };
}

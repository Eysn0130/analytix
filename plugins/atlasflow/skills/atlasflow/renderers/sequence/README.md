...
Render `diagram_type: "sequence"` JSON files into the standard AtlasFlow HTML
template.

```bash
node atlasflow/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

Run `npm install` once in the skill folder first — the renderer validates the
input against `atlasflow/schemas/sequence.schema.json` via ajv. Without it, the
...
`atlasflow/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
atlasflow/schemas/sequence.schema.json
```

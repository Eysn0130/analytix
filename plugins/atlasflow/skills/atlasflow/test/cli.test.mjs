...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'atlasflow-cli-'));
const cli = path.join(skillRoot, 'bin/atlasflow.mjs');
...
test('cli: inspect emits architecture layout json', () => {
  const input = path.join(skillRoot, 'examples/atlasflow-repo-grid.architecture.json');

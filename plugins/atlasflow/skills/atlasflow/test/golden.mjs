// Golden-file harness for the atlasflow renderers. No test framework needed:
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'atlasflow-test-'));
...
const webApp = fs.readFileSync(path.join(examplesRoot, 'web-app.html'), 'utf8');
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="atlasflow ${pkg.version}">`),

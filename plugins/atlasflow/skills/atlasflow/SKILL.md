...
When the user pastes Mermaid code, do NOT try to render or parse it mechanically — read it for structure and **lay out from scratch** in the matching atlasflow mode:

| Mermaid | AtlasFlow mode | Mapping |
|---------|--------------|---------|
...
3. Render: `node bin/atlasflow.mjs render <type> <input>.json <output>.html` (paths relative to this skill's folder).
4. Validate the generated artifact: `node bin/atlasflow.mjs validate <type> <input>.json --json`, or check an existing HTML file with `node bin/atlasflow.mjs check <output>.html`. This catches malformed SVG output, non-finite SVG values, two-point diagonal arrows, and arrows crossing the legend.
...
Render: `node bin/atlasflow.mjs render architecture <input>.json <output>.html`.
...
`pos` still wins when present (override one cell). This is **not** auto-layout — spacing is fixed cell math. Example: `examples/atlasflow-repo-grid.architecture.json`.

**Inspect layout (#9)** — after editing JSON, dump computed boxes without opening HTML:

```bash
node bin/atlasflow.mjs inspect architecture my.architecture.json
# or: node bin/atlasflow.mjs validate architecture my.architecture.json --layout-json
```

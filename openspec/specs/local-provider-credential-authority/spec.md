# local-provider-credential-authority Specification

## Purpose
Define the single local Provider Registry and protected Secret Store, credential lifecycle, migration, consumer authority and isolated acceptance.
...
- **THEN** creation and unlock use only admitted non-login paths under its unique owner-only root, preserve the actual OS user identity, and keep passwords off argv, environment, files, and evidence
- **AND** a bootstrap-to-final migration, when used, preserves one inode through exclusive link/unlink, retains only the exact basename-derived lock companions, and requires pinned identities and no open task Keychain handles

#### Scenario: The bound database identity is checked or changes
- **WHEN** a task unlocks, launches, idles, restarts, or reads back credentials
- **THEN** it fails closed on unowned identity drift; unlock success alone does not authorize an identity refresh
- **AND** an owned protected credential write may accept an inode transition only with unchanged path, device, owner, mode and link count and successful protected readback, while subsequent idle and restart checks retain the committed identity

#### Scenario: Cleanup or failure evidence is produced

# Case Evidence Kernel Specification

## Purpose
Define the additive per-effect case-evidence authority chain for the single
general Agent while freezing authority-topology expansion.
...
Provider analysis SHALL use stable, case-scoped, non-reversible entity
references plus host-verified semantic features through the existing host
evidence receipt, registry, ClaimRecord, and Final Evidence Gate contracts. A
display mask or sequential alias SHALL NOT serve as analytical identity, and an
...
perform a separate callback-scoped retained-binding resolution against the
accepted result's original witnessed immutable DSV2 snapshot; unavailable or
invalid retained material SHALL fail closed without current-snapshot
substitution.
...
or external trusted sink SHALL gate either standard local seam. The system
...
reports, and compaction prose SHALL remain source-exact-PII-free. The typed
...
The provider SHALL receive only the task-relevant stable entity references,

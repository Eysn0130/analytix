# Case Desktop Release Safety Specification

## Purpose
Define independently evidenced packaged general-Agent and longitudinal funds
milestones on one additive Agent, including desktop propagation, persistence,
controlled rendering, rollback, and truthful release evidence.
...
preview navigation. Direct Source Preview and AcceptedSlotDisplay SHALL resolve
source fields from host-owned bindings, never from a public tool result.
...
latest user constraints, stable case-scoped entity references,
...
stale approval, compaction/restart/fork/subagent recovery, stable entity
identity, snapshot evolution, Direct Source Preview and AcceptedSlotDisplay in
`full|masked`, provider/generic-channel PII zero-scans, allowlisted typed-sink
...
Fresh formal packaged evidence SHALL prove stable case-scoped identity, exact
DuckDB semantics, both typed local-display seams, forbidden generic-channel
...
- **THEN** the execution evidence carrier has already been consumed or
  discarded exactly once, Go resolves only those fields through one retained-
  binding callback against the original witnessed DSV2, and the typed local
  sink shows source-exact values while generic accepted-final/history/HTTP/
  SSE/events/stores remain PII-free
...
- **WHEN** account, card, identity-number, phone, and address sentinels traverse
  Direct Preview and Agent Result scenarios
- **THEN** provider/model/continuation, subagent, MCP/public tool result, generic
  ...
  same sentinel appears in the allowlisted typed local sink; an all-UI/all-DOM
...
- **WHEN** the same account/subject is analyzed over multiple turns and related
  queries
- **THEN** its case-scoped reference remains stable and exact amounts,
...
- **WHEN** `count_case_rows` succeeds without longitudinal flow analysis,
  snapshot evolution, both typed local-display seams, and lifecycle evidence
...
scan plus allowlisted-sink positive scan. Later benchmark rows SHALL remain
separate and SHALL NOT be reported as first-stage passes.
...
- **WHEN** release evidence shows a violation, a missing applicable
  ...
  applicable P0/P1 scenario; a later benchmark scenario enters this gate only
  when its independent P3 scope has explicitly begun and a benchmark result is
  being claimed

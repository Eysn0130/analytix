# Case Provider Structured Output Specification

## Purpose
Define provider-native typed proposal and safe longitudinal semantic context
mapping while keeping local grants, receipts, claims, and the Final Evidence
Gate authoritative.
...
contain stable case-scoped entity references, verified semantic features,
exact analytical values, coverage, and evidence references, but SHALL contain
no complete PII, raw row, database path, reverse mapping, or unrestricted tool
...
source-exact request to audit use, and replace each provider-visible span with
its stable case-scoped entity reference plus safe semantic context.
...
- **THEN** every provider family receives only the stable reference and
  authorized semantics, and the source-exact value is absent from request
  bytes, retry/repair inputs, cache observations, errors, and traces
...
`displayMode=full|masked`, Direct Source Preview, and source-exact
AcceptedSlotDisplay resolution SHALL remain outside every provider request,
response schema, retry/repair body, cache-visible body, continuation, and
durable provider history. The provider SHALL receive the same stable
case-scoped references, verified necessary semantics, exact minor-unit amounts,
...
task-relevant context containing stable entity references, verified semantic
...
tool results SHALL remain absent.

...
**Reason**: Development must prove the same protected local Provider onboarding and authority used by the product.
**Migration**: Use an isolated development profile and enter developer-owned credentials through normal onboarding or Provider Settings.

### Requirement: Packaged builds reject bootstrap
**Reason**: The Hub login gate and Hub bootstrap are superseded, and packages now require local Provider setup.
**Migration**: Continue rejecting all test bootstrap input while routing missing Provider state to protected local onboarding instead of Hub login.

### Requirement: Test credentials are isolated
**Reason**: Credential isolation now applies to the unified local Registry/Secret Store flow rather than a Hub test-account bootstrap.
**Migration**: Use isolated profiles, synthetic markers, normal protected entry, and redacted evidence for all development and package acceptance.

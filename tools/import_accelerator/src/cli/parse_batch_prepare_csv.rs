...
pub(crate) fn parse_batch_prepare_csv_args(
    iter: &mut impl Iterator<Item = String>,
) -> Result<Command> {
    let manifest = required_path_arg(iter, "--manifest")?;
    reject_extra(iter)?;
    Ok(Command::BatchPrepareCsv { manifest })
}
...
mod tests {
    ...
    fn parse(args: Vec<&'static str>) -> Result<Command> {
        ...
        parse_batch_prepare_csv_args(&mut iter)
    }
    ...
    #[test]
    fn parse_batch_prepare_csv_args_parses_manifest() {
        assert_eq!(
            parse(vec!["--manifest", "manifest.json"]).unwrap(),
            Command::BatchPrepareCsv {
    ...
    #[test]
    fn parse_batch_prepare_csv_args_reports_missing_manifest() {
        assert_eq!(error_message(vec![]), "missing --manifest");
    }

    #[test]
    fn parse_batch_prepare_csv_args_rejects_extra_arguments() {
        assert_eq!(

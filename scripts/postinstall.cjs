...
if (buildAnalytix.status !== 0) {
  process.exit(buildAnalytix.status || 1)
}

// Analytix is spawned with the Electron binary (ELECTRON_RUN_AS_NODE) and resolves
...
// loaded there and Analytix would silently fall back to JSONL scanning. Best
...
try {
  ...
  if (result.status !== 0) {
    console.warn('[postinstall] better-sqlite3 electron prebuild failed; Analytix will use the JSONL fallback')
  }
} catch (error) {

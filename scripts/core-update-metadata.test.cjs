...
test('Core builder metadata requires exact ZIP/DMG bytes and remains channel bound', async () => {
  ...
  try {
    for (const channel of ['stable', 'beta']) {
      ...
      fs.writeFileSync(join(root,name),yaml.dump({version:'1.0.7', files}))
      await hook({outDir:root,configuration:{extraMetadata:{releaseProfile:'core',releaseChannel:channel}}})

...
release_normalize_channel() {
  ...
  case "${raw}" in
    ...
    *) die "Release channel must be stable or beta, got: ${raw}" ;;
  esac
}

release_export_update_channel() {
  RELEASE_CHANNEL="$(release_normalize_channel "${RELEASE_CHANNEL:-stable}")"
  export RELEASE_CHANNEL
  export ANALYTIX_UPDATE_CHANNEL="${RELEASE_CHANNEL}"
...
release_load_local_env() {
  local env_file="${ANALYTIX_RELEASE_ENV:-}"
...
release_compute_version() {
  ...
  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    ...
    RELEASE_NAME="analytix ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_export_app_version() {
  ...
  export ANALYTIX_APP_VERSION="${RELEASE_VERSION}"
  cyan "  App:     ${ANALYTIX_APP_VERSION}"
}

release_ensure_tag_available() {
  if release_git rev-parse -q --verify "refs/tags/${TAG_NAME}" >/dev/null; then
    if [[ "${RELEASE_ALLOW_EXISTING_TAG:-}" == "1" ]]; then
      cyan "Using existing local tag ${TAG_NAME}."
      return
    fi
...
release_create_local_tag() {
  if release_git rev-parse -q --verify "refs/tags/${TAG_NAME}" >/dev/null; then
    local tag_commit
    tag_commit="$(release_git rev-list -n 1 "${TAG_NAME}")"
    local head_commit
    head_commit="$(release_git rev-parse HEAD)"
    if [[ "${tag_commit}" == "${head_commit}" ]]; then
      cyan "Local tag ${TAG_NAME} already points at HEAD."
      return
    fi
    die "Local tag ${TAG_NAME} exists but does not point at HEAD."
  fi

  release_git tag -a "${TAG_NAME}" -m "${RELEASE_NAME}" \
    || die "Failed to create local tag ${TAG_NAME}"
  cyan "Created local tag ${TAG_NAME}."
}

release_write_local_release_index() {
  local notes_file="$1"
  local assets_file="${ROOT}/dist/.release-assets.txt"
  shift
  ...
  cyan "Release notes: ${notes_file}"
  cyan "Asset list:    ${assets_file}"
}

release_maybe_publish_notice() {
  if [[ "${PUBLISH:-false}" == true ]]; then
    yellow "--publish is a no-op for local releases. Use --r2-promote to update R2 latest metadata."
    return
  fi
}
...
release_clean_dist_artifacts() {
  rm -rf "${ROOT}/dist/mac" "${ROOT}/dist/mac-arm64" "${ROOT}/dist/.mac-build" "${ROOT}/dist/win-unpacked" "${ROOT}/dist-standard-win/win-unpacked" "${ROOT}/dist/linux-unpacked"
  rm -f "${ROOT}"/dist/analytix-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
  rm -f "${ROOT}"/dist-standard-win/analytix-standard-* "${ROOT}"/dist-standard-win/latest*.yml "${ROOT}"/dist-standard-win/*.blockmap
}
...
release_write_notes_file() {
...

  {
    ...
    echo "- Release channel: \`${RELEASE_CHANNEL:-stable}\`"

  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        // Adapted from DeepSeek-Reasonix internal/serve/serve.go: open the
        // SSE stream immediately with a comment frame so proxies see bytes
        // before the first replay/live event. EventSource ignores comments.

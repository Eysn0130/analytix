...
import { applyCheckpointRewindPlan, createCheckpointRewindPlan } from './checkpoints.js'
...
import { killTaskJob, readTaskJobOutput, waitTaskJobs } from './task-jobs.js'
import {
  getThreadSummary,
  getThreadSummaryTaskOutput,
  killThreadSummaryTask,
  restartThreadSummaryTask
 ...
 * - `POST /v1/runtime/task-jobs/wait|output|kill` (auth)
 * - `GET /v1/skills` (auth)
 ...
 * - `GET /v1/threads/{id}/summary` and task output/kill/restart (auth)
 ...
 * - `POST /v1/threads/{id}/rewind` (auth)
 * - `POST /v1/threads/{id}/review` (auth)
 * - `POST /v1/threads/{id}/checkpoints/{checkpointId}/rewind-plan` (auth)
 * - `POST /v1/threads/{id}/checkpoints/{checkpointId}/rewind-apply` (auth)
...
export function buildRouter(runtime: ServerRuntime): Router {
  ...
  router.add('POST', '/v1/runtime/task-jobs/wait', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return waitTaskJobs(runtime.taskJobs, request)
  })
  router.add('POST', '/v1/runtime/task-jobs/output', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return readTaskJobOutput(runtime.taskJobs, request)
  })
  router.add('POST', '/v1/runtime/task-jobs/kill', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return killTaskJob(runtime.taskJobs, request)
  })
  ...
  router.add('GET', '/v1/case-projects', async (request) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return jsonResponse({ projects: [] })
  })
  router.add('GET', '/v1/case-projects/:id/threads', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return jsonResponse({ caseProjectId: ctx.params.id, threads: [] })
  })
  router.add('GET', '/v1/case-projects/:id/detail', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return jsonResponse({ caseProject: { id: ctx.params.id }, threads: [] })
  })
  ...
  router.add('GET', '/v1/threads/:id/summary', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return getThreadSummary(runtime, ctx.params.id)
  })
  router.add('GET', '/v1/threads/:id/summary/tasks/:taskId/output', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return getThreadSummaryTaskOutput(runtime, ctx.params.id, ctx.params.taskId, request)
  })
  router.add('POST', '/v1/threads/:id/summary/tasks/:taskId/kill', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return killThreadSummaryTask(runtime, ctx.params.id, ctx.params.taskId)
  })
  router.add('POST', '/v1/threads/:id/summary/tasks/:taskId/restart', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return restartThreadSummaryTask(runtime, ctx.params.id, ctx.params.taskId)
  })
  ...
  router.add('POST', '/v1/threads/:id/turns', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return startTurn(runtime.turnService, ctx.params.id, request, ({ threadId, turnId }, turnRequest) => {
      void runtime.threadTitleService?.generateForFirstUserPrompt({
        threadId,
        prompt: turnRequest.displayText?.trim() || turnRequest.prompt,
        ...(turnRequest.model?.trim() ? { model: turnRequest.model.trim() } : {})
      }).catch(() => undefined)
  ...
  router.add('POST', '/v1/threads/:id/checkpoints/:checkpointId/rewind-plan', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return createCheckpointRewindPlan(
      runtime.checkpointRewindService,
      ctx.params.id,
      ctx.params.checkpointId,
      request
    )
  })
  router.add('POST', '/v1/threads/:id/checkpoints/:checkpointId/rewind-apply', async (request, ctx) => {
    if (!authorize(request, runtime)) return ERRORS.unauthorized()
    return applyCheckpointRewindPlan(
      runtime.checkpointRewindService,
      ctx.params.id,
      ctx.params.checkpointId,
      request

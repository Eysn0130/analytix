...
import type { ForkThreadOptions, ListThreadsOptions, ThreadService } from '../../services-test-support/thread-service.js'
import type { RuntimeError } from './runtime-error.js'
import type { SessionStore } from '../../ports/session-store.js'
import type { Turn } from '../../contracts/turns.js'
import type { TurnItem } from '../../contracts/items.js'
import { finalizePublicToolResultItem } from '../../domain/item.js'
...
export async function setThreadGoal(
  ...
  try {
    ...
    if (error instanceof Error && /cannot mark goal complete|cannot mark research goal complete|cannot mark strict goal complete/i.test(error.message)) {
      return jsonResponse(
        { code: 'validation_error', message: error.message },
        400
      )
...
function validationError(_message: string, _issues: unknown): JsonResponse {

...
} from '../../services-test-support/usage-service.js'
import {
  emptyUsageSnapshot,
 ...
 */
export type UsageEndpointResponse = RuntimeUsageResponse & {
...
export async function buildUsageResponse(runtime: ServerRuntime): Promise<UsageEndpointResponse> {
  ...
  const records = await usageRecords(runtime)
  return {
    ...
    bySource: buildUsageBySourceResponse(records)
  }
}
...
async function usageRecords(
  ...
  if (typeof runtime.sessionStore.loadUsageRecords === 'function') {
    try {
      ...
      const fallbackThreadCache = new Map<
        string,
        Promise<ThreadRecord | (ThreadSummary & { turns: [] }) | null>
      >()
      const loadFallbackThread = (threadId: string) => {
        if (explicitThread?.id === threadId) return Promise.resolve(explicitThread)
        const cached = fallbackThreadCache.get(threadId)
        if (cached) return cached
        const summary = summariesById.get(threadId)
        const loaded = runtime.threadService.get(threadId)
          .then((thread) => thread ?? (summary ? { ...summary, turns: [] as [] } : null))
        fallbackThreadCache.set(threadId, loaded)
        return loaded
      }
      ...
      for (const record of indexed) {
        ...
        const needsFallback = !model || !providerId
        const thread = needsFallback ? await loadFallbackThread(record.threadId) : null
        const event = { turnId: record.turnId, model, providerId }
        records.push({
          threadId: record.threadId,
          model: model || (thread ? usageRecordModel(thread, event) : undefined),
          providerId: providerId || (thread ? usageRecordProvider(thread, event) : undefined),
      ...
      for (const threadId of liveThreadIds) {
        ...
        records.push({
          ...
          providerId: usageRecordProvider(thread),
...
function usageRecordProvider(
  thread: { providerId?: string },
  event?: Pick<UsageEvent, 'providerId'>
): string {
  return event?.providerId?.trim() || thread.providerId?.trim() || 'unknown'
}

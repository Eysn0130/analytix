...
export class UsageCounter {
  ...
  record(threadId: string, snapshot: UsageSnapshot): UsageSnapshot {
    ...
    const reasoningTokens = (current.reasoningTokens ?? 0) + (snapshot.reasoningTokens ?? 0)
    const totalTokens = promptTokens + completionTokens
    const currentHasTurns = current.turns > 0
    const hasCachedTokens = (currentHasTurns && current.cachedTokens !== undefined) ||
      snapshot.cachedTokens !== undefined
    const hasCacheTelemetry = (currentHasTurns && hasHitMissTelemetry(current)) ||
      hasHitMissTelemetry(snapshot)
    const cachedTokens = (currentHasTurns ? current.cachedTokens ?? 0 : 0) +
      (snapshot.cachedTokens ?? 0)
    const cacheHitTokens = (currentHasTurns ? current.cacheHitTokens ?? 0 : 0) +
      (snapshot.cacheHitTokens ?? 0)
    const cacheMissTokens = (currentHasTurns ? current.cacheMissTokens ?? 0 : 0) +
      (snapshot.cacheMissTokens ?? 0)
    const cacheTotal = cacheHitTokens + cacheMissTokens
    const cacheHitRate = hasCacheTelemetry && cacheTotal > 0 ? cacheHitTokens / cacheTotal : null
    const cacheableTokenHitRate = hasCacheTelemetry && cacheTotal > 0 ? cacheHitTokens / cacheTotal : undefined
    const totalInputTokenHitRate =
      hasCacheTelemetry && promptTokens > 0 && cacheTotal > 0 ? cacheHitTokens / promptTokens : undefined
    const cacheMissReasons = unionStrings(current.cacheMissReasons, snapshot.cacheMissReasons)
    const cacheSuggestions = unionStrings(current.cacheSuggestions, snapshot.cacheSuggestions)
    const turns = current.turns + (snapshot.turns > 0 ? snapshot.turns : 1)
    const priceConfigured = current.priceConfigured === true || snapshot.priceConfigured === true
    const costUsd =
    ...
    const next: UsageSnapshot = {
      ...
      ...(hasCacheTelemetry ? { cacheHitTokens, cacheMissTokens } : {}),
      cacheHitRate,
...
function normalizeUsageSnapshot(snapshot: UsageSnapshot): UsageSnapshot {
  ...
  return {
    ...
    ...(snapshot.cacheableTokenHitRate !== undefined ? { cacheableTokenHitRate: snapshot.cacheableTokenHitRate } : {}),
    ...(snapshot.totalInputTokenHitRate !== undefined ? { totalInputTokenHitRate: snapshot.totalInputTokenHitRate } : {}),
    ...(snapshot.cacheMissReasons?.length ? { cacheMissReasons: [...snapshot.cacheMissReasons] } : {}),
    ...(snapshot.cacheSuggestions?.length ? { cacheSuggestions: [...snapshot.cacheSuggestions] } : {}),
    turns: Math.max(0, Math.floor(snapshot.turns)),
    ...(snapshot.priceConfigured ? { priceConfigured: true } : {}),
...
function hasHitMissTelemetry(snapshot: UsageSnapshot): boolean {
  return snapshot.cacheHitTokens !== undefined || snapshot.cacheMissTokens !== undefined
}
...
function mergeUsage(into: UsageSnapshot, delta: UsageSnapshot): UsageSnapshot {
  ...
  const totalTokens = promptTokens + completionTokens
  const intoHasTurns = into.turns > 0
  const hasCachedTokens = (intoHasTurns && into.cachedTokens !== undefined) ||
    delta.cachedTokens !== undefined
  const hasCacheTelemetry = (intoHasTurns && hasHitMissTelemetry(into)) ||
    hasHitMissTelemetry(delta)
  const cachedTokens = (intoHasTurns ? into.cachedTokens ?? 0 : 0) + (delta.cachedTokens ?? 0)
  const cacheHitTokens =
    (intoHasTurns ? into.cacheHitTokens ?? 0 : 0) + (delta.cacheHitTokens ?? 0)
  const cacheMissTokens =
    (intoHasTurns ? into.cacheMissTokens ?? 0 : 0) + (delta.cacheMissTokens ?? 0)
  const cacheTotal = cacheHitTokens + cacheMissTokens
  const cacheHitRate = hasCacheTelemetry && cacheTotal > 0 ? cacheHitTokens / cacheTotal : null
  const cacheableTokenHitRate = hasCacheTelemetry && cacheTotal > 0 ? cacheHitTokens / cacheTotal : undefined
  const totalInputTokenHitRate =
  ...
  const priceConfigured = into.priceConfigured === true || delta.priceConfigured === true
  const costUsd =

...
import { resolvePlanModeToolSpecs } from '../loop-test-support/agent-loop.js'
...
describe('resolvePlanModeToolSpecs', () => {
  ...
  it('keeps plan approval separate from tool approval by excluding guarded tools during plan turns', () => {
    const result = resolvePlanModeToolSpecs([
      ...ALL_TOOLS,
      spec('request_deployment')
    ], {
      planTurnActive: true,
      createPlanSatisfied: false,
      stepIndex: 0
    ...
    expect(names).toContain('create_plan')
    expect(names).toContain('read')
    expect(names).not.toContain('request_deployment')
    expect(names).not.toContain('bash')
    expect(names).not.toContain('write')
  })

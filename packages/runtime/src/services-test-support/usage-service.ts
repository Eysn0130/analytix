...
import type { CacheDiagnostics } from '../cache/prefix-cache-diagnostics.js'
import type {
  DailyUsageBucket,
  DailyUsageCounters,
  DailyUsageResponse,
...
export class UsageService {
  ...
  record(
    ...
    const enriched = cacheDiagnostics ? enrichUsageWithCacheDiagnostics(usage, cacheDiagnostics) : usage
...
function enrichUsageWithCacheDiagnostics(
  usage: UsageSnapshot,
  diagnostics: CacheDiagnostics
): UsageSnapshot {
  const cacheHitTokens = typeof usage.cacheHitTokens === 'number'
    ? usage.cacheHitTokens
    : diagnostics.cacheHitTokens
  const cacheMissTokens = typeof usage.cacheMissTokens === 'number'
    ? usage.cacheMissTokens
    : diagnostics.cacheMissTokens
  const hasProviderMetrics =
    typeof cacheHitTokens === 'number' && typeof cacheMissTokens === 'number'
  const cacheableTokens = (cacheHitTokens ?? 0) + (cacheMissTokens ?? 0)
  const reasons = new Set(usage.cacheMissReasons ?? [])
  const suggestions = new Set(usage.cacheSuggestions ?? [])

  if (!diagnostics.cacheTelemetrySupported || !hasProviderMetrics) {
    reasons.add('provider_metrics_unavailable')
    suggestions.add('This provider did not report cache hit/miss tokens, so cache effectiveness cannot be verified.')
  }
  for (const reason of diagnostics.prefixChangeReasons ?? []) {
    switch (reason) {
      case 'model':
        reasons.add('model_changed')
        suggestions.add('Keep the model unchanged while warming a conversation cache.')
        break
      case 'provider':
        reasons.add('provider_changed')
        suggestions.add('Keep the provider unchanged while warming a conversation cache.')
        break
      case 'endpoint':
      case 'endpoint_format':
        reasons.add('endpoint_changed')
        suggestions.add('Keep the provider endpoint format unchanged while warming a conversation cache.')
        break
      case 'tools':
        reasons.add('tool_catalog_changed')
        suggestions.add('Keep MCP, Skill, and built-in tool schemas stable within a thread.')
        break
      case 'system':
      case 'mode':
      case 'prefix':
        reasons.add('stable_prefix_changed')
        suggestions.add('Keep timestamps, workspace snippets, and other volatile data out of the stable prefix.')
        break
      default:
        if (reason) reasons.add('stable_prefix_changed')
        break
    }
  }
  if (diagnostics.toolSourceChanged || (diagnostics.toolSourceChangeReasons ?? []).length > 0) {
    reasons.add('tool_catalog_changed')
    suggestions.add('Keep MCP, Skill, and built-in tool schemas stable within a thread.')
  }
  if (hasProviderMetrics && (cacheMissTokens ?? 0) > 0 && (cacheHitTokens ?? 0) === 0) {
    reasons.add('provider_cache_miss')
    reasons.add('cache_ttl_unknown')
    suggestions.add('Provider cache TTL may have expired or the provider declined to reuse the prefix.')
  }

  return {
    ...usage,
    ...(hasProviderMetrics && cacheableTokens > 0
      ? { cacheableTokenHitRate: (cacheHitTokens ?? 0) / cacheableTokens }
      : {}),
    ...(hasProviderMetrics && usage.promptTokens > 0
      ? { totalInputTokenHitRate: (cacheHitTokens ?? 0) / usage.promptTokens }
      : {}),
    ...(reasons.size > 0 ? { cacheMissReasons: [...reasons] } : {}),
    ...(suggestions.size > 0 ? { cacheSuggestions: [...suggestions] } : {})
  }
}
...
function addUsageCounters(
  ...
  target.price_configured = target.price_configured || usage.priceConfigured === true
...
function finalizeDailyBucket(bucket: DailyUsageAccumulator): DailyUsageBucket {
  ...
  return {
    ...
    price_configured: finalized.price_configured,
...
function finalizeModelBucket(bucket: ModelUsageAccumulator): ModelUsageBucket {
  ...
  return {
    model: bucket.model,
    provider: modelProviderValue(bucket),
    input_tokens: finalized.input_tokens,
    output_tokens: finalized.output_tokens,
    reasoning_tokens: finalized.reasoning_tokens,
    cached_tokens: finalized.cached_tokens,
...
function modelProviderValue(bucket: ModelUsageAccumulator): string {
  if (bucket.providerConflict) return 'mixed'
  return bucket.provider?.trim() || 'unknown'
}

function usageRecordSource(record: ThreadUsageRecord): string {
  const source = record.usageSource?.trim()
  return source || 'turn'
}

function sumUsageSnapshots(snapshots: readonly UsageSnapshot[]): UsageSnapshot {
  const counter = new UsageCounter()
  for (const snapshot of snapshots) {
    counter.record('__source__', snapshot)
  }
  return counter.forThread('__source__')
}

export function buildUsageBySourceResponse(records: readonly ThreadUsageRecord[]): UsageSourceBucket[] {
  const buckets = new Map<string, ThreadUsageRecord[]>()
  const childRunIds = new Map<string, Set<string>>()
  for (const record of records) {
    const source = usageRecordSource(record)
    const current = buckets.get(source) ?? []
    current.push(record)
    buckets.set(source, current)
    const childRunId = record.childRunId?.trim()
    if (childRunId) {
      const ids = childRunIds.get(source) ?? new Set<string>()
      ids.add(childRunId)
      childRunIds.set(source, ids)
    }
  }
  return [...buckets.keys()]
    .sort()
    .map((source) => ({
      source,
      usage: sumUsageSnapshots((buckets.get(source) ?? []).map((record) => record.usage)),
      childRunIds: [...(childRunIds.get(source) ?? new Set<string>())].sort()
    }))
}
...
export function buildThreadUsageResponse(records: readonly ThreadUsageRecord[]): ThreadUsageResponse {
  ...
  const totalsBase = finalized.reduce(
    (acc, bucket) => {
      ...
      acc.price_configured = acc.price_configured || bucket.price_configured
...
export function buildModelUsageResponse(
  ...
  for (const record of records) {
    ...
    if (provider) {
      if (!modelBucket.provider) {
        modelBucket.provider = provider
      } else if (modelBucket.provider !== provider) {
        modelBucket.providerConflict = true
      }
    }

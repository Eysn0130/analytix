...
export function createReadLocalTool(options: ReadLocalToolOptions = {}): LocalTool {
  ...
  return defineLocalTool({
    name: 'read',
    description:
      'Read a file. Relative paths resolve inside the active workspace; explicit external paths are allowed only when the turn runs with danger-full-access. Supports optional line offset and limit for large files.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string' },
    ...
    // Adapted from DeepSeek-Reasonix internal/tool/builtin/readfile.go SnipHint.
    ...
    execute: async (args, context) => withToolBoundary(async () => {
      ...
      const { absolutePath, relativePath } = await resolveReadablePath(rawPath, context)
      ...
      if (isBinaryBuffer(fileBuffer)) {
        return { output: { error: 'read only supports text files in Analytix serve mode', path: absolutePath }, isError: true }
      }

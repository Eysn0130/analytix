...
const TOOL_DESCRIPTION = [
  ...
  '.analytixsdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .analytixsdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.analytixsdd/plan/foo.md`)
 ...
 *    `.analytixsdd/plan/<feature>.md` under the active workspace.
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .analytixsdd/plan' }
  }
  if (input.operation === 'draft' && !isGuiPlanCurrentRelativePath(relativePath)) {
    return { error: 'legacy .analytix/plan paths can only be refined' }
  }
 ...
 * allocates a fresh, non-colliding `.analytixsdd/plan/<feature>.md`.
 */

...
import { resolveReadablePath, withToolBoundary } from './builtin-tool-utils.js'
import {
  acquireLspSession,
  lspCloseDocument,
  lspDefinition,
...
export function createLspLocalTool(): LocalTool {
  return defineLocalTool({
    ...
    policy: 'auto',
    toolKind: 'tool_call',
    // Adapted from DeepSeek-Reasonix internal/lsp/tool.go SnipHint.
    snipHint: { head: 60, tail: 10, headChars: 10_000, tailChars: 1_500 },
    execute: async (args, context) =>
      withToolBoundary(async () => {
        ...
        const { absolutePath, workspaceRoot } = await resolveReadablePath(rawPath, context)

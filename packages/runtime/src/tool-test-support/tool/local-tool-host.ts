...
import type { ModelToolSnipHint } from '../../ports/model-client.js'
...
import { defineLocalTool, type LocalToolDefinition } from './local-tool-definition.js'
import {
  runPostToolUseHooks,
  runPreToolUseHooks,
...
import { bashApprovalSafetySummary } from '../../shared/shell-safety.js'
...
export class LocalToolHost implements ToolHost {
  ...
  contractEntries(context?: ToolHostContext) {
    return this.registry.contractEntries(context)
  }
  ...
  async execute(
    ...
    const executionContext: ToolHostContext = {
      ...context,
      toolCallId: activeCall.callId
    }
    try {
      result = await tool.execute(activeCall.arguments, executionContext, async (update) => {
        if (!onUpdate) return
        const partialItem = makePrivateToolResultItem({
  ...
  private buildApprovalSummary(call: ToolCallLike): string {
    const args = Object.entries(call.arguments)
      ...
      .join(', ')
    const summary = `Run ${call.toolName}(${args})`
    if (call.toolName !== 'bash') return summary
    const safety = bashApprovalSafetySummary(call.arguments)
    return safety ? `${summary} ${safety}` : summary
  }
  ...
  private errorToolResult(
    ...
    return makePrivateToolResultItem({
  ...
  static defineTool(tool: LocalToolDefinition): LocalTool {
...
function createUserInputTool(name: string): LocalTool {
  ...
  return LocalToolHost.defineTool({
    ...
    execute: async (args, context) => {
      ...
      const validationError = validateUserInputQuestions(name, questions)
      if (validationError) {
        return {
          output: {
...
function validateUserInputQuestions(
  toolName: string,
  questions: Array<{
    id: string
    options: Array<{ label: string; description: string }>
  ...
  if (questions.length < 1) return `${toolName} requires at least one question.`
  if (questions.length > 3) return `${toolName} supports at most three questions.`
  for (const question of questions) {
    if (question.options.length === 0) continue
    if (question.options.length < 2) {
      return `${toolName} question "${question.id}" must provide at least two options when options are used.`
    }
    if (question.options.length > 3) {
      return `${toolName} question "${question.id}" supports at most three options.`
    }
    const seen = new Set<string>()
    for (const option of question.options) {
      const label = option.label.trim().toLowerCase()
      if (seen.has(label)) {
        return `${toolName} question "${question.id}" option labels must be unique.`
      }
      seen.add(label)
    }
  }

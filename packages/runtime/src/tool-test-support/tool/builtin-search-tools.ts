...
export function createLsLocalTool(options: LsLocalToolOptions = {}): LocalTool {
  ...
  return defineLocalTool({
    name: 'ls',
    description:
      'List directory contents. Relative paths resolve inside the active workspace; explicit external paths are allowed only with danger-full-access. Returns entries sorted alphabetically and marks directories.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string' },
    ...
    // Adapted from DeepSeek-Reasonix internal/tool/builtin/ls.go SnipHint.
    ...
    execute: async (args, context) => withToolBoundary(async () => {
      ...
      const { workspaceRoot: root, absolutePath, relativePath } = await resolveReadablePath(rawPath, context)
...
export function createFindLocalTool(options: FindLocalToolOptions = {}): LocalTool {
  return defineLocalTool({
    name: 'find',
    description:
      'Find files by glob pattern, similar to pi find. Relative paths resolve inside the active workspace; explicit external paths are allowed only with danger-full-access.',
    inputSchema: {
      type: 'object',
      properties: {
        pattern: { type: 'string' },
    ...
    // Adapted from DeepSeek-Reasonix internal/tool/builtin/glob.go SnipHint.
    snipHint: { head: 80, tail: 8, headChars: 10_000, tailChars: 1_000 },
    policy: 'auto',
    execute: async (args, context) => withToolBoundary(async () => {
      ...
      const { workspaceRoot: root, absolutePath, relativePath } = await resolveReadablePath(rawPath, context)
      const preferSearchRootOutput = isExternalSearchRoot(root, absolutePath)
      ...
      if (fd) {
        ...
        matches = candidates
          .map((path) => {
            const candidatePath = resolveToolOutputPath(root, path, absolutePath, preferSearchRootOutput)
            return {
              path: candidatePath,
              relative_path: searchRelativePath(root, absolutePath, candidatePath)
            }
          })
          .slice(0, limit)
      } else if (rg) {
        ...
        matches = paths
          .map((path) => ({ path, relative_path: searchRelativePath(root, absolutePath, path) }))
...
export function createGrepLocalTool(options: GrepLocalToolOptions = {}): LocalTool {
  return defineLocalTool({
    name: 'grep',
    description:
      'Search file contents for a pattern and return matching lines with paths and line numbers. Relative paths resolve inside the active workspace; explicit external paths are allowed only with danger-full-access.',
    inputSchema: {
      type: 'object',
      properties: {
        pattern: { type: 'string' },
    ...
    // Adapted from DeepSeek-Reasonix internal/tool/builtin/grep.go SnipHint.
    snipHint: { head: 80, tail: 8, headChars: 10_000, tailChars: 1_000 },
    policy: 'auto',
    execute: async (args, context) => withToolBoundary(async () => {
      ...
      if (rg) {
        ...
        for (const row of rows) {
          ...
          if (!parsed) continue
          const candidatePath = resolveToolOutputPath(root, parsed[1] ?? '', absolutePath, preferSearchRootOutput)
          const lineNumber = Number(parsed[2] ?? '0')
          const lineText = parsed[3] ?? ''
          const candidateRelative = searchRelativePath(root, absolutePath, candidatePath)
...
export function searchRelativePath(workspaceRoot: string, searchRoot: string, candidatePath: string): string {
  if (isExternalSearchRoot(workspaceRoot, searchRoot)) {
    return toolRelativePath(searchRoot, candidatePath)
  }
  return toolRelativePath(workspaceRoot, candidatePath)
}

function isExternalSearchRoot(workspaceRoot: string, searchRoot: string): boolean {
  return toolRelativePath(workspaceRoot, searchRoot) === normalizeToolPath(searchRoot)
}

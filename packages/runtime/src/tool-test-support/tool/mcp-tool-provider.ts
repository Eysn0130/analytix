...
export type McpServerDiagnostic = {
  ...
  knownOverride?: 'codegraph' | 'codebase-memory'
  effectiveCwd?: string
  lowPriority?: boolean
...
export type McpToolProviderBuildResult = {
   ...
   */
  startBackgroundReconnect: (register: (provider: CapabilityToolProvider) => boolean | void) => Promise<void>
  close: () => Promise<void>
}
...
export type McpToolProviderOptions = {
  ...
  schemaCache?: McpToolSchemaCache
  /**
...
export type McpToolSchemaCache = {
  load(serverId: string, serverFingerprint: string): Promise<McpToolDescriptor[] | undefined>
  save(serverId: string, serverFingerprint: string, tools: readonly McpToolDescriptor[]): Promise<void>
}

type McpToolSchemaCacheFile = {
  version: 1
...
export class FileMcpToolSchemaCache implements McpToolSchemaCache {
  constructor(private readonly filePath: string, private readonly nowIso: () => string = () => new Date().toISOString()) {}

  async load(serverId: string, serverFingerprint: string): Promise<McpToolDescriptor[] | undefined> {
    const file = await this.readFile()
    const entry = file.entries[mcpSchemaCacheEntryKey(serverId, serverFingerprint)]
    if (!entry || entry.serverFingerprint !== serverFingerprint) return undefined
    return entry.tools.map(normalizeMcpToolDescriptor)
  }

  async save(serverId: string, serverFingerprint: string, tools: readonly McpToolDescriptor[]): Promise<void> {
    const file = await this.readFile()
    file.entries[mcpSchemaCacheEntryKey(serverId, serverFingerprint)] = {
      serverId,
      serverFingerprint,
      updatedAt: this.nowIso(),
      tools: tools.map(cacheableMcpToolDescriptor)
    }
  ...
  private async readFile(): Promise<McpToolSchemaCacheFile> {
    try {
      const parsed = JSON.parse(await readFile(this.filePath, 'utf8')) as Partial<McpToolSchemaCacheFile>
      return {
        version: 1,
        entries: isRecord(parsed.entries) ? parsed.entries as McpToolSchemaCacheFile['entries'] : {}
      }
    } catch {
...
export async function buildMcpToolProviders(
  ...
  const outcomes = await Promise.all(
    Object.entries(mcp.servers).map(async ([serverId, configuredServer]): Promise<ConnectOutcome> => {
      const server = applyKnownMcpServerOverrides(serverId, configuredServer, options.workspaceRoot)
      if (!server.enabled) {
        return { serverId, server, status: 'disabled' }
      }
      const schemaCacheFingerprint = mcpServerSchemaCacheFingerprint(server)
      const attempt = (async () => {
        ...
        const state: McpConnectionState = {
          ...
          clientFactory,
          nowIso,
          ...(options.schemaCache ? { schemaCache: options.schemaCache } : {}),
          schemaCacheFingerprint,
      ...
      try {
        ...
        const cached = await options.schemaCache?.load(serverId, schemaCacheFingerprint).catch(() => undefined)
        if (cached?.length) {
          const state: McpConnectionState = {
            serverId,
            server,
            client: createDisconnectedMcpClient(serverId),
            clientFactory,
            nowIso,
            schemaCache: options.schemaCache,
            schemaCacheFingerprint,
            lastError: redactSecretText(errorMessage(error))
          }
          const nextFingerprint = catalogFingerprint(cached.map((tool) => tool.name))
          state.catalogFingerprint = nextFingerprint
          return { serverId, server, status: 'cached', state, listed: cached, error }
        }
  ...
  for (const outcome of outcomes) {
    ...
    diagnostics.push(serverDiagnostic(
      ...
      outcome.status === 'cached'
        ? `using cached MCP tool schema after startup error: ${errorMessage(outcome.error)}`
        : undefined
    ))
  }
...
type McpBackgroundReconnectParams = {
  ...
  register: (provider: CapabilityToolProvider) => boolean | void
  isAborted: () => boolean
  delay: (ms: number) => Promise<void>
...
async function reconnectFailedMcpServer(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      if (!registerLateMcpConnection(params, state, listed)) {
        await client.close().catch(() => undefined)
      }
      return
    } catch {
...
function registerLateMcpConnection(
  ...
  if (!params.searchActive) {
    try {
      const registered = params.register({
...
export function applyKnownMcpServerOverrides(
  serverId: string,
  server: McpServerConfig,
  workspaceRoot?: string
): McpServerConfig {
  const knownOverride = knownMcpServerOverride(serverId, server)
  if (!knownOverride) return server
  const effectiveWorkspaceRoot = workspaceRoot?.trim() || server.trustedWorkspaceRoots[0]?.trim()
  const env = { ...server.env }
  if (
    knownOverride === 'codegraph' &&
    env.CODEGRAPH_DAEMON_IDLE_TIMEOUT_MS === undefined
  ) {
    env.CODEGRAPH_DAEMON_IDLE_TIMEOUT_MS = '5000'
  }
  return {
    ...server,
    ...(server.transport === 'stdio' && !server.cwd && effectiveWorkspaceRoot
...
async function createSdkMcpClient(serverId: string, server: McpServerConfig): Promise<McpClientLike> {
  const client = new Client({ name: `analytix-${serverId}`, version: '0.1.0' })
...
function createMcpLocalTool(
  ...
  return LocalToolHost.defineTool({
    ...
    inputSchema: descriptor.inputSchema ?? defaultMcpInputSchema(),
    ...
    execute: async (args, context) => {
      ...
      const result = await callMcpToolWithReconnect(
        state,
        { name: descriptor.name, arguments: mcpArgumentsWithRuntimeContext(state, args, context) },
        context.abortSignal
      )
      ...
      return {
        ...
        isError: typeof redactedResult === 'object' &&
          redactedResult !== null &&
          (redactedResult as { isError?: boolean }).isError === true
      }
    }
  })
}
  ...
  return {
    ...
    client: {
      callTool: (input, options) =>
        callMcpToolWithReconnect(
          state,
          {
            ...input,
            arguments: mcpArgumentsWithRuntimeContext(state, input.arguments, options?.runtimeContext)
          },
          options?.signal,
          options?.timeout
        )
...
function mcpArgumentsWithRuntimeContext(
  state: McpConnectionState,
  args: Record<string, unknown>,
  context?: ToolHostContext
): Record<string, unknown> {
  if (state.serverId !== 'analytix_funds' || !context?.workspace?.trim()) return args
  const existing = isRecord(args._analytix) ? args._analytix : {}
  return {
    ...args,
    _analytix: {
      ...existing,
...
async function refreshMcpConnectionCatalog(state: McpConnectionState): Promise<McpToolDescriptor[]> {
  ...
  if (state.schemaCache && state.schemaCacheFingerprint) {
    await state.schemaCache.save(state.serverId, state.schemaCacheFingerprint, listed)
  }
  return listed
}

function normalizeMcpToolDescriptor(descriptor: McpToolDescriptor): McpToolDescriptor {
  const { inputSchema, outputSchema: rawOutputSchema, ...rest } = descriptor
  const outputSchema = normalizeMcpOutputSchema(rawOutputSchema)
  return {
    ...rest,
    inputSchema: normalizeMcpInputSchema(inputSchema),
    ...(outputSchema ? { outputSchema } : {})
  }
}

function normalizeMcpInputSchema(schema: unknown): Record<string, unknown> {
  if (!isRecord(schema)) return defaultMcpInputSchema()
  const normalized: Record<string, unknown> = { ...schema }
  if (typeof normalized.type === 'string' && normalized.type !== 'object') {
    return defaultMcpInputSchema()
  }
  normalized.type = 'object'
  if (normalized.properties !== undefined && !isRecord(normalized.properties)) {
    delete normalized.properties
  }
  if (Array.isArray(normalized.required)) {
    normalized.required = normalized.required.filter((value) => typeof value === 'string')
...
function normalizeMcpOutputSchema(schema: unknown): Record<string, unknown> | undefined {
  return isRecord(schema) ? schema : undefined
}

function defaultMcpInputSchema(): Record<string, unknown> {
...
async function callMcpToolWithReconnect(
  ...
  try {
    ...
    if (!looksLikeMcpTransportError(error)) throw new Error(state.lastError)
    try {
      const client = await reconnectMcpConnection(state)
      return await client.callTool(input, { signal, timeout })
    } catch (retryError) {
      state.lastError = redactSecretText(errorMessage(retryError))
...
function serverDiagnostic(
  ...
  const auth = diagnoseMcpAuth(state.server, lastError)
  return {
    id: state.serverId,
    enabled: state.server.enabled,
    transport: state.server.transport,
    trustScope: state.server.trustScope,
    ...knownMcpServerDiagnosticMetadata(state.serverId, state.server),
...
// Adapted from DeepSeek-Reasonix internal/mcpdiag/auth.go: surface only a
// redacted remote-MCP auth hint; configuration clearing remains a GUI concern.
function diagnoseMcpAuth(
  server: McpServerConfig,
  lastError: string | undefined
): Pick<McpServerDiagnostic, 'authStatus' | 'authUrl'> | undefined {
  if (!lastError || !isRemoteMcpTransport(server.transport)) return undefined
  if (!isMcpAuthFailure(lastError)) return undefined
  const authUrl = safeMcpAuthUrl(server.url)
  return {
    authStatus: 'required',
    ...(authUrl ? { authUrl } : {})
  }
...
function isMcpAuthFailure(value: string): boolean {
...
function safeMcpAuthUrl(rawUrl: string | undefined): string | undefined {
  const trimmed = rawUrl?.trim()
  if (!trimmed || !looksLikeHttpUrl(trimmed)) return undefined
  try {
...
function isRemoteMcpTransport(transport: McpServerConfig['transport']): boolean {
...
function isAuthQueryKey(value: string): boolean {
  const lower = value.toLowerCase()
...
function knownMcpServerDiagnosticMetadata(
  serverId: string,
  server: McpServerConfig
): Pick<McpServerDiagnostic, 'knownOverride' | 'effectiveCwd' | 'lowPriority' | 'backgroundStart'> {
  const knownOverride = knownMcpServerOverride(serverId, server)
  return {
    ...(knownOverride ? { knownOverride } : {}),
    ...(server.cwd ? { effectiveCwd: server.cwd } : {}),
    ...(server.lowPriority ? { lowPriority: true } : {}),
    ...(server.backgroundStart ? { backgroundStart: true } : {})
  }
}

function knownMcpServerOverride(
  serverId: string,
  server: Pick<McpServerConfig, 'command' | 'args'>
): 'codegraph' | 'codebase-memory' | undefined {
  const haystack = [serverId, server.command ?? '', ...server.args].join(' ').toLowerCase()
  if (haystack.includes('codebase-memory') || haystack.includes('codebase_memory')) {
    return 'codebase-memory'
  }
  if (haystack.includes('codegraph')) return 'codegraph'
  return undefined
}
...
function mcpSchemaCacheEntryKey(serverId: string, serverFingerprint: string): string {
  return `${serverId}:${serverFingerprint}`
}

function mcpServerSchemaCacheFingerprint(server: McpServerConfig): string {
  return createHash('sha256')
    .update(stableJson({
      ...
      lowPriority: server.lowPriority,
      backgroundStart: server.backgroundStart,
      headers: Object.keys(server.headers ?? {}).sort(),
...
function cacheableMcpToolDescriptor(descriptor: McpToolDescriptor): McpToolDescriptor {
  return normalizeMcpToolDescriptor({
    ...
    ...(descriptor.execution !== undefined ? { execution: descriptor.execution } : {}),
    ...(descriptor.icons !== undefined ? { icons: descriptor.icons } : {})
  })
}

function createDisconnectedMcpClient(serverId: string): McpClientLike {
  const disconnected = async (): Promise<never> => {
    throw new Error(`MCP server "${serverId}" is disconnected; reconnect required`)
  }
  return {
    listTools: disconnected,
    callTool: disconnected,
    close: async () => undefined
  }
}
...
function commonMcpCommandPathEntries(
  ...
  if (platform === 'darwin') {
    return [
      ...
      homePath(env, '.local/node/current/bin'),
...
function homePath(env: NodeJS.ProcessEnv, relativePath: string): string {
  ...
  return home ? posix.join(home, relativePath) : ''
}
...
function missingBareCommandHint(command: string): string {
  if (process.platform === 'win32') {
    return `Could not find "${command}" on PATH while starting the MCP server. Make sure Node/npm is installed and available to Analytix, or set the MCP command to an absolute path.`
  }
  if (process.platform === 'darwin') {
    return `Could not find "${command}" on PATH while starting the MCP server. If Analytix was launched from Finder or the desktop, make sure Node/npm is installed and available to GUI apps, or set the MCP command to an absolute path such as /opt/homebrew/bin/${command}.`
  }
  return `Could not find "${command}" on PATH while starting the MCP server. Make sure Node/npm is installed and available to Analytix, or set the MCP command to an absolute path such as /usr/local/bin/${command}.`
}

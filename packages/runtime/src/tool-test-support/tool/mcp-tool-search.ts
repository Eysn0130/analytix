...
export type McpSearchClientLike = {
  callTool(
    ...
    options?: { signal?: AbortSignal; timeout?: number; runtimeContext?: ToolHostContext }
  ): Promise<unknown>
}
    ...
    LocalToolHost.defineTool({
      ...
      execute: async (args, context) => {
        ...
        const result = await record.client.callTool(
          ...
          { signal: context.abortSignal, timeout: record.server.timeoutMs, runtimeContext: context }
        )
        return {
          output: {

...
export function workspaceRoot(workspace: string): string {
  ...
  if (!expanded.trim()) return process.cwd()
...
export async function resolveWorkspacePath(inputPath: string, context: ToolHostContext): Promise<{
  ...
  const expandedInputPath = expandHomePath(inputPath)
  const lexicalAbsolutePath = isAbsolute(expandedInputPath) ? resolve(expandedInputPath) : resolve(root, expandedInputPath)
  if (effectiveSandboxMode(context) === 'danger-full-access') {
    return {
      workspaceRoot: root,
      absolutePath: lexicalAbsolutePath,
      relativePath: toolRelativePath(root, lexicalAbsolutePath)
    }
  }
...
export async function resolveReadablePath(inputPath: string, context: ToolHostContext): Promise<{
  workspaceRoot: string
  absolutePath: string
  relativePath: string
}> {
  ...
  return resolveWorkspacePath(inputPath, context)
}
...
export function shellConfig(
  ...
  if (platform === 'win32') {
    ...
    const standardPwsh = firstExistingWindowsPowerShellCandidate('pwsh', fileExists, env)
    if (standardPwsh) {
      return {
        shell: standardPwsh,
        args: ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command']
    ...
    const standardPowerShell = firstExistingWindowsPowerShellCandidate('powershell', fileExists, env)
    if (standardPowerShell) {
      return {
        shell: standardPowerShell,
        args: ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command']
      }
    }
...
export function shellSupportsConditionalChaining(shell: string): boolean {
  return shellDisplayName(shell) !== 'powershell'
}

export function hasUnquotedShellConditionalSequence(command: string): boolean {
...
function shellSyntaxHint(name: string): string {
  switch (name) {
    ...
    case 'zsh':
      return 'POSIX shell'
    case 'pwsh':
      return 'PowerShell 7; conditional && and || chaining is supported'
    case 'powershell':
      return 'Windows PowerShell; use ; or if ($?) for conditional chaining'
    case 'cmd.exe':
...
function firstExistingWindowsPowerShellCandidate(
  shellName: 'pwsh' | 'powershell',
  fileExists: (path: string) => boolean,
  env: NodeJS.ProcessEnv = process.env
): string {
  return windowsPowerShellCandidatePaths(env)
    .filter((candidate) => shellDisplayName(candidate) === shellName)
    .find(fileExists) ?? ''
}

export function windowsPowerShellCandidatePaths(env: NodeJS.ProcessEnv = process.env): string[] {
  const candidates: string[] = []
  const programRoots = [env.ProgramFiles, env.ProgramW6432, env['ProgramFiles(x86)']]
    .filter((value): value is string => Boolean(value?.trim()))
  for (const root of programRoots) {
    candidates.push(joinWindowsPath(root, 'PowerShell', '7', 'pwsh.exe'))
  }
  const systemRoot = env.SystemRoot || env.windir
  if (systemRoot?.trim()) {
    candidates.push(joinWindowsPath(systemRoot, 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe'))
  }
...
export function makeListEntry(path: string, root: string, fileStat: FsStats): ListEntry {
  return {
    path,
    relative_path: toolRelativePath(root, path),
    name: basename(path),
...
export function toolRelativePath(root: string, path: string): string {
  const rel = relative(root, path) || '.'
  if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel)) return normalizeToolPath(path)
...
export function resolveToolOutputPath(
  ...
  if (fallbackRoot && resolve(fallbackRoot) !== resolve(root)) {
    const fallbackCandidate = resolve(fallbackRoot, trimmed)
    if (preferFallback && existsSync(fallbackCandidate)) return fallbackCandidate
    if (!existsSync(workspaceCandidate) && existsSync(fallbackCandidate)) return fallbackCandidate
  }

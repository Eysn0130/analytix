...
import type { BashLocalToolOptions, ShellConfig, TextSlice, TruncateMode } from './builtin-tool-types.js'
...
async function bashExecute(
  ...
  const shellRuntime = shellRuntimeInfo(shellConfig)
  assertShellCommandSupported(shellRuntime.shell, command)
...
async function startBashSession(
  ...
  const shellRuntime = shellRuntimeInfo(input.shellConfig)
  assertShellCommandSupported(shellRuntime.shell, input.command)
...
function assertShellCommandSupported(shell: string, command: string): void {
  if (shellSupportsConditionalChaining(shell)) return
  if (!hasUnquotedShellConditionalSequence(command)) return
  throw new Error(
    'Windows PowerShell does not support unquoted && or || conditional chaining; use ; or if ($?) instead, or run under PowerShell 7.'
  )
}

export function createBashLocalTool(options: BashLocalToolOptions = {}): LocalTool {
  const bashOps = options.operations
  const shellRuntime = shellRuntimeInfo(options.shellConfig)
  return defineLocalTool({
    ...
    // Adapted from DeepSeek-Reasonix internal/tool/builtin/bash.go SnipHint.
    snipHint: { head: 40, tail: 40, headChars: 8_000, tailChars: 8_000 },

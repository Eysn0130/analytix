...
export function createWriteLocalTool(_options: WriteLocalToolOptions = {}): LocalTool {
  ...
  return defineLocalTool({
    name: 'write',
    description:
      'Create or overwrite a file with the provided content. Relative paths resolve inside the active workspace; explicit external paths are allowed only with danger-full-access.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string' },
...
export function createEditLocalTool(_options: EditLocalToolOptions = {}): LocalTool {
  ...
  return defineLocalTool({
    name: 'edit',
    description:
      'Edit a file using exact text replacement. Relative paths resolve inside the active workspace; explicit external paths are allowed only with danger-full-access. Supports multiple disjoint edits in one call.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string' },

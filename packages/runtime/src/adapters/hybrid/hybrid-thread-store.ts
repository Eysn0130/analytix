...
type ThreadMetadataLine = {
  ...
  timestamp: string
  thread: ThreadRecord
  summary?: ThreadSidecarSummary
}

type ThreadSidecarSummary = {
...
const THREAD_SIDECAR_SUMMARY_SCHEMA_VERSION = 1
...
export class HybridThreadStore implements ThreadStore {
    ...
    try {
      this.cachedStatement(`
        INSERT INTO usage_events (
          thread_id, seq, timestamp, turn_id, model, provider_id, usage_source, child_run_id, usage_json
        )
        VALUES (
          @thread_id, @seq, @timestamp, @turn_id, @model, @provider_id, @usage_source, @child_run_id, @usage_json
        )
  ...
  private async initialize(): Promise<void> {
    ...
    if (process.env.ANALYTIX_RUNTIME_DISABLE_SQLITE === '1') {
      this.db = null
      return
    }
    try {
  ...
  private migrate(): void {
    ...
    addColumnIfMissing(this.db, 'threads', 'provider_id TEXT')
    addColumnIfMissing(this.db, 'usage_events', 'provider_id TEXT')
    addColumnIfMissing(this.db, 'usage_events', 'usage_source TEXT')
    addColumnIfMissing(this.db, 'usage_events', 'child_run_id TEXT')
    addColumnIfMissing(this.db, 'threads', 'usage_backfilled INTEGER NOT NULL DEFAULT 0')
    addColumnIfMissing(this.db, 'threads', 'turn_count INTEGER NOT NULL DEFAULT 0')
  }
  ...
  private async backfill(): Promise<void> {
    ...
    for (const threadId of await this.threadIdsFromFilesystem()) {
      ...
      if (usageBackfilled === undefined) {
        const record = await this.readIndexRecordFromFilesystem(threadId)
        if (!record) continue
  ...
  private upsertIndexBestEffort(record: ThreadIndexRecord): void {
    ...
    try {
      ...
      this.db
        .prepare(`
          INSERT INTO threads (
            id, title, workspace, model, provider_id, mode, status, approval_policy, sandbox_mode,
            ...
            updated_at_ms, preview, message_count, turn_count, event_seq_high_water, metadata_path,
  ...
  private async appendMetadata(thread: ThreadRecord): Promise<void> {
    ...
    const run = previous.catch(() => undefined).then(async () => {
      ...
      const line: ThreadMetadataLine = {
        ...
        summary: sidecarSummaryForThread(thread)
      }
  ...
  private async maybeCompactMetadata(threadId: string): Promise<void> {
    ...
    try {
      ...
      const line: ThreadMetadataLine = {
        ...
        summary: sidecarSummaryForThread(record)
      }
      ...
      console.warn('[analytix] event=ANALYTIX_HYBRID_METADATA_COMPACTION_SKIPPED')
    }
  }
  ...
  private async readIndexRecordFromFilesystem(threadId: string): Promise<ThreadIndexRecord | null> {
    const metadata = await this.readLatestMetadataEntry(threadId)
    if (metadata && hasAuthoritativeSidecarSummary(metadata.summary)) {
      return indexRecordFromSidecar(metadata.thread, metadata.summary)
    }

    const thread = await this.readThreadFromDisk(threadId)
    if (!thread) return null
    const record = this.indexRecordForThread(thread)
    if (!metadata || !hasAuthoritativeSidecarSummary(metadata.summary)) {
      await this.appendMetadata(thread).catch((error) => {
        warnSqlite(`backfill sidecar summary for ${threadId}`, error)
      })
    }
    return record
  }
  ...
  private async readLatestMetadata(threadId: string): Promise<ThreadRecord | null> {
    return (await this.readLatestMetadataEntry(threadId))?.thread ?? null
  }

  private async readLatestMetadataEntry(
    threadId: string
  ): Promise<{ thread: ThreadRecord; summary: ThreadSidecarSummary } | null> {
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
      ...
      if (parsed.success) {
        const thread = normalizeThreadMetadata(parsed.data, entries.slice(0, index + 1))
        return {
          thread,
          summary: normalizeSidecarSummary(entry.summary)
        }
      }
    }
    return null
  ...
  private async listFromFilesystem(): Promise<ThreadSummary[]> {
    ...
    for (const threadId of await this.threadIdsFromFilesystem()) {
      const record = await this.readIndexRecordFromFilesystem(threadId)
      if (record) summaries.push(summaryFromIndexRecord(record))
    }
...
function rowFromIndexRecord(
  ...
  return {
    ...
    model: thread.model,
    provider_id: thread.providerId ?? null,
    mode: thread.mode,
...
function summaryFromIndexRecord(record: ThreadIndexRecord): ThreadSummary {
  return {
    ...toThreadSummary(record.thread),
...
function indexRecordFromSidecar(thread: ThreadRecord, summary: ThreadSidecarSummary): ThreadIndexRecord {
  return {
    thread,
    messageCount: summary.messageCount ?? 0,
    turnCount: summary.turnCount ?? thread.turns.length,
    eventSeqHighWater: 0,
    preview: summary.preview ?? ''
  }
}

function sidecarSummaryForThread(thread: ThreadRecord): Required<ThreadSidecarSummary> {
  const items = thread.turns.flatMap((turn) => turn.items)
  return {
    schemaVersion: THREAD_SIDECAR_SUMMARY_SCHEMA_VERSION,
...
function normalizeSidecarSummary(raw: ThreadSidecarSummary | undefined): ThreadSidecarSummary {
  return {
    ...(typeof raw?.schemaVersion === 'number' && Number.isFinite(raw.schemaVersion)
      ? { schemaVersion: Math.max(0, Math.floor(raw.schemaVersion)) }
      : {}),
    ...(typeof raw?.preview === 'string' ? { preview: raw.preview.slice(0, 500) } : {}),
    ...(typeof raw?.messageCount === 'number' && Number.isFinite(raw.messageCount)
      ? { messageCount: Math.max(0, Math.floor(raw.messageCount)) }
      : {}),
    ...(typeof raw?.turnCount === 'number' && Number.isFinite(raw.turnCount)
      ? { turnCount: Math.max(0, Math.floor(raw.turnCount)) }
      : {})
  }
}

function hasAuthoritativeSidecarSummary(summary: ThreadSidecarSummary): boolean {
  return typeof summary.schemaVersion === 'number'
    && summary.schemaVersion >= THREAD_SIDECAR_SUMMARY_SCHEMA_VERSION
...
function usageRowFromEvent(event: RuntimeEvent & { kind: 'usage' }): UsageRow {
  return {
    ...
    child_run_id: event.childRunId ?? null,
...
function diffUsage(current: UsageSnapshot, previous: UsageSnapshot): UsageSnapshot {
  ...
  const cacheableTokenHitRate = cacheHitTokens !== undefined && cacheTotal > 0
    ? cacheHitTokens / cacheTotal
    : current.cacheableTokenHitRate
  const totalInputTokenHitRate = cacheHitTokens !== undefined && promptTokens > 0
    ? cacheHitTokens / promptTokens
    : current.totalInputTokenHitRate
  return {
    promptTokens,
    completionTokens,
    ...
    ...(current.cacheMissReasons?.length ? { cacheMissReasons: [...current.cacheMissReasons] } : {}),
    ...(current.cacheSuggestions?.length ? { cacheSuggestions: [...current.cacheSuggestions] } : {}),
...
function warnSqlite(_action: string, _error: unknown): void {
  console.warn('[analytix] event=ANALYTIX_HYBRID_SQLITE_FALLBACK')
}

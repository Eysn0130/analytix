 ...
 * keyboard input through optional native dependencies.
 *
 ...
 * logical-point space before driving the cursor.
 */
...
export type HostControlBackendId = 'analytix-computer-use' | 'nut-js'

export type HostControlTarget = {
  app?: string
  elementIndex?: number
  x?: number
  y?: number
...
export type HostControlDoctorResult = {
  backendId: string
...
export type HostAppState = {
  backendId: HostControlBackendId | string
  screenshot: HostScreenshot
  app?: unknown
  window?: unknown
...
export interface HostControlBackend {
  readonly id: HostControlBackendId | string
  readonly platform: NodeJS.Platform
  ensureReady(): Promise<HostControlAvailability>
  availability(): Promise<HostControlAvailability>
  doctor(): Promise<HostControlDoctorResult>
  listApps(): Promise<unknown[]>
  getAppState(app?: string): Promise<HostAppState>
  capture(app?: string): Promise<HostScreenshot>
  click(target?: HostControlTarget, button?: MouseButton, count?: 1 | 2, modifiers?: string[]): Promise<void>
  performSecondaryAction?(target: HostControlTarget, action: string): Promise<void>
  drag(start: HostControlTarget, end: HostControlTarget): Promise<void>
  scroll(target: HostControlTarget | undefined, direction: ScrollDirection, amount?: number): Promise<void>
  typeText(text: string, target?: HostControlTarget): Promise<void>
  pressKey(key: string, target?: HostControlTarget): Promise<void>
  setValue(value: string, target?: HostControlTarget): Promise<void>
...
export class HostController implements HostControlBackend {
  readonly id = 'nut-js'
  ...
  async doctor(): Promise<HostControlDoctorResult> {
    ...
    return {
      ...
      checks: [{
        id: 'nut-js-load',
        status: availability.available ? 'passed' : 'failed',
        message: availability.available
          ? 'nut-js screenshot and input modules are loaded.'
          : availability.reason ?? 'nut-js backend is unavailable.'
      }]
    }
  }
  ...
  async getAppState(): Promise<HostAppState> {
    return {
      backendId: this.id,
      screenshot: await this.capture(),
      accessibilityTree: [],
      elements: [],
      warnings: ['nut-js fallback only provides screenshots and coordinate input; semantic element actions are unavailable.']
    }
  }
  ...
  async performSecondaryAction(target: HostControlTarget, _action: string): Promise<void> {
    if (typeof target.x === 'number' && typeof target.y === 'number') {
      await this.click(target, 'right', 1)
      return
    }
    throw new Error('nut-js fallback cannot perform semantic secondary actions without coordinates')
  }

  async drag(start: HostControlTarget, endTarget: HostControlTarget): Promise<void> {
    if (typeof start.x !== 'number' || typeof start.y !== 'number' || typeof endTarget.x !== 'number' || typeof endTarget.y !== 'number') {
      throw new Error('nut-js drag requires start and end coordinates')
    }
    const nut = this.requireNut()
    await this.moveTo(start.x, start.y)
    await nut.sleep(100)
    const end = await this.toLogical(endTarget.x, endTarget.y)
    await nut.mouse.drag(nut.straightTo(new nut.Point(end.x, end.y)))
  }

  async scroll(
    target: HostControlTarget | undefined,
  ...
  async pressKey(keyStr: string): Promise<void> {
    await this.pressHotkey(keyStr)
  }

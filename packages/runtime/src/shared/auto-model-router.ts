...
export const AUTO_MODEL_ROUTER_CONTRACT_VERSION = 1
...
export type AutoModelRouterFingerprintInput = {
  routerModel?: string
  systemPrompt?: string
  timeoutMs?: number
...
export function buildAutoModelRouterFingerprint(input?: AutoModelRouterFingerprintInput): string {
  return createHash('sha256')
    .update(JSON.stringify({
      version: AUTO_MODEL_ROUTER_CONTRACT_VERSION,
      routerModel: input?.routerModel ?? AUTO_MODEL_ROUTER_MODEL,
      systemPrompt: input?.systemPrompt ?? AUTO_MODEL_ROUTER_SYSTEM_PROMPT,
      timeoutMs: input?.timeoutMs ?? AUTO_MODEL_ROUTER_TIMEOUT_MS,
...
export const AUTO_MODEL_ROUTER_FINGERPRINT = buildAutoModelRouterFingerprint()
...
function routerTextForItem(item: TurnItem): string {
  switch (item.kind) {
    ...
    case 'tool_result':
      return `[tool result: ${item.toolName}] status=${item.output.status} disclosure=${item.output.disclosure}`
    case 'compaction':
      return item.summary
    case 'approval':

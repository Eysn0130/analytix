...
} from '../shared/compaction-marker.js'
import {
...
} from '../shared/model-context-profile.js'
...
export class ContextCompactor {
  ...
  compact(input: {
    ...
    const requestedTailStart = keepRecent === 0 ? history.length : history.length - keepRecent
    const tailStart = keepRecent === 0 ? history.length : toolPairSafeTailStart(history, requestedTailStart)
    const head = history.slice(0, tailStart)
    const tail = keepRecent === 0 ? [] : history.slice(tailStart)
    if (head.length === 0) {
      return {
        next: [...frozen, ...history],
        summaryItem: makeCompactionItem({
...
export function toolPairSafeTailStart(history: TurnItem[], start: number): number {
  let nextStart = Math.max(0, Math.min(start, history.length))
  while (nextStart > 0) {
    const seenCalls = new Set<string>()
    const missingCalls = new Set<string>()
    for (const item of history.slice(nextStart)) {
      if (item.kind === 'tool_call') {
        seenCalls.add(item.callId)
      } else if (item.kind === 'tool_result' && !seenCalls.has(item.callId)) {
        missingCalls.add(item.callId)
      }
    }
    if (missingCalls.size === 0) return nextStart
    const callIndex = findLastToolCallIndex(history, nextStart - 1, missingCalls)
    if (callIndex < 0) return nextStart
    nextStart = callIndex
  }
  return nextStart
}
...
function findLastToolCallIndex(
  ...
  for (let index = fromIndex; index >= 0; index -= 1) {
    ...
    if (item.kind === 'tool_call' && callIds.has(item.callId)) return index
  }
  return -1
}
...
function trustworthyPromptTokens(
  ...
  if (estimate > 0 && reported > estimate * PROMPT_TOKEN_TRUST_FACTOR) {
    warnInflatedPromptTokens(model)
    return undefined
  }
  return reported
}

function warnInflatedPromptTokens(model?: string): void {
  const key = model || 'unknown'
  const now = Date.now()
  if (now - (inflationWarnedAt.get(key) ?? 0) < INFLATION_WARN_INTERVAL_MS) return
  inflationWarnedAt.set(key, now)
  console.warn('[analytix] event=ANALYTIX_CONTEXT_PROMPT_TOKENS_INFLATED')
}
...
function summarizeItem(item: TurnItem): string {
  switch (item.kind) {
    ...
    case 'tool_call':
      return `- Tool call ${item.toolName}: ${clipText(stringifyCompact(item.arguments))}`
    case 'tool_result':

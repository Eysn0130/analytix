...
const PLUGIN_MENTION_URI = /plugin:\/\/([^\s)\]]+)/giu
...
export class SkillRuntime {
  ...
  catalogInstruction(): string | undefined {
    ...
    const footer = '### How to use skills\n' +
      ...
      '(`$id`, `@id`, or `/skill:id`), mentions its owning plugin, or trips one ' +
      'of its triggers; its full instructions are then injected for that turn.\n' +
  ...
  private matchSkills(input: {
    ...
    const pluginMatches = pluginMentionActivations(this.skills, extractPluginMentionAliases(prompt))
    ...
    for (const skill of this.skills) {
      ...
      const plugin = pluginMatches.get(skill.id)
      if (plugin) {
        matches.push({ skill, skillId: skill.id, reason: plugin, score: 980 + skill.priority })
        continue
      }
...
async function loadSkillPackage(root: string, allowLegacy: boolean): Promise<LoadedSkill | null> {
  ...
  if (await exists(manifestPath)) {
    ...
    const entryPath = containedSkillPath(root, manifest.entry, 'Skill manifest entry must stay inside the skill package root.')
    ...
    return {
      ...
      assets: manifest.assets.map((asset) =>
        containedSkillPath(root, asset, 'Skill manifest asset must stay inside the skill package root.')
      ),
...
function containedSkillPath(root: string, value: string, message: string): string {
  ...
  if (candidateComparable === rootComparable || candidateComparable.startsWith(`${rootComparable}/`)) {
    return candidate
  }
...
function extractPluginMentionAliases(prompt: string): Set<string> {
  const aliases = new Set<string>()
  for (const match of prompt.matchAll(PLUGIN_MENTION_URI)) {
    for (const alias of pluginMentionAliasVariants(match[1] ?? '')) aliases.add(alias)
  }
  return aliases
}

function pluginMentionActivations(skills: LoadedSkill[], mentionedAliases: Set<string>): Map<string, string> {
  const activations = new Map<string, string>()
  if (mentionedAliases.size === 0) return activations
  const skillsByAlias = new Map<string, LoadedSkill[]>()
  for (const skill of skills) {
    const aliases = new Set([...pluginAliasesForSkill(skill), ...skillIdAliases(skill)])
    for (const alias of aliases) {
      const current = skillsByAlias.get(alias)
      if (current) current.push(skill)
      else skillsByAlias.set(alias, [skill])
    }
  }
  for (const alias of mentionedAliases) {
    const matched = skillsByAlias.get(alias) ?? []
    if (matched.length === 0) continue
    const selected = selectPluginMentionSkills(matched, alias)
    for (const skill of selected) {
      if (!activations.has(skill.id)) activations.set(skill.id, `plugin:${alias}`)
    }
  }
  return activations
}

function selectPluginMentionSkills(skills: LoadedSkill[], alias: string): LoadedSkill[] {
  const baseAlias = alias.split('@')[0] ?? alias
  const exact = skills.filter((skill) => skillIdAliases(skill).has(alias) || skillIdAliases(skill).has(baseAlias))
  if (exact.length > 0) return exact
  if (skills.length === 1) return skills
  const index = skills.filter((skill) => skill.id === 'index' || slug(skill.name) === 'index')
  if (index.length > 0) return index
  return skills
}

function skillIdAliases(skill: LoadedSkill): Set<string> {
  return new Set([skill.id, slug(skill.name)].filter(Boolean))
}

function pluginAliasesForSkill(skill: LoadedSkill): Set<string> {
  const aliases = new Set<string>()
  const parts = comparablePath(skill.root).split('/').filter(Boolean)
  const skillsIndex = parts.lastIndexOf('skills')
  if (skillsIndex < 0) return aliases
  const cacheIndex = parts.lastIndexOf('cache', skillsIndex)
  if (cacheIndex < 1 || parts[cacheIndex - 1] !== 'plugins') return aliases
  const marketplace = parts[cacheIndex + 1] ?? ''
  const pluginName = parts[cacheIndex + 2] ?? ''
  for (const alias of pluginMentionAliasVariants(pluginName)) aliases.add(alias)
  if (marketplace && pluginName) {
    for (const alias of pluginMentionAliasVariants(`${pluginName}@${marketplace}`)) aliases.add(alias)
  }
  return aliases
}

function pluginMentionAliasVariants(rawValue: string): string[] {
  const decoded = decodePluginMentionValue(rawValue)
    .replace(/[),.;，。；、]+$/u, '')
    .trim()
  if (!decoded) return []
  const full = normalizePluginAlias(decoded)
  const base = normalizePluginAlias(decoded.split('@')[0] ?? decoded)
  return [...new Set([full, base].filter(Boolean))]
}

function decodePluginMentionValue(value: string): string {
  try {
...
function normalizePluginAlias(value: string): string {
  return value.trim().normalize('NFKC').toLowerCase().replace(/\/+$/g, '')
}

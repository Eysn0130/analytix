...
describe('SkillRuntime', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'analytix-skills-'))
  })
  ...
  it('rejects skill.json entry and assets that escape the skill package root', async () => {
    const outside = join(root, 'outside.md')
    await writeFile(outside, 'outside', 'utf8')
    await mkdir(join(root, 'bad-entry'), { recursive: true })
    await writeFile(join(root, 'bad-entry', 'skill.json'), JSON.stringify({
      id: 'bad-entry',
      name: 'Bad Entry',
      entry: '../outside.md'
    }), 'utf8')
    await mkdir(join(root, 'bad-asset'), { recursive: true })
    await writeFile(join(root, 'bad-asset', 'skill.json'), JSON.stringify({
      id: 'bad-asset',
      name: 'Bad Asset',
      assets: ['../outside.md']
    }), 'utf8')
    await writeFile(join(root, 'bad-asset', 'SKILL.md'), 'bad asset body', 'utf8')
    await writeSkill('safe', {
      id: 'safe',
      name: 'Safe',
      assets: ['assets/reference.md']
    }, 'safe body')
    await mkdir(join(root, 'safe', 'assets'), { recursive: true })
    await writeFile(join(root, 'safe', 'assets', 'reference.md'), 'reference', 'utf8')

    const runtime = await createRuntime()
    const diagnostics = runtime.diagnostics()

    expect(diagnostics.skills.map((skill) => skill.id)).toEqual(['safe'])
    expect(diagnostics.validationErrors.map((error) => error.message)).toEqual(expect.arrayContaining([
      'Skill manifest entry must stay inside the skill package root.',
      'Skill manifest asset must stay inside the skill package root.'
    ]))
  })
  ...
  it('discovers a skill package symlinked into a root (e.g. cc switch)', async (ctx) => {
    ...
    const realDir = await mkdtemp(join(tmpdir(), 'analytix-skill-real-'))
    try {
  ...
  it('activates a plugin-owned skill from a composer plugin mention URI', async () => {
    const pluginSkillRoot = join(root, 'plugins', 'cache', 'analytix-hub', 'documents', '26.506.11943', 'skills')
    await writeSkillInRoot(pluginSkillRoot, 'documents', {
      name: 'documents',
      description: 'Create and edit document artifacts'
    }, 'Documents instructions')
    const runtime = await createRuntimeForRoots([pluginSkillRoot])

    const resolution = runtime.resolveTurn({
      prompt: '你可以用 @[Documents](plugin://documents) 写一篇日记吗？',
      workspace: root
    })

    expect(resolution.activeSkillIds).toEqual(['documents'])
    expect(resolution.activations[0]).toMatchObject({
      skillId: 'documents',
      reason: 'plugin:documents'
    })
    expect(resolution.instructions[0]).toContain('Documents instructions')
  })

  it('activates plugin-owned skills from encoded marketplace-qualified plugin URIs', async () => {
    const pluginSkillRoot = join(root, 'plugins', 'cache', 'analytix-hub', 'browser', '26.623.70822', 'skills')
    await writeSkillInRoot(pluginSkillRoot, 'control-in-app-browser', {
      name: 'control-in-app-browser'
    }, 'Browser instructions')
    const runtime = await createRuntimeForRoots([pluginSkillRoot])

    const resolution = runtime.resolveTurn({
      prompt: 'use @[Browser](plugin://browser%40analytix-hub) to inspect this',
      workspace: root
    })

    expect(resolution.activeSkillIds).toEqual(['control-in-app-browser'])
    expect(resolution.activations[0]).toMatchObject({
      skillId: 'control-in-app-browser',
      reason: 'plugin:browser@analytix-hub'
    })
  })

  it('activates a copied hub skill when the plugin URI matches the skill id', async () => {
    await writeSkill('analytix-fund-analysis', {
      name: 'Analytix Fund Analysis'
    }, 'Fund analysis instructions')
    const runtime = await createRuntime()

    const resolution = runtime.resolveTurn({
      prompt: '请使用 @[Analytix 涉案资金研判](plugin://analytix-fund-analysis) 做分析',
      workspace: root
    })

    expect(resolution.activeSkillIds).toEqual(['analytix-fund-analysis'])
    expect(resolution.activations[0]).toMatchObject({
      skillId: 'analytix-fund-analysis',
      reason: 'plugin:analytix-fund-analysis'
    })
  })
  ...
  async function createRuntime(options: Parameters<typeof SkillRuntime.create>[1] = {}) {
    return createRuntimeForRoots([root], options)
  }

  async function createRuntimeForRoots(
    roots: string[],
  ...
  async function writeSkillInRoot(
    skillRoot: string,
    folder: string,
    manifest: Record<string, unknown>,
    entry: string
  ): Promise<void> {
    const dir = join(skillRoot, folder)
    await mkdir(dir, { recursive: true })
    const entryName = typeof manifest.entry === 'string' ? manifest.entry : 'SKILL.md'
    await writeFile(join(dir, 'skill.json'), JSON.stringify(manifest), 'utf8')
    await writeFile(join(dir, entryName), entry, 'utf8')

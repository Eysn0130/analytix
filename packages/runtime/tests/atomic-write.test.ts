...
describe('atomicWriteFile', () => {
  it('creates and retightens owner-only files on POSIX', async () => {
    if (process.platform === 'win32') return
    const dir = await mkdtemp(join(tmpdir(), 'analytix-atomic-mode-'))
    const path = join(dir, 'secrets.json')
    try {
      await writeFile(path, '{"old":true}', { encoding: 'utf8', mode: 0o644 })
      await chmod(path, 0o644)

      await atomicWriteFile(path, '{"ok":true}', { mode: 0o600 })
  ...
  it('retries transient Windows rename lock failures', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'analytix-atomic-'))

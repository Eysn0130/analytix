...
import {
  ...
  type ImageGenClient
} from '../src/tool-test-support/tool/image-gen-tool-provider.js'
...
function attachmentStore(rootDir: string, overrides: Record<string, unknown> = {}) {
  return new FileAttachmentStore({
    rootDir,
    config: AnalytixCapabilitiesConfig.parse({ attachments: { enabled: true, ...overrides } }).attachments,
...
describe('Image gen tool provider', () => {
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'analytix-imagegen-'))
  })
  ...
  it('generates an image, saves it to the workspace, and scopes the attachment', async () => {
    ...
    expect(output.files[0].relativePath.startsWith('.analytix-images/')).toBe(true)

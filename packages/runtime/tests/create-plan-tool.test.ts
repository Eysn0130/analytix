...
} from '../src/tool-test-support/tool/create-plan-tool.js'
...
function buildGuiPlan(relativePath = '.analytixsdd/plan/login.md', operation: 'draft' | 'refine' = 'draft') {
  return {
    operation,
...
describe('create_plan tool: advertisement', () => {
  ...
  it('keeps the normal agent-mode default tool advertisement unchanged', async () => {
    ...
    expect(names).toEqual(expect.arrayContaining(['read', 'edit', 'write', 'grep', 'find', 'ls', 'echo']))
...
describe('create_plan tool: path validation', () => {
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# nested',
        operation: 'draft',
        plan_relative_path: '.analytixsdd/plan/nested/a.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.analytixsdd/plan/nested/a.md')
      })
    )
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# not md',
        operation: 'draft',
        plan_relative_path: '.analytixsdd/plan/login.txt'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.analytixsdd/plan/login.txt')
      })
    )
    expect(result.isError).toBe(true)
  })
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# mismatch',
        operation: 'refine',
        plan_relative_path: '.analytixsdd/plan/other.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: {
          operation: 'refine',
          workspaceRoot: '/tmp/ws',
          relativePath: '.analytixsdd/plan/login.md',
          planId: 'plan_login'
        }
      })
    )
  ...
  it('rejects an operation that differs from the active GUI plan operation', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.analytixsdd/plan/login.md', 'refine')
      })
    )
    expect(result.isError).toBe(true)
...
describe('create_plan tool: execution safety', () => {
  it('allows a free-form plan-mode call without a GUI plan context and self-allocates a path', async () => {
    ...
    expect((result.output as { relative_path: string }).relative_path).toBe(
      '.analytixsdd/plan/disk-cleanup.md'
    )
  })
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# legacy',
        operation: 'draft',
        plan_relative_path: '.analytix/plan/legacy.md'
      },
      buildContext({ threadMode: 'plan', workspace: '/tmp/ws' })
    )
    expect(result.isError).toBe(true)
    expect(JSON.stringify(result.output)).toMatch(/\.analytixsdd\/plan/)
  })
  ...
  it('still executes create_plan through the tool host during plan mode', async () => {
    ...
    expect(result.item.kind === 'tool_result'
      ...
      : ''
    ).toBe('.analytixsdd/plan/safe-plan.md')
  })
})
...
describe('create_plan tool: success and atomic write', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'analytix-plan-'))
    previousMarkdown = '# previous plan\n'
    await mkdir(join(workspace, '.analytix/plan'), { recursive: true })
    await writeFile(join(workspace, '.analytix/plan/login.md'), previousMarkdown, 'utf8')
    await mkdir(join(workspace, '.analytixsdd/plan'), { recursive: true })
    await writeFile(join(workspace, '.analytixsdd/plan/login.md'), previousMarkdown, 'utf8')
  })
  ...
  it('writes a fresh plan to the reserved path and returns structured metadata', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          ...
          relativePath: '.analytixsdd/plan/login.md',
          planId: `${workspace}:.analytixsdd/plan/login.md`,
    ...
    expect(output.relative_path).toBe('.analytixsdd/plan/login.md')
    expect(output.operation).toBe('draft')
    expect(output.summary).toContain('.analytixsdd/plan/login.md')
    expect(output.content_hash).toMatch(/^[a-f0-9]{16}$/)
    expect(output.byte_size).toBe(Buffer.byteLength('# Login plan\n\n- step 1', 'utf8'))
    expect(output.absolute_path).toBe(join(workspace, '.analytixsdd/plan/login.md'))
    ...
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          operation: 'draft',
          workspaceRoot: workspace,
          relativePath: '.analytix/plan/login.md',
          planId: `${workspace}:.analytix/plan/login.md`
        }
      })
    )
    expect(result.isError).toBe(true)
  ...
  it('overwrites an existing plan when the same reserved path is reused', async () => {
    ...
    expect(result.isError).toBeFalsy()
    const persisted = await readFile(join(workspace, '.analytix/plan/login.md'), 'utf8')
  ...
  it('self-allocates a non-colliding path when plan mode has no reserved context', async () => {
    ...
    // `.analytixsdd/plan/login.md` already exists in this workspace.
    expect(output.relative_path).toBe('.analytixsdd/plan/login-2.md')
    const persisted = await readFile(output.absolute_path, 'utf8')
  ...
  it('leaves the previous plan untouched when the abort signal fires before rename', async () => {
    ...
    expect(result.isError).toBe(true)
    const persisted = await readFile(join(workspace, '.analytixsdd/plan/login.md'), 'utf8')

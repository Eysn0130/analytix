...
} from '../src/tool-test-support/tool/builtin-tools.js'
import { computeEditDiff } from '../src/tool-test-support/tool/edit-diff.js'
import { withFileMutationQueue } from '../src/tool-test-support/tool/file-mutation-queue.js'
import { DEFAULT_MAX_BYTES } from '../src/tool-test-support/tool/truncate.js'
import type { InternalToolResultItem } from '../src/domain/item.js'
import type { FsStats } from '../src/tool-test-support/tool/builtin-tool-types.js'
import type { ToolHostContext } from '../src/ports/tool-host.js'
import { canWritePath } from '../src/tool-test-support/tool/sandbox-policy.js'
...
const FULL_ACCESS_CONTEXT = { sandboxMode: 'danger-full-access' as const }
...
async function executeTool(
  ...
  contextOverrides: Partial<ToolHostContext> = {}
) {
  const result = await host.execute(
    ...
    buildContext(workspace, contextOverrides)
  )
...
describe('Analytix built-in tools', () => {
  let workspace: string
  let host: LocalToolHost

  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'analytix-tools-'))
  ...
  it('advertises the pi-style built-in tool family by default', async () => {
    ...
    expect([...allBuiltinToolNames].filter((name) => name !== 'bash').every((name) => toolNames.has(name))).toBe(true)
  ...
  it('does not execute a tool after GUI approval is denied', async () => {
    let executed = false
    const guardedTool = LocalToolHost.defineTool({
      name: 'guarded_write',
      description: 'would mutate the workspace',
      inputSchema: { type: 'object', properties: {} },
      policy: 'on-request',
      execute: async () => {
        executed = true
    ...
    const guardedHost = new LocalToolHost({ tools: [guardedTool] })

    const result = await guardedHost.execute(
      { callId: 'call_guarded', toolName: 'guarded_write', arguments: {} },
      buildContext(workspace, { awaitApproval: async () => 'deny' })
    )
    ...
    expect(result.item).toMatchObject({
      ...
      approvalId: 'appr_call_guarded'
    })
  })

  it('enforces the ask/auto/yolo approval posture matrix at the permission gate', async () => {
    ...
    const guardedTool = LocalToolHost.defineTool({
      ...
      execute: async (_args, context) => {
        executions.push(`${context.approvalPolicy}:${context.sandboxMode ?? 'default'}`)
        return { output: { ok: true } }
      }
    })
    const guardedHost = new LocalToolHost({ tools: [guardedTool] })

    const ask = await guardedHost.execute(
      { callId: 'call_ask', toolName: 'guarded_write', arguments: {} },
      buildContext(workspace, {
        approvalPolicy: 'on-request',
        awaitApproval: async (approval) => {
          decisions.push(`ask:${approval.toolName}`)
          return 'allow'
        }
      })
    )
    const auto = await guardedHost.execute(
      { callId: 'call_auto', toolName: 'guarded_write', arguments: {} },
      buildContext(workspace, { approvalPolicy: 'auto', sandboxMode: 'workspace-write' })
    )
    const yolo = await guardedHost.execute(
      { callId: 'call_yolo', toolName: 'guarded_write', arguments: {} },
      buildContext(workspace, { approvalPolicy: 'auto', sandboxMode: 'danger-full-access' })
    )

    expect(decisions).toEqual(['ask:guarded_write'])
    expect(ask.item).toMatchObject({ kind: 'tool_result', isError: false })
    expect(auto.item).toMatchObject({ kind: 'tool_result', isError: false })
    expect(yolo.item).toMatchObject({ kind: 'tool_result', isError: false })
  ...
  it('rejects malformed structured GUI input choices before opening a user-input gate', async () => {
    let awaited = 0
    const cases = [
      {
        ...
        arguments: {
          questions: [
            ...
            { question: 'Fourth?', options: ['A', 'B'] }
          ]
        },
        message: 'supports at most three questions'
      },
      {
        name: 'single option',
        arguments: {
        ...
        message: 'must provide at least two options'
      },
      {
        ...
        message: 'supports at most three options'
      },
      {
        name: 'duplicate labels',
        arguments: {
          question: 'Pick one',
          options: ['Alpha', { label: ' alpha ', description: 'Duplicate' }]
        },
    ...
    for (const testCase of cases) {
      const result = await host.execute(
        {
          callId: `call_${testCase.name.replaceAll(' ', '_')}`,
          toolName: 'request_user_input',
          arguments: testCase.arguments
        },
        {
          ...buildContext(workspace),
          awaitUserInput: async () => {
            awaited += 1
      ...
      expect(result.item, testCase.name).toMatchObject({
        kind: 'tool_result',
        toolName: 'request_user_input',
        isError: true,
        output: {
      ...
      if (result.item.kind !== 'tool_result') throw new Error('request_user_input did not return a tool result')
      expect(JSON.stringify(result.item.output), testCase.name).toContain(testCase.message)
    }
  ...
  it('supports injected backend operations like pi tool factories', async () => {
    ...
    const bashOutput = await executeTool(
      customHost,
      workspace,
      'bash',
  ...
  it('writes, reads, edits, and searches workspace files', async () => {
    ...
    const editOutput = await executeTool(host, workspace, 'edit', {
      path: 'notes/demo.txt',
      edits: [
        { oldText: 'hello world', newText: 'hello analytix' },
    ...
    expect(editedDisk).toContain('hello analytix')
    expect(editedDisk).toContain('done')
    expect(String(editOutput.diff)).toContain('+2 hello analytix')
  ...
  it('guards Windows PowerShell chaining without blocking pwsh or quoted text', async () => {
    const powershellExec = vi.fn(async (
      _command: string,
      _cwd: string,
      options: { onData?: (data: Buffer) => void }
    ) => {
      options.onData?.(Buffer.from('powershell ok\n'))
      return { exitCode: 0, shell: 'powershell' }
    })
    const powershellHost = new LocalToolHost({
      tools: [createBashLocalTool({
        shellConfig: {
          shell: 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe',
          args: ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command']
        },
        operations: { exec: powershellExec }
      })]
    })

    const blocked = await powershellHost.execute(
      {
        callId: 'call_powershell_chain',
        toolName: 'bash',
        arguments: { command: 'Write-Output a && Write-Output b' }
      },
      buildContext(workspace, FULL_ACCESS_CONTEXT)
    )
    ...
    expect(blocked.item).toMatchObject({
      ...
      isError: true,
      output: {
        error: expect.stringContaining('Windows PowerShell')
      }
    })
    ...
    expect(powershellExec).toHaveBeenCalledTimes(1)

    const pwshExec = vi.fn(async (
      _command: string,
      _cwd: string,
      options: { onData?: (data: Buffer) => void }
    ) => {
      options.onData?.(Buffer.from('pwsh ok\n'))
      return { exitCode: 0, shell: 'pwsh' }
    })
    const pwshHost = new LocalToolHost({
      tools: [createBashLocalTool({
        shellConfig: {
          shell: 'C:\\Program Files\\PowerShell\\7\\pwsh.exe',
          args: ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command']
        },
        operations: { exec: pwshExec }
      })]
    })
    await executeTool(
      pwshHost,
      workspace,
      'bash',
      { command: 'Write-Output a && Write-Output b' },
      FULL_ACCESS_CONTEXT
    )
    expect(pwshExec).toHaveBeenCalledTimes(1)
  })
  ...
  it('finishes bash commands after the shell exits even when a background child keeps stdio open', async () => {
    ...
    const output = await executeTool(
      ...
      workspace,
      'bash',
      { command: 'sleep 5 & echo done', timeout: 2 },
      FULL_ACCESS_CONTEXT
    )
  ...
  it('returns a pollable bash session for foreground long-running commands', async () => {
    ...
    const output = await executeTool(
      ...
      workspace,
      'bash',
      { command: 'echo ready; sleep 5', yield_seconds: 1, timeout: 10 },
      FULL_ACCESS_CONTEXT
    )
    ...
    const stopped = await executeTool(
      ...
      workspace,
      'bash',
      { action: 'stop', session_id: String(output.session_id) },
      FULL_ACCESS_CONTEXT
    )
  ...
  it('polls completed bash sessions for final output', async () => {
    const output = await executeTool(
      ...
      workspace,
      'bash',
      { command: 'echo ready; sleep 2; echo done', yield_seconds: 1, timeout: 10 },
      FULL_ACCESS_CONTEXT
    )
    ...
    const polled = await executeTool(
      ...
      workspace,
      'bash',
      { action: 'poll', session_id: String(output.session_id) },
      FULL_ACCESS_CONTEXT
    )
  ...
  it('blocks the poll action for at least yield_seconds while the session keeps running', async () => {
    const output = await executeTool(
      ...
      workspace,
      'bash',
      { command: 'echo ready; sleep 5; echo done', yield_seconds: 1, timeout: 10 },
      FULL_ACCESS_CONTEXT
    )
    expect(output.status).toBe('running')
    ...
    const polled = await executeTool(
      ...
      workspace,
      'bash',
      { action: 'poll', session_id: String(output.session_id), yield_seconds: 2 },
      FULL_ACCESS_CONTEXT
    )
  ...
  it('returns from poll early once the session exits before yield_seconds', async () => {
    const output = await executeTool(
      ...
      workspace,
      'bash',
      { command: 'echo ready; sleep 1; echo done', yield_seconds: 1, timeout: 10 },
      FULL_ACCESS_CONTEXT
    )
    expect(output.status).toBe('running')
    ...
    const polled = await executeTool(
      ...
      workspace,
      'bash',
      { action: 'poll', session_id: String(output.session_id), yield_seconds: 10 },
      FULL_ACCESS_CONTEXT
    )
    const elapsed = Date.now() - startedAt
  ...
  it('includes the active shell in bash partial updates', async () => {
    const updates: InternalToolResultItem[] = []
    const result = await host.execute(
      {
  ...
  it('allows read paths outside the workspace root under danger-full-access', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'analytix-tools-outside-'))
    try {
      const outsideFile = join(outside, 'desktop.txt')
      await writeFile(outsideFile, 'external notes', 'utf8')
      const result = await host.execute(
        {
          callId: 'call_read_external',
          toolName: 'read',
          arguments: { path: outsideFile }
        },
        buildContext(workspace, { sandboxMode: 'danger-full-access' })
      )
      expect(result.item).toMatchObject({
        kind: 'tool_result',
        toolName: 'read',
  ...
  it('allows explicit external file writes under danger-full-access', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'analytix-tools-outside-'))
    try {
      const outsideFile = join(outside, 'write.txt')
      const result = await host.execute(
        {
          callId: 'call_write_external',
          toolName: 'write',
          arguments: { path: outsideFile, content: 'nope' }
        },
        buildContext(workspace, { approvalPolicy: 'auto', sandboxMode: 'danger-full-access' })
      )
      expect(result.item).toMatchObject({
      ...
      await expect(readFile(outsideFile, 'utf8')).resolves.toBe('nope')
    } finally {
      await rm(outside, { recursive: true, force: true })
    }
  })

  it('allows the TS fallback write permission helper under danger-full-access', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'analytix-tools-outside-'))
    try {
      const decision = canWritePath(join(outside, 'write.txt'), {
        workspace,
  ...
  it('rejects symlink escapes for workspace writes', async () => {
    const outside = await mkdtemp(join(tmpdir(), 'analytix-tools-outside-'))
    try {
      await symlink(outside, join(workspace, 'out'), 'dir')
      const result = await host.execute(
        {
          callId: 'call_write_symlink_escape',
          toolName: 'write',
          arguments: { path: 'out/evil.txt', content: 'escaped' }
        },
        buildContext(workspace, { approvalPolicy: 'auto', sandboxMode: 'workspace-write' })
      )
      expect(result.item).toMatchObject({

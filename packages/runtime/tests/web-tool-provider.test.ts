...
import { buildWebToolProviders } from '../src/tool-test-support/tool/web-tool-provider.js'
import {
  ...
  return new DeterministicWebProvider({
    ...
    searchResults: {
      'analytix web': [
        {
          url: 'https://docs.example.test/page',
          title: 'Analytix Web Docs',
          snippet: 'How Analytix web access works.'
        }
      ]
    }
  })
...
describe('Web tool provider', () => {
  ...
  it('searches through a configured provider with citations and telemetry', async () => {
    ...
    const result = await host.execute({
      callId: 'call_1',
      toolName: 'web_search',
      arguments: { query: 'analytix web', limit: 3 }
    }, buildContext())

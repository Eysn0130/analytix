...
} from '../src/shared/token-economy.js'
import {
...
describe('token economy', () => {
  ...
  it('keeps request history hygiene limits with the token economy config', () => {
    ...
    expect(normalized.historyHygiene.maxCumulativeToolResultTokens).toBe(0)
    expect(normalized.historyHygiene.keepRecentToolResults).toBe(0)
  })
})

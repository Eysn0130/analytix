...
import { healLoadedHistoryItems } from '../src/shared/history-healing.js'
import {
...
describe('model history repair', () => {
  it('keeps complete multi-tool blocks across assistant text bridges', () => {
    ...
    expect(repaired.map((item) => item.id)).toEqual([
      'missing_call',
      'tool_result_repaired_call_missing',
      'user_1',
      'call_a',
      'call_b',
      'assistant_bridge',
  ...
  it('backfills incomplete tool calls with interrupted placeholder results', () => {
    const repaired = repairModelHistoryItems([
      makeToolCallItem({
        id: 'call_missing',
        threadId: 'thr_1',

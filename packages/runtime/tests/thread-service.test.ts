...
import { DEFAULT_ANALYTIX_MODEL } from '../src/config/analytix-config.js'
...
describe('ThreadService.fork with side relation', () => {
  it('persists providerId on create, update, fork, and resume', async () => {
    const { service } = buildService()
    const created = await service.create(
      { workspace: '/tmp/p', model: 'glm-5', providerId: 'zai-coding-plan', mode: 'agent' },
      { id: 'thr_provider', title: 'Provider thread' }
    )
    expect(created.providerId).toBe('zai-coding-plan')

    const updated = await service.update('thr_provider', { providerId: 'zhipu-coding-plan' })
    expect(updated.providerId).toBe('zhipu-coding-plan')

    const fork = await service.fork('thr_provider')
    expect(fork.providerId).toBe('zhipu-coding-plan')

    const resumed = await service.resumeSession('thr_provider')
    expect(resumed.thread.providerId).toBe('zhipu-coding-plan')

    const cleared = await service.update('thr_provider', { providerId: null })
    expect(cleared.providerId).toBeUndefined()
  })
  ...
  it('truncates forks at a requested turn and rewrites cloned ownership', async () => {
    const { service, threadStore, sessionStore, nowIso } = buildService()
    await seedParentWithTurns(service, threadStore, sessionStore, nowIso, {
      parentId: 'thr_cut',
      inflight: true
    })
    const source = await threadStore.get('thr_cut')
    if (!source) throw new Error('source thread was not created')
    await threadStore.upsert({ ...source, mode: 'plan' })
    await service.setGoal('thr_cut', { objective: 'do not inherit', status: 'active' })
    await service.setTodos('thr_cut', {
      todos: [{ content: 'later todo', status: 'pending' }]
    })

    const fork = await service.fork('thr_cut', { turnId: 'turn_completed' })
    ...
    expect(fork.todos).toBeUndefined()
    expect(fork.goal).toBeUndefined()
    expect(fork.turns[0]?.threadId).toBe(fork.id)
    expect(fork.turns[0]?.items.every((item) => item.threadId === fork.id)).toBe(true)
    expect(fork.turns.flatMap((turn) => turn.items).some((item) =>
      item.kind === 'user_message' && item.text === 'mid-flight ask'
    )).toBe(false)
    await expect(service.fork('thr_cut', { turnId: 'turn_missing' }))
      .rejects.toThrow('turn not found: turn_missing')
  })
  ...
  it('repairs malformed tool-call history when cloning a fork', async () => {
    ...
    expect(forkItems.some((item) => item.kind === 'tool_call' && item.callId === 'call_missing')).toBe(true)
    expect(forkItems.find((item) => item.kind === 'tool_result' && item.callId === 'call_missing')).toMatchObject({
      kind: 'tool_result',
      callId: 'call_missing',
      output: {
...
describe('ThreadService goals', () => {
  ...
  it('rejects completing a goal without service-level evidence', async () => {
    const { service } = buildService()
    await service.create(
      { workspace: '/tmp/p', model: 'deepseek-chat', mode: 'agent' },
      { id: 'thr_goal_gate', title: 'Goal thread' }
    )
    await service.setGoal('thr_goal_gate', {
      objective: 'ship goal mode',
      status: 'active'
    })

    await expect(service.setGoal('thr_goal_gate', { status: 'complete' }))
      .rejects.toThrow(/without evidence/)
    expect((await service.getGoal('thr_goal_gate'))?.status).toBe('active')

    await service.appendGoalEvidence('thr_goal_gate', {
      step: 'Verified service guard',
      evidence: ['packages/runtime/tests/thread-service.test.ts covers service-level completion gating']
    })
    await service.setTodos('thr_goal_gate', {
      todos: [
        { content: 'Write contract test', status: 'completed' },
        { content: 'Run contract test', status: 'pending' }
      ]
    })
    await expect(service.setGoal('thr_goal_gate', { status: 'complete' }))
      .rejects.toThrow(/incomplete todos/)
    expect((await service.getGoal('thr_goal_gate'))?.status).toBe('active')

    await service.setTodos('thr_goal_gate', {
      todos: [
        { content: 'Write contract test', status: 'completed' },
        { content: 'Run contract test', status: 'completed' }
      ]
    })
    await expect(service.setGoal('thr_goal_gate', { status: 'complete' }))
  ...
  it('requires strict completion self-check before completing strict goals', async () => {
    const { service } = buildService()
    await service.create(
      { workspace: '/tmp/p', model: 'deepseek-chat', mode: 'agent' },
      { id: 'thr_goal_strict', title: 'Strict goal thread' }
    )
    await service.setGoal('thr_goal_strict', {
      objective: 'ship strict goal mode',
      status: 'active',
      strictCompletion: true
    })
    await service.appendGoalEvidence('thr_goal_strict', {
      step: 'Implemented strict goal mode',
      evidence: ['implementation was reviewed']
    })

    await expect(service.setGoal('thr_goal_strict', { status: 'complete' }))
      .rejects.toThrow(/self-check/)
    expect((await service.getGoal('thr_goal_strict'))?.status).toBe('active')

    await service.setGoal('thr_goal_strict', {
      selfCheckRequired: true,
      selfCheckCompleted: true,
      selfCheckTurnId: 'turn_self_check'
    })
  ...
  it('logs goal persistence failures before surfacing them', async () => {
    const { service, threadStore } = buildService()
    const threadId = 'thr_goal_persist__/private/thread-pii-13900000000'
    const errorSentinel = 'disk full: /private/error-pii-13900000001'
    await service.create(
      { workspace: '/tmp/p', model: 'deepseek-chat', mode: 'agent' },
      { id: threadId, title: 'Goal thread' }
    )
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined)
    const error = new Error(errorSentinel)
    const originalUpsert = threadStore.upsert.bind(threadStore)
    vi.spyOn(threadStore, 'upsert').mockImplementation(async (thread) => {
      if (thread.id === threadId && thread.goal) throw error
      return originalUpsert(thread)
    })

    try {
      await expect(service.setGoal(threadId, {
        objective: 'ship goal mode',
        status: 'active'
      })).rejects.toBe(error)
      expect(warnSpy).toHaveBeenCalledTimes(1)
      expect(warnSpy).toHaveBeenCalledWith('[analytix] event=ANALYTIX_GOAL_PERSISTENCE_FAILED')
      const warning = warnSpy.mock.calls.flat().map(String).join(' ')
      expect(warning).not.toContain(threadId)
      expect(warning).not.toContain(errorSentinel)
      expect(warning).not.toContain('during set')
    } finally {
      warnSpy.mockRestore()
    }
  })
...
describe('ThreadService todos', () => {
  it('syncs plan checklists, patches linked checkboxes, and preserves removed tasks', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'analytix-todos-'))
    try {
      const relativePath = '.analytixsdd/plan/demo.md'
      const absolutePath = join(workspace, relativePath)
      await mkdir(join(workspace, '.analytixsdd', 'plan'), { recursive: true })
...
describe('ThreadService.resumeSession', () => {
  it('uses the Analytix default model when resuming item-only legacy sessions', async () => {
    const { service, sessionStore } = buildService()
    ...
    expect(result.thread.model).toBe(DEFAULT_ANALYTIX_MODEL)
  })
})

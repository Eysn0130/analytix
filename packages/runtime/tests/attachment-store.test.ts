...
import type { LocalTool } from '../src/tool-test-support/tool/local-tool-host.js'
...
describe('Attachment store and multimodal input', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'analytix-attachments-'))
  })
  ...
  it('stores PDF documents with extracted text metadata', async () => {
    const store = createStore()
    ...
    await expect(store.resolveContent(attachment.id, { threadId: 'thr_pdf' })).resolves.toMatchObject({
      id: attachment.id,
      data
    })
  })
  ...
  it('serves authenticated upload, metadata, content, and diagnostics routes', async () => {
    ...
    const metadata = await dispatchRequest(
      h.router,
      new Request(`http://localhost/v1/attachments/${uploaded.attachment.id}?thread_id=thr_1&workspace=${encodeURIComponent('/tmp/workspace')}`, {
        headers: { authorization: 'Bearer tok-1' }
      })
    )
    expect(metadata.status).toBe(200)
    const publicMetadata = await readJson(metadata) as { attachment?: Record<string, unknown> }
    expect(publicMetadata.attachment).toMatchObject({ scope: 'thread', mimeType: 'image/png' })
    expect(publicMetadata.attachment).not.toHaveProperty('localFilePath')
    expect(publicMetadata.attachment).not.toHaveProperty('textFallback')
    expect(publicMetadata.attachment).not.toHaveProperty('hash')
    const content = await dispatchRequest(
      h.router,
      new Request(`http://localhost/v1/attachments/${uploaded.attachment.id}/content?thread_id=thr_1&workspace=${encodeURIComponent('/tmp/workspace')}`, {
        headers: { authorization: 'Bearer tok-1' }
      })
    )

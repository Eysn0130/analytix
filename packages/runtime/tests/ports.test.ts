...
describe('InMemoryApprovalGate', () => {
  ...
  it('returns false when deciding an already resolved approval', async () => {
    const gate = new InMemoryApprovalGate()
    const approval = createApprovalRequest({
      id: 'a',
      threadId: 't',
    ...
    const pending = gate.request(approval)
    expect(gate.decide('a', 'deny')).toBe(true)
    await expect(pending).resolves.toBe('deny')
    expect(gate.decide('a', 'allow')).toBe(false)
  })

  it('expires a pending approval and rejects late decisions', async () => {
    const gate = new InMemoryApprovalGate()
    const approval = createApprovalRequest({
      id: 'a',
      threadId: 't',
    ...
    expect(gate.expire('a', 'turn aborted while awaiting approval')).toBe(true)
    await expect(pending).rejects.toThrow('turn aborted while awaiting approval')
    expect(gate.pending()).toHaveLength(0)
    expect(gate.get('a')?.status).toBe('expired')
    expect(gate.decide('a', 'allow')).toBe(false)
    expect(gate.expire('a')).toBe(false)
  })
...
describe('LocalToolHost', () => {
  ...
  it('annotates bash approval summaries with shell safety signals', async () => {
    const host = new LocalToolHost({ tools: defaultLocalTools })
    const approvals: string[] = []
    const result = await host.execute(
      { callId: 'c1', toolName: 'bash', arguments: { command: 'git status && rm -rf /tmp/x' } },
      {
        threadId: 'th',
        turnId: 'tu',
        workspace: '/tmp',
    ...
    expect(approvals[0]).toContain('Run bash(command=')
    expect(approvals[0]).toContain('Shell safety: review required (shell syntax detected).')
  })
  ...
  it('updates in-memory session items in place', async () => {
    ...
    await store.appendItem('th', {
      ...
      callId: 'c1',
      toolKind: 'tool_call',
      output: makePublicToolResultWithheldProjection({ status: 'unknown' }),
      isError: false
    })

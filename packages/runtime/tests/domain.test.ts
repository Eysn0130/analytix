...
describe('domain.thread', () => {
  ...
  it('derives listing preview and counts without exposing full turns', () => {
    const thread = createThreadRecord({
      id: 'thr_summary',
      title: 'summary',
      workspace: '/tmp',
    ...
    const summary = toThreadSummary({
      ...thread,
      turns: [startTurn(appendTurnItem(appendTurnItem(turn, user), assistant))]
    })

    expect(summary).not.toHaveProperty('turns')
    expect(summary.preview).toBe('first question')
...
describe('domain.turn', () => {
  ...
  it('replaces an existing item with the same id', () => {
    ...
    expect(JSON.stringify(next.items[0])).not.toContain('exit_code')
  })
...
describe('domain.usage', () => {
  it('adds two usage snapshots and reports a cache hit rate', () => {
    ...
    expect(merged.cacheableTokenHitRate).toBeCloseTo(9 / 10)
    expect(merged.totalInputTokenHitRate).toBeCloseTo(9 / 300)
  })

  it('unions cache miss diagnostics while folding usage', () => {
    const merged = addUsage(
      ...
      },
      {
        ...
        cacheHitTokens: 80,
        cacheMissTokens: 20,
        cacheMissReasons: ['tool_catalog_changed', 'model_changed'],
        cacheSuggestions: ['keep tools stable', 'keep model stable']
      }
    )

    expect(merged.cacheableTokenHitRate).toBeCloseTo(120 / 200)
    expect(merged.totalInputTokenHitRate).toBeCloseTo(120 / 200)
    expect(merged.cacheMissReasons).toEqual(['cold_request', 'tool_catalog_changed', 'model_changed'])
    expect(merged.cacheSuggestions).toEqual(['keep tools stable', 'keep model stable'])
  })

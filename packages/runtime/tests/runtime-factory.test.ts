...
import { seedUsageCarryover } from '../src/shared/usage-carryover.js'
...
describe('runtime factory subagent tool boundary', () => {
  it('keeps the retired TypeScript composition root deleted', async () => {
    const runtimeFactory = new URL('../src/server/runtime-factory.ts', import.meta.url)
    const serveEntry = await readFile(new URL('../src/cli/serve-entry.ts', import.meta.url), 'utf8')

    expect(existsSync(runtimeFactory)).toBe(false)
    expect(serveEntry).not.toContain('../server/runtime-factory')
    expect(serveEntry).toContain("argsPrefix: ['run', '-tags', 'analytix_prod', './cmd/runtime-server']")
  })
})

describe('runtime factory environment diagnostics boundary', () => {
  it('keeps the Go runtime-server as the only serve-time diagnostics implementation', async () => {
    const serveEntry = await readFile(new URL('../src/cli/serve-entry.ts', import.meta.url), 'utf8')

    expect(serveEntry).toContain('startGoRuntimeServe')
    expect(serveEntry).toContain('GO_RUNTIME_SERVER_READY_PREFIX')
    expect(serveEntry).toContain('!ready.productionRuntime')
    expect(serveEntry).toContain('projectAnalytixServeReadyPublicV1({')
    expect(serveEntry).not.toContain('productionRuntime: server.ready.productionRuntime === true')
    expect(serveEntry).not.toContain('startAnalytixServe')
  })
})

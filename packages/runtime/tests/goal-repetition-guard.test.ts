...
function makeGoalTools(getHarness: () => Harness) {
  return [
    ...
    LocalToolHost.defineTool({
      ...
      description: 'Complete goal step',
      inputSchema: {
        type: 'object',
        properties: {
          step: { type: 'string' },
      ...
      policy: 'auto',
      execute: async (args, context) => ({
        output: await getHarness().threads.appendGoalEvidence(context.threadId, {
          step: String(args.step),
          evidence: Array.isArray(args.evidence) ? args.evidence.map((item) => String(item)) : [],
        ...
        async *stream(request): AsyncIterable<ModelStreamChunk> {
          ...
          if (calls === 3) {
            yield {
              ...
              arguments: {
                step: 'Build command recovery verified',
                evidence: ['goal recovery prompt led to an evidence-bearing completion step']
              }
            }
        ...
        async *stream(request): AsyncIterable<ModelStreamChunk> {
          ...
          if (calls === 3) {
            yield {
              ...
              toolName: COMPLETE_STEP_TOOL_NAME,
              arguments: {
                step: 'Benchmark note finished',
                evidence: ['distinct no-tool progress produced an evidence-bearing completion step']
              }
            }
            yield { kind: 'completed', stopReason: 'tool_calls' }
            return

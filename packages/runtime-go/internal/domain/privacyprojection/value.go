...
// value while still giving typed PII keys precedence. It is intentionally more
...
func publicValueNeedsProjection(value any, textSubtree bool, typed Kind) bool {
	...
	case map[string]any:
		for key, child := range current {
			if inputID, ok := hostUserInputQuestionsAuthority(current, key); ok {
...
// authority. Other values named questions remain untrusted text subtrees.

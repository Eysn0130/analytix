...
func MatchRuntimeRoute(path string) RuntimeRouteMatch {
	...
	case path == LocalDisplayImportMappingPathV1 || path == LocalDisplayCleaningDiffPathV1 ||
		...
		path == HostFundsDeterministicCleaningPathV1 || path == HostFundsCleaningRevokePathV1 || path == ObjectEditingPath || path == WorkspaceReadPath || path == PluginPackageHostPath || path == GeneratedArtifactPath || path == OfficePrivateAdmissionPath:

...
func writeCanvasSkillSourceV1(t *testing.T) (string, []byte) {
	t.Helper()
	root := writeDevelopmentSourceV1(t, "analytix-canvas")
	...
	declaration.Contributions.Skills = []domainpackage.PathContributionV1{{ID: domainpackage.CanvasSkillContributionIDV1, Path: domainpackage.CanvasSkillRelativePathV1}}
	...
	skillPath := filepath.Join(root, filepath.FromSlash(domainpackage.CanvasSkillRelativePathV1))
...
func TestCanvasSkillTreeBindsActualBytesAndInstalledGenerationV1(t *testing.T) {
	ctx := context.Background()
	source, skill := writeCanvasSkillSourceV1(t)
	...
	if err != nil || registration.CanvasSkillSHA256 != hex.EncodeToString(digest[:]) || registration.SourceTreeFileCount != 5 {
	...
	if err := os.WriteFile(filepath.Join(source, filepath.FromSlash(domainpackage.CanvasSkillRelativePathV1)), changedSkill, 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(active, filepath.FromSlash(domainpackage.CanvasSkillRelativePathV1)), []byte("tampered installed Skill"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestCanvasSkillTreeRejectsMissingAndSymlinkedContributionV1(t *testing.T) {
	for _, name := range []string{"missing", "symlink"} {
		t.Run(name, func(t *testing.T) {
			source, skill := writeCanvasSkillSourceV1(t)
			path := filepath.Join(source, filepath.FromSlash(domainpackage.CanvasSkillRelativePathV1))
...
func TestCanvasActualPluginSourceHasFixedFiveFileContributionV1(t *testing.T) {
	source, err := filepath.Abs("../../../../../../plugins/analytix-canvas")
	if err != nil {
		t.Fatal(err)
	}
	...
	registration, err := domainpackage.ParseDevelopmentSourceRegistrationV1([]byte(observed.SourceRegistrationJSON))
	if err != nil || registration.Identity.PackageID != "analytix-canvas" || registration.SourceTreeFileCount != 5 || registration.CanvasSkillSHA256 == "" {
		t.Fatal("actual source not admitted", err)
	}
}

...
func documentsRuntimeFixture(t *testing.T) (*runtimeServerHandler, hostapp.PackageView) {
	...
	root, err := filepath.Abs("../../../../plugins/analytix-documents")
	if err != nil {
		t.Fatal(err)
	}
	...
	store, err := pluginstore.NewPackageStoreV1(home, "analytix-documents", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	view, err := host.SetDesiredState(ctx, hostapp.SetDesiredStateRequest{PackageID: "analytix-documents", GenerationID: installed.Receipt.GenerationID, DesiredState: domainplugin.DesiredEnabledV1})
	if err != nil {
		t.Fatal(err)
	}
	return h, view
...
func TestDocumentsRuntimeDiscoveryInlineAndPreparedGenerationFollowActivation(t *testing.T) {
	h, view := documentsRuntimeFixture(t)
	ctx := context.Background()
	if !h.runtimeToolCatalog().DocumentGeneration {
		t.Fatal("enabled generation not advertised")
	}
	if _, ok := h.runtimeSkillByName("$analytix-documents"); !ok {
		t.Fatal("installed skill not discovered")
	}
	args := map[string]any{"name": "analytix-documents", "arguments": "Create a synthetic report."}
	...
	pending := runtimePendingToolCall{Call: provider.ToolCall{Name: "generate_office_document", Arguments: json.RawMessage(`{"kind":"docx","path":"report.docx","markdown":"Synthetic"}`)}, ExecutionGrant: domainsecurity.ExecutionGrant{ToolName: "generate_office_document"}, SecurityContext: domainsecurity.TurnSecurityContext{WorkspaceRealPath: t.TempDir()}}
	...
	if _, ok := h.runtimeSkillByName(toolcatalogapp.DocumentsSkillID); ok {

...
func BuiltinToolSchemas(input BuiltinToolSchemaInput) []domainmodel.ToolSchema {
	...
	tools := []domainmodel.ToolSchema{
		{Name: "read_task_history", Description: "Read a bounded page of original user history referenced by the current task continuation. Sources are chronological user messages, not permanent permissions or case evidence. Newer corrections supersede older requests. This reads only an opaque reference in the current thread and workspace; offset/limit count Unicode characters.", Parameters: json.RawMessage(`{"type":"object","properties":{"reference":{"type":"string","pattern":"^(hist1_[a-p]{64}|[a-f0-9]{64})$"},"offset":{"type":"integer","minimum":0},"limit":{"type":"integer","minimum":1,"maximum":16000}},"required":["reference"],"additionalProperties":false}`)},
		{
			Name:        "read",

...
> migration background. Current analytix runtime and closure rules live in
> `docs/analytix/specs/01-06`, `AGENTS.md`, and `docs/AGENTS.md`.
...
- 让发送给上游模型的请求前缀尽可能字节稳定。
- 让缓存命中统计优先使用 provider 原生字段，而不是依赖猜测。
...
Kun 的 cache-first 设计按 GUI 场景做了收束：
...
- 避免上游模型 provider 因消息结构不合法返回 400
...
Kun 的缓存命中统计优先使用 provider 原生 usage 字段：
...
原因是 provider 原生 miss 口径不保证等于 `prompt_tokens - hit`。如果分母错了，
...
接近模型实际上下文压力，也能在工具 continuation 前保住热前缀占比。
...
- LLM fold summarizer：未来若使用模型做 compaction，应复用主前缀，避免
  summarizer 自己变成冷请求
- 大工具结果 token cap：当前已加入轻量 token-aware 估算；如未来内置精确
  tokenizer，可改为 token 精确上限
- volatile scratch 边界：把“展示给用户的思考”与“重放给模型的历史”继续分离

## 相关文档

- 架构总览：`docs/legacy/kun/kun-architecture.md`
- Kun 贡献指南：`docs/legacy/kun/kun-contributing.md`

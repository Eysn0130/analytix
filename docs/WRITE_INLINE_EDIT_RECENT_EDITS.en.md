...
1. The user manually changes the first `Project GUI` to `Write mode`.
...
- Will check word boundaries to avoid accidentally replacing local strings in `myproject gui`.

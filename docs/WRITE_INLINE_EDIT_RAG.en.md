...
- When users manually perform one-time phrase replacement, they will first use deterministic rules to replace other identical phrases in the same paragraph, such as `project gui -> Project GUI`.
...
```json
{
  ...
  "messages": [
    {
      "role": "system",
      "content": "You are Project GUI inline writing..."
    },
    {
      "role": "user",
      "content": "<<<PREFIX ... >>>\n<<<EDIT_SCOPE ... >>>\n<<<SUFFIX ... >>>"
...
Additionally, term casing and simple renaming are not entirely model dependent. The editor will propagate one-time phrase replacements in the same paragraph: when you change `project gui` to `Project GUI` or `DXGUI`, other `project gui` in the same natural paragraph will be replaced simultaneously. This deterministic layer is responsible for the consistency of "must happen", and recent edits and RAG are responsible for understanding this intent during subsequent AI edits.
...
- `src/main/ipc/app-ipc-schemas.ts`, `src/preload/index.ts`, `src/shared/analytix-api.ts`: the existing `write:inline-completion` IPC accepts `mode: "edit"` plus `editCandidate` and recent edits.
